...
SET ANSI_NULLS ON
GO
CREATE VIEW [dbo].[SUMASX]
--with encryption
AS

--VISTA
select RucE, Ejer, Cd_Fte, left(NroCta,2) as NroCta, 
...
from Voucher 
where RucE=(select RucE from PrdoCons) and (Prdo>=(select PrdoI from PrdoCons) and Prdo<=(select PrdoF from PrdoCons) or Prdo='00') and  IB_Anulado=0 
group by RucE, Ejer, Cd_Fte, left(NroCta,2) --order by RucE, NroCta
