...
CREATE  FUNCTION [dbo].[verificarAutNvl](@RucE nvarchar(11), @Cd_Doc char(10), @nivel int, @IB_AutComNiv bit, @TipoDoc int )
RETURNS BIT AS  
BEGIN
	declare @idNiv int, @num1 int, @num2 int, @comp bit
			
	if(@TipoDoc = 0)
	begin
		select @idNiv = id_niv, @comp = IB_AutComNiv from cfgnivelaut 
	...
	begin
		...
		and tipo = (select tipAut from solicitudCom where RucE = @RucE and Cd_SCo = @Cd_Doc)) 
		and niv = @nivel
