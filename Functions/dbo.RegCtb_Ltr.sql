...
SET ANSI_NULLS ON
GO
CREATE FUNCTION [dbo].[RegCtb_Ltr](@RucE nvarchar(11), @Cd_MR nvarchar(2), @Cd_Area nvarchar(6), @Ejer nvarchar(4), @Prdo nvarchar(2))
...
begin 
      ...
      select @c = count(RegCtb) from Canje where RucE=@RucE and Ejer=@Ejer and Prdo=@Prdo and Cd_Area=@Cd_Area
      if @c=0
	begin
	...
	begin
	select @c=max(right(RegCtb,5)) from Canje where RucE=@RucE and Ejer=@Ejer and Prdo=@Prdo and Cd_Area=@Cd_Area
