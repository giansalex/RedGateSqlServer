...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [user321].[Inv_InventarioConsUn_x_RegCtb_paGR_1]
@RucE nvarchar(11),
@RegCtb char(15),
@Ejer nvarchar(4),
	...
	if(@IC_ES = 'S')
  		begin
			...
			begin 
				...
				where i.RucE = @RucE and i.Ejer = @Ejer and i.RegCtb = @RegCtb and i.Cd_Prod is not null and IC_ES = 'S'
			end
			else 
			begin
				...
				left join Prod_UM as UMP on UMP.RucE = i.RucE and UMP.Cd_Prod = i.Cd_Prod and UMP.ID_UMP = i.ID_UMP
				left join UnidadMedida as UM  on UM.Cd_UM = UMP.Cd_UM
				where i.RucE = @RucE and i.Ejer = @Ejer and i.RegCtb = @RegCtb and IC_ES = 'S'
			end 
  		end
  	else if (@IC_ES = 'E')
  	begin
...
-- exec user321.Inv_InventarioConsUn_x_RegCtb_paGR '11111111111','INGE_LD02-00014','2012',''



-- select * from Inventario where RucE = '11111111111' and IC_ES = 'E'
