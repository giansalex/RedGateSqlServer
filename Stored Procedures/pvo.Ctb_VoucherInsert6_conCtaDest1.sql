...
SET ANSI_NULLS ON
GO
CREATE procedure [pvo].[Ctb_VoucherInsert6_conCtaDest1]
@RucE		nvarchar(11),
--@Cd_Vou	int,
@Ejer		nvarchar(4),
@Prdo		nvarchar(2),
...
print '---- Entramos a SP: [Ctb_VoucherInsert6_conCtaDest1] -----'
...
if(select IB_Aux from PlanCtas where RucE=@RucE and Ejer=@Ejer and NroCta=@NroCta )=0
begin
	--set @Cd_Aux = null
	set @Cd_Clt = null
	set @Cd_Prv = null
...
if(select IB_NDoc from PlanCtas where RucE=@RucE and Ejer=@Ejer and NroCta=@NroCta )=0
begin
	set @Cd_TD = null
	set @NroSre = null
	set @NroDoc = null
...
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	if(select IB_CtaD from PlanCtas where RucE=@RucE and Ejer=@Ejer and NroCta=@NroCta )=1
	begin
...
'20000000002','2011','05' ,'CTGN_RC05-00802','RC'   ,'12/05/2011',null   ,'94.3.2.50',null   ,null   ,null  ,null   ,null   ,null  ,  null,'facturas por cobrar',0.00  ,100.00, 0.00, '01'   ,'02'    ,2.75    ,'01010101','01010101','01010101','010101','03'  ,null     ,'01'  ,'a'      ,'jvega' ,'S'         ,null    ,null , 0        ,null     ,null    ,null    ,null    ,'M'    ,null
@RucE,        @Ejer, @Prdo, @RegCtb         ,@Cd_Fte,@FecMov     ,@FecCbr,@NroCta    ,@Cd_Clt,@Cd_Prv,@Cd_TD,@NroSre,@NroDoc,@FecED,@FecVD, @Glosa              ,@MtoOr,@MtoD ,@MtoH,@Cd_MdOr,@Cd_MdRg,@CamMda ,@Cd_CC	  ,@Cd_SC    ,@Cd_SS	,@Cd_Area,@Cd_MR,@NroChke ,@Cd_TG,@IC_CtrMd,@UsuCrea,@IC_TipAfec ,@TipOper,@Grdo,@IB_EsProv,@DR_FecED,@DR_CdTD,@DR_NSre,@DR_NDoc,@TipMov,@msj
...
--PV: MIE 27/06/2012 --> Mdf: se arreglo error que invertia amarre en ME. Cita: if(@MtoD>0 or @MtoD_ME>0) --> cuando es 'M' el monto ingresado es la BIM y va al Debe Ejem: 63.1.0.01   100
--																					set @IB_InvertAmr = 0 --> es normal
