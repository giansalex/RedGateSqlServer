...
CREATE procedure [dbo].[Inv_Producto2Cons_paInv20]
@RucE nvarchar(11),
@Cd_Com nvarchar(10),
@Cd_Vta nvarchar(10),
@Cd_OC nvarchar(10),
...
begin  -- MM: VERIFICACION DE SALDO
	...
	from (
		select RucE, Cd_SR, Cd_Prod, ID_UMP, sum(case when (Cant<0) then 0 else Cant end) as 'Total', sum(Cant) as 'Saldo' from(
			...
			union all
			--select RucE, Cd_SR, Cd_Prod, isnull(-Cant_Ing,0) as Cant, ID_UMP --(esta mal)
			--select RucE, Cd_SR, Cd_Prod, isnull(Cant_Ing,0) as Cant, ID_UMP  --(esta bien)
			select RucE, Cd_SR, Cd_Prod, isnull(-abs(Cant_Ing),0) as Cant, ID_UMP --(esta bien)
...
-- Venta : exec Inv_Producto2Cons_paInv20 '20545551641',null,null,null,null,'GR00000016','','','','','11/09/2012',''
