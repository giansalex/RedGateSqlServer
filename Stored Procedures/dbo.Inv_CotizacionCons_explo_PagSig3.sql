...
CREATE PROCEDURE [dbo].[Inv_CotizacionCons_explo_PagSig3]
...
--exec Inv_CotizacionCons_explo_PagSig3 '11111111111','01/08/2012','31/08/2012','','',500,'',null,null,null,null,null
--------------------------------------
...
if(@Colum = 'Cd_Cot') Set @Cond = @Cond + ' and c.Cd_Cot like '''+@Dato+''' '
else if(@Colum = 'NroCot') Set @Cond = @Cond + ' and c.NroCot like '''+@Dato+''' '
else if(@Colum = 'FecEmi') Set @Cond = @Cond + ' and Convert(nvarchar,c.FecEmi,103) like '''+@Dato+''' '
else if(@Colum = 'FecCad') Set @Cond = @Cond + ' and Convert(nvarchar,c.FecCad,103) like '''+@Dato+''' '
else if(@Colum = 'Asunto') Set @Cond = @Cond + ' and c.Asunto like '''+@Dato+''' '
else if(@Colum = 'CostoTot') Set @Cond = @Cond + ' and c.CostoTot like '''+@Dato+''' '
...
set @Consulta='select  top '+Convert(nvarchar,@TamPag)+'
			...
			isnull(cfg.DescripTip, ''**No requiere autorizacion**'') as DescripTip,c.CA01,c.CA02,c.CA03,c.CA04,c.CA05
		from '+@Inter+
