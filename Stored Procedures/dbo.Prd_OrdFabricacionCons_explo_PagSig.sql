...
SET ANSI_NULLS ON
GO
CREATE proc [dbo].[Prd_OrdFabricacionCons_explo_PagSig]
...
set @Inter = 'OrdFabricacion ofb
		...
		left join CfgAutorizacion ca on ca.Tipo = ofb.TipAut and Cd_DMA = ''OF'''
...
begin
	set @Cond = ' Cd_OF not in (select CdOF_BASE from ordfabricacion where CdOF_BASE is not null) and ofb.RucE= '''+@RucE+''' and ofb.FecE between '''+
...
set @Consulta = 'select top '+Convert(nvarchar,@TamPag)+' ofb.RucE, ofb.Cd_OF, ofb.NroOF, convert(nvarchar,ofb.FecE,103) as FecE, 
		...
		ofb.AutorizadoPor, ofb.CA01, ofb.CA02, ofb.CA03, ofb.CA04, ofb.CA05, ofb.CA06, ofb.CA07, ofb.CA08, ofb.CA09, ofb.CA10, ofb.IB_Aut
		from '
		+@Inter+' 
...
-- exec Prd_OrdFabricacionCons_explo_PagSig  '11111111111', '01/02/2010', '01/04/2011', null, null, 100, '', null, null, null, null, null
