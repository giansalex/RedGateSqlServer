...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Rpt_BalanceComprobacion2]
...
Begin 
	if(len(@NroCtaD) = 2)	Set @RangoD = ' and left(s.NroCta,2)>='''+@NroCtaD+''''--' and left(s.NroCta,2)>='+@NroCtaD
	else if(len(@NroCtaD) = 4) Set @RangoD = ' and left(s.NroCta,4)>='''+@NroCtaD+''''--' and left(s.NroCta,4)>='+@NroCtaD
	else if(len(@NroCtaD) = 6) Set @RangoD =' and left(s.NroCta,6)>='''+@NroCtaD+''''--' and left(s.NroCta,6)>='+@NroCtaD
	else Set @RangoD = ' and s.NroCta>='''+@NroCtaD+''''--' and s.NroCta>='+@NroCtaD
End
if(isnull(len(@NroCtaH),0) = 0) 
	set @NroCtaH = '9999999999'
else
Begin 
	if(len(@NroCtaH) = 2)	Set @RangoH = ' and left(s.NroCta,2)<='''+@NroCtaH+''''--' and left(s.NroCta,2)<='+@NroCtaH
	else if(len(@NroCtaH) = 4) Set @RangoH = ' and left(s.NroCta,4)<='''+@NroCtaH+''''--' and left(s.NroCta,4)<='+@NroCtaH
	else if(len(@NroCtaH) = 6) Set @RangoH = ' and left(s.NroCta,6)<='''+@NroCtaH+''''--' and left(s.NroCta,6)<='+@NroCtaH
	else Set @RangoH = ' and s.NroCta<='''+@NroCtaH+''''--' and s.NroCta<='+@NroCtaH
End
...
-- DI 18/11/09 Creacion : se creo el procedimient almacenado con los cambios de CC,SC,SS acorde a las nuevas ventanas
GO
