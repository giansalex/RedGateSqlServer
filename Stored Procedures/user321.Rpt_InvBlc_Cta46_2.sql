...
SET ANSI_NULLS ON
GO
CREATE Procedure [user321].[Rpt_InvBlc_Cta46_2]
@RucE nvarchar(11),
@Ejer varchar(4),
@Cd_Mda char(2),
@FecIni datetime,
...
as
	...
	begin
	--Cuerpo Reporte
		select 	Max(pv.Cd_TDI) as Cd_TDI, Max(pv.NDoc) as NDoc,Max(vou.Cd_Prv) as Cd_Prv, 
		case(isnull(max(pv.RSocial),'')) when '' then Max(pv.ApPat) + ' ' + Max(pv.ApMat) + ', '+ Max(pv.Nom) else Max(pv.RSocial) end as Proveedor,
		...
		from 	Voucher vou left join proveedor2 pv on vou.RucE=pv.RucE  and Vou.Cd_Prv=pv.Cd_Prv 
