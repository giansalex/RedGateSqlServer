...
CREATE proc [dbo].[Inv_CotizacionCons_explo_PagAnt_SoloAutorizadas]
...
Declare @Ult_CdCot nvarchar(15)	Set @Ult_CdCot = 'CT00000003'
...
begin
	Set @Cond = ' c.RucE='''+@RucE+''' and c.Cd_Cot < '''+Convert(nvarchar,@Ult_CdCot)+'''and ((isnull(c.TipAut,0) = 0) or (isnull(c.TipAut,0) != 0 and isnull(c.IB_Aut,0) = 1))'
end
else
begin
	...
	Convert(nvarchar,@Ult_CdCot)+'''and ((isnull(c.TipAut,0) = 0) or (isnull(c.TipAut,0) != 0 and isnull(c.IB_Aut,0) = 1))'
end
...
Set @Consulta = 'select * from (select  top '+Convert(nvarchar,@TamPag)+'
		...
		from '+@Tablas+
		'where'+@Cond+ ' Order by c.RucE, c.Cd_Cot desc) as Cotizacion order by Cd_Cot'
...
/*----------------------------------------------------------------------------*/
	...
	set @sql = 'select top 1 @Maxi = c.Cd_Cot from '+@Tablas+' where '+@Cond+' order by c.RucE, c.Cd_Cot desc'
	print @sql
...
/*----------------------------------------------------------------------------*/
	...
	set @sql = 'select @Min = min(Cd_Cot) from (select top '+Convert(nvarchar,@TamPag)+' c.Cd_Cot from '+@Tablas+' where '+@Cond+' Order by c.RucE,c.Cd_Cot desc) as CotizacionX'
	print @sql
	exec sp_executesql @sql,N'@Min varchar(15) output',@Min output
...
-- MM : 06/06/2011 <SE AGREGO LOS CAMPOS DE AUTORIZACIONES>
-- MM : 08/11/2011 <Modificacion para mostrar solo los autorizados en OP>

/********************************** PRUEBAS *********************************/
/*
declare @Max nvarchar(15)
declare @Min nvarchar(15)
exec Inv_CotizacionCons_explo_PagAnt '11111111111','2010','04','04','','',2,'2','CT00000003',@NroRegs output, @NroPags output,@Max output,@Min output,null
