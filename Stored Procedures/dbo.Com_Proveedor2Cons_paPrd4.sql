...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Com_Proveedor2Cons_paPrd4]
...
select	@check as Sel, pv.RucE,pv.Cd_Prv, case(isnull(len(pv.RSocial),@check))when @check then pv.ApPat + '' '' + pv.ApMat + '' '' + pv.Nom 
		else pv.RSocial end as Nombre, TDI.NCorto as descrip , pv.NDoc, pv.Correo
from(
...
where pv.RucE = '''+@RucE+'''
group by pv.RucE, pv.Cd_Prv,pv.ApPat,pv.ApMat,pv.Nom,pv.RSocial,pv.NDoc,TDI.NCorto,pv.Correo')
GO
