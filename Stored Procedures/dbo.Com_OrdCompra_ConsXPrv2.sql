...
SET ANSI_NULLS ON
GO
CREATE proc [dbo].[Com_OrdCompra_ConsXPrv2]
@RucE nvarchar(11),
@Cd_Prv char(7),
@EstaAut bit,
...
from OrdCompra co
where co.RucE = @RucE and co.Cd_Prv = @Cd_Prv and co.Id_EstOC in ('01','02','04','05','07','08') and (co.IB_Aut = 1 or co.TipAut = 0 or co.TipAut is null)
...
convert(Nvarchar,co.FecE,103) as FecED 
from OrdCompra co
where co.RucE = @RucE and co.Cd_Prv = @Cd_Prv and co.Id_EstOC in ('01','02','04','05','07','08')
order by  year(co.FecE) desc, month(co.FecE) desc, day(co.FecE) desc
END
...
-- CAM : 13/07/11 : <Creacion del procedimiento almacenado 2>
