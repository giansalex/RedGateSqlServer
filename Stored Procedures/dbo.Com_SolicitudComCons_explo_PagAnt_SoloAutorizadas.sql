...
CREATE procedure [dbo].[Com_SolicitudComCons_explo_PagAnt_SoloAutorizadas] --<Explorador de Solicitud de Compras>
@RucE nvarchar(11),
--@Ejer nvarchar(4),
@FecD datetime,
@FecH datetime,
...
as
	...
	begin
		set @Cond = 'sc.RucE= '''+@RucE+ ''' and sc.Cd_SCo<'''+isnull(@Ult_CdSC,'')+''' and ((isnull(sc.TipAut,0) = 0) or (isnull(sc.TipAut,0) != 0 and isnull(IB_Aut,0) = 1))'
	end
	else
	begin
		      set @Cond = 'sc.RucE= '''+@RucE+''' and sc.FecEmi between '''+
 		     Convert(nvarchar(10),@FecD,103)+''' and '''+Convert(nvarchar(10),@FecH,103)+
		     ''' and sc.Cd_SCo<'''+isnull(@Ult_CdSC,'')+''' and ((isnull(sc.TipAut,0) = 0) or (isnull(sc.TipAut,0) != 0 and isnull(IB_Aut,0) = 1))'
	end
	if(@esParaOC = 1) set @Cond = @Cond + ' and sc.Id_EstSC in (''02'',''03'',''04'',''08'',''11'')'
