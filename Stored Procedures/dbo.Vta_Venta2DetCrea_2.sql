...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Vta_Venta2DetCrea_2]
@RucE nvarchar(11),
@Cd_Vta nvarchar(10),
@Nro_RegVdt int,
--@Cd_Pro nvarchar(7),
...
set @Nro_RegVdt=dbo.Nro_RegVdt(@RucE,@Cd_Vta)
	insert into VentaDet(RucE,Cd_Vta,Nro_RegVdt,Cd_Pro_NO,Cant,Valor,DsctoP,DsctoI,IMP,IGV,Total,CA01,CA02,CA03,CA04,CA05,CA06,CA07,CA08,CA09,CA10,
			     FecReg,UsuCrea,Cd_CC,Cd_SC,Cd_SS,Cd_Prod,Cd_Srv,Descrip,ID_UMP,PU,Obs,Cd_Alm,Cd_IAV,CU, Costo,CU_ME, Costo_ME)
		      values(@RucE,@Cd_Vta,@Nro_RegVdt,null,@Cant,@Valor,@DsctoP,@DsctoI,@IMP,isnull(@IGV,0.00),@Total,@CA01,@CA02,@CA03,@CA04,@CA05,@CA06,@CA07,@CA08,@CA09,@CA10,
			     GETDATE(),@UsuCrea,@Cd_CC,@Cd_SC,@Cd_SS,@Cd_Prod,@Cd_Srv,@Descrip,@ID_UMP,@PU,@Obs,@Cd_Alm,@Cd_IAV,@CU,@Costo,@CU_ME,@Costo_ME)
	if @@rowcount <= 0
	Set @msj = 'Error al registrar detalle de Venta'
end
-- Leyenda --
-- JJ : 2010-09-14 : <Creacion del procedimiento almacenado>
-- MP : 2011-03-17 : <Modificacion del procemiento almacenado>
