...
SET ANSI_NULLS ON
GO
CREATE proc [dbo].[Vta_OrdPedidoCons_Explo_PagSig_Aut1]
...
as
	...
	join(
	select cd_doc as ''Cd_OPAut'' from(
	select ac.cd_dma, isnull(opd.cd_op,'''')as CD_DOC, isnull(aop.cd_op, '''') as CD_DOC_AUT
	...
	left join ordpedido opd on opd.RucE = '''+@RucE+''' and ac.cd_dma = ''OP'' and opd.tipaut = ac.tipo and (opd.ib_aut = 0 or opd.ib_aut is null)
	left join autop aop on aop.RucE = '''+@RucE+''' and aop.NomUsu = '''+@usuario+''' and opd.cd_op = aop.cd_op
	where opd.cd_op is not null 
	) OPAut 
	where cd_doc != cd_doc_aut
	) OrdPedAut on op.Cd_op = OrdPedAut.Cd_OPAut and op.RucE = '''+@RucE+''''
...
--	set @Cond = 'op.RucE= '''+@RucE+''' and op.FecE between '''+Convert(nvarchar,@FecD,103)+''' and '''+Convert(nvarchar,@FecH,103)+''' and op.Cd_OP >'''+isnull(@Ult_CdOP,'')+''''
	...
	begin
		set @Cond = 'op.RucE= '''+@RucE+''' and op.FecE <= '''+	Convert(nvarchar,@FecH,103)+''' and op.Cd_OP >'''+isnull(@Ult_CdOP,'')+''''
/*
...
*/
	...
	set @Consulta = 'select top ' +Convert(nvarchar,@Tampag)+'	op.Cd_OP,op.NroOP,op.Cd_Vdr, (ve.ApPat + '' '' + ve.ApMat +'', ''+ve.Nom) as Vendedor,
			...
			, isnull(op.tipAut, 0) as TipAut, isnull(cfg.DescripTip, ''**No requiere autorizacion**'') as DescripTip, op.AutorizadoPor, case op.IB_Aut when 1 then 1 else 0 end as IB_Aut,s.Descrip  
			from '
			+@Inter+' 
