...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Rpt_RegistroVentas_X_RegCtb]
@RucE nvarchar(11),
@Eje nvarchar(4),
@Rprdo1 nvarchar(2),
@Rprdo2 nvarchar(2),
...
select
	...
	/*case(isnull(len(ca.RSocial),0))
	     when 0 then ca.ApPat+' '+ca.ApMat+' '+ca.Nom else ca.RSocial end as NomCli,*/ --<<-- Modificado en linea 31
	case(isnull(len(c2.RSocial),0))
	     when 0 then c2.ApPat+' '+c2.ApMat+' '+c2.Nom else c2.RSocial end as NomCli, --<<-- Nueva
	
	/*v.BIM, v.EXO, v.INF, v.IGV, v.Total,*/RegCtb as Grupo,@Rprdo1+' - '+@Rprdo2+' del '+@Eje as Prdo, --v.CamMda
  ...
  --   Case(IB_Anulado)  when 0 then ( case(v.Cd_Mda) when '02' then v.Total*v.CamMda else v.Total end) else '0.00' end as Total, 
	...
	Cliente2 c2 --<<-- Nueva. la tabla Aux no es necesaria
	where v.RucE=@RucE and v.Eje=@Eje and  v.Prdo >= @Rprdo1 and v.Prdo <= @Rprdo2 and v.RucE=e.Ruc and 
		...
		-- v.Cd_Cte=c.Cd_Aux and c.RucE=ca.RucE and c.Cd_Aux=ca.Cd_Aux --and v.IB_Anulado<>1 --<<-- Modificado linea 61
...
--sp_help Rpt_RegistroVentas_X_RegCtb
--exec Rpt_RegistroVentas_X_RegCtb '11111111111','2010','01','09','01',''
