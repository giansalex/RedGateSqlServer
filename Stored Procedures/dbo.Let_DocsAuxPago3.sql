...
SET ANSI_NULLS ON
GO
CREATE PROC [dbo].[Let_DocsAuxPago3]
...
Declare @Cd_Prv char(10)	Set @Cd_Prv='PRV0004'
...
Begin
	select 
		...
		doc.Cd_Com,'' As Cd_Vou,Max(doc.Cd_Ltr) As Cd_Ltr,convert(varchar,doc.Cd_TD) As Cd_TD,t.NCorto As NomTD, doc.NroSre, doc.NroDoc,doc.Cd_Mda,
...
Begin
	...
	--***********************************************************************
	SELECT
		Res.DR_CdTD,Res.DR_NSre,Res.DR_NDoc,Res.Cd_Com,Res.Cd_Vou,Res.Cd_Ltr,
	...
	SELECT
		...
		'' As NomMda,
		Tb.CamMda,
		Sum(Tb.TotalS) As SaldoS,
		Sum(Tb.TotalD) As SaldoD
		,Convert(bit,0) As Sel
    FROM 
    (
		SELECT 
	...
	Left Join (	Select 
				...
				From 
					CanjePagoDet d
					Inner Join CanjePago c On c.RucE=d.RucE and c.Cd_Cnj=d.Cd_Cnj
					Left Join Compra p On p.RucE=d.RucE and p.Cd_Com=d.Cd_Com
					Left Join Voucher v On v.RucE=c.RucE and v.RegCtb=c.RegCtb
				Where
					d.RucE=@RucE and c.Cd_Prv=@Cd_Prv and isnull(v.RegCtb,'')=''
				Group by
	...
	SELECT 
	Res.DR_CdTD,Res.DR_NSre,Res.DR_NDoc,Res.Cd_Com,Res.Cd_Vou,Res.Cd_Ltr,Res.Cd_TD,Res.NomTD,Res.NroSre,Res.NroDoc,Res.Cd_Mda,Res.NomMda,Res.CamMda,
	Res.SaldoS-isnull(Ref.TotalS,0) As SaldoS,
	Res.SaldoD-isnull(Ref.TotalD,0) As SaldoD,
	Res.Sel
	FROM
	...
	Left Join (	Select 
					c.Cd_Prv,d.Cd_Com,d.Cd_Vou,d.Cd_Ltr,
					Case When isnull(d.Cd_Com,'')<>'' Then p.Cd_TD Else d.Cd_TD End As Cd_TD,
					Case When isnull(d.Cd_Com,'')<>'' Then p.NroSre Else d.NroSre End As NroSre,
					Case When isnull(d.Cd_Com,'')<>'' Then p.NroDoc Else d.NroDoc End As NroDoc,
					d.Cd_Mda,
					c.TipCam,
