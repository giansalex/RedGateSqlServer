...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Ctb_VoucherCons4_PagSig2]
@RucE nvarchar(11),
@Eje nvarchar(4),
@PrdoIni nvarchar(2),
@PrdoFin nvarchar(2),
...
SET CONCAT_NULL_YIELDS_NULL off --Para que se pueda concatenar NULLs --PV 
	...
	set @Consulta ='		select top '+Convert(nvarchar,@TamPag)+'
		...
		case vou.IB_EsProv when 1 then 1 else 0 end as IB_EsProv, --Indicador si es provicion
		...
		vou.Grdo, --Girado
		case vou.IB_Conc when 1 then 1 else 0 end as IB_Conc, -- Indicador booleano conciliado
		...
		case vou.IB_Anulado when 1 then 1 else 0 end as IB_Anulado, --Indicador si esta anulado
		vou.DR_NSre,
		vou.DR_NDoc,
		vou.DR_NroDet,
		vou.DR_FecDet
