...
CREATE PROCEDURE [dbo].[SP_kbrVentas]
@RucE nvarchar(11),
@Ejer nvarchar(4),
@Cd_Vdr nvarchar(7),
@FecIni datetime,
...
,case when @Cd_Mda = v.Cd_Mda then isnull(vd.Valor,0.00) else case when @Cd_Mda = '01' then isnull(v.CamMda,0.00)*isnull(vd.Valor,0.00) else isnull(vd.Valor,0.00)/isnull(v.CamMda,0.00)end end as Valor_Det
,case when @Cd_Mda = v.Cd_Mda then isnull(vd.DsctoI,0.00) else case when @Cd_Mda = '01' then isnull(v.CamMda,0.00)*isnull(vd.DsctoI,0.00) else isnull(vd.DsctoI,0.00)/isnull(v.CamMda,0.00)end end as DsctoI_Det
,case when @Cd_Mda = v.Cd_Mda then isnull(vd.IMP,0.00) else case when @Cd_Mda = '01' then isnull(v.CamMda,0.00)*isnull(vd.IMP,0.00) else isnull(vd.IMP,0.00)/isnull(v.CamMda,0.00)end end as BIM_Det
,case when @Cd_Mda = v.Cd_Mda then isnull(vd.IGV,0.00) else case when @Cd_Mda = '01' then isnull(v.CamMda,0.00)*isnull(vd.IGV,0.00) else isnull(vd.IGV,0.00)/isnull(v.CamMda,0.00)end end as IGV_Det
,case when @Cd_Mda = v.Cd_Mda then isnull(vd.Total,0.00) else case when @Cd_Mda = '01' then isnull(v.CamMda,0.00)*isnull(vd.Total,0.00) else isnull(vd.Total,0.00)/isnull(v.CamMda,0.00)end end as Total_Det
...
v.RucE = @RucE
 ...
 and v.Eje = @Ejer and v.FecMov between @FecIni and @FecFin--and v.Cd_Vta = 'VT00001202'
and case when isnull(@Cd_Vdr,'') <> '' then isnull(v.Cd_Vdr,'') else '' end = isnull(@Cd_Vdr,'') 
END

select*from Venta where Venta.Cd_Mda='01'
GO
