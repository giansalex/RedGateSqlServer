...
SET ANSI_NULLS ON
GO
CREATE procedure [pvo].[Ctb_VoucherInsert4Imdo3_Import] --(Se usa solo para importar voucher)
@RucE		nvarchar(11),
--@Cd_Vou	int,
@Ejer		nvarchar(4),
@Prdo		nvarchar(2),
...
if not exists ( select * from Cliente2 where RucE=@RucE and Cd_Clt=@Cd_Clt and Cd_TDI=@Cd_TDI ) and (ISNULL(@Cd_Clt,'')<>'')
begin
set @msj = 'Codigo de cliente no existe: '+@Cd_Clt 
return	
end

if not exists ( select * from Proveedor2 where RucE=@RucE and Cd_Prv=@Cd_Prv) and (ISNULL(@Cd_Prv,'')<>'')
begin
set @msj = 'Codigo de Proveedor no existe: '+@Cd_Prv 
return	
end
