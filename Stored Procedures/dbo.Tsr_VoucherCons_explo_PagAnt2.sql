...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Tsr_VoucherCons_explo_PagAnt2]
@RucE nvarchar(11),
@Ejer nvarchar(4),
@PrdoIni nvarchar(2),
@PrdoFin nvarchar(2),
...
as
	...
	set @Consulta = '	select *from(select top '+ convert(nvarchar,@TamPag) +'
			...
			vou.IC_TipAfec,case vou.IB_EsProv when 1 then convert(bit,1) else convert(bit,0) end as IB_EsProv, vou.Ejer,vou.Prdo,convert(varchar(10),vou.FecMov,103) as FecMov, 
...
--FL : 13/06/2011	: Se agrego la columna ncorto
