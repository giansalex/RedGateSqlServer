...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Seg_UsuCreaxNivel4]
@NomUsu	nvarchar(10),
...
begin
    insert into Usuario(NomUsu,Pass,NomComp,Cd_Trab,Cd_Prf,Nivel,Estado,IB_TipCamCrear,IB_TipCamMdf,IB_TipCamElim,Correo1,UsuCrea,FecReg, [GUID])
	        values (@NomUsu,@Pass,@NomComp,@Cd_Trab,@Cd_Prf,
			dbo.Asigna_NivelxUsuario(@NomUsuCrea),1,@IB_TipCamCrear,@IB_TipCamMdf,@IB_TipCamElim,@correo1,@UsuCrea, @FecReg,@GUID)
    if @@rowcount<=0
