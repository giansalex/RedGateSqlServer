...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Rpt_InventarioKardex]
@RucE nvarchar(11),
@Cd_Prod char(7),
@Cd_Mda char(2) = '01',
@Id_UMP int,
...
begin
			...
			0.00 as ECant ,0.00 as ECosUnt,0.00 as ETotal,0.00 as SCant ,0.00 as SCosUnt,0.00 as STotal,
			isnull(Suma_Cantidad,0) as Suma_Cantidad, isnull(CProm,0) as CProm, isnull(SCT,0) as SCT
			 from(
				...
				select a.Cd_Inv, a.Cd_Prod, d.Nombre1 as NombreProducto ,d.CodCo1_ as CodCom, isnull(a.FecMov,'--/--/----') as FecMov, 				
					...
					--Case(a.IC_ES) when 'E' then Convert(nvarchar,a.Cant) else 0.00 end as ECant ,
					--Case(a.IC_ES) when 'E' then Convert(nvarchar,case(@Cd_Mda) when '01' then a.CosUnt else a.CosUnt_ME end) else 0.00 end as ECosUnt,
					--Case(a.IC_ES) when 'E' then Convert(nvarchar,case(@Cd_Mda) when '01' then a.Total else a.Total_ME end) else 0.00 end as ETotal,
					--Case(a.IC_ES) when 'S' then Convert(nvarchar,a.Cant*-1) else 0.00 end as SCant ,
					--Case(a.IC_ES) when 'S' then Convert(nvarchar,case(@Cd_Mda) when '01' then a.CosUnt else a.CosUnt_ME end) else 0.00 end as SCosUnt,
					--Case(a.IC_ES) when 'S' then Convert(nvarchar,case(@Cd_Mda) when '01' then a.Total*-1 else a.Total_ME*-1 end) else 0.00 end as STotal,
