...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Ctb_VoucherCons4_PagSig1]
@RucE nvarchar(11),
@Eje nvarchar(4),
@PrdoIni nvarchar(2),
@PrdoFin nvarchar(2),
...
SET CONCAT_NULL_YIELDS_NULL off --Para que se pueda concatenar NULLs --PV 
	...
	begin
		...
		else if(@Dato like '%Dólares%') set @Dato = '%$%'
	...
	set @Consulta ='		select top '+Convert(nvarchar,@TamPag)+'
		...
		(case(IC_CtrMd) when ''$'' then ''Dólares'' else '''' end) end) end as ''CtrMdaDetalle'', 
		vou.IC_TipAfec, --Indicador Tipo Afecto
		vou.IB_EsProv, --Indicador si es provicion
		vou.Ejer, --Ejericio o aÃƒÂ±o de registro
		...
		vou.DR_NroDet,
		vou.DR_FecDet
		--,Case When (Select Count(d.Id_Doc) From DocsVou d Where d.RucE=vou.RucE and d.RegCtb=vou.RegCtb)>0 Then 1 Else 0 End as DocAdd
		'
	...
	--Primer Metodo (+ EFICIENTE) (paracerÃƒÂ­a que se hace lento con top's altos, pero las pruebas demostraron lo contrario)
