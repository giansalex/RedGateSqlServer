...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Tsr_DocsAuxCons2]
@RucE nvarchar(11),
...
begin	
	...
	select * from (
		...
		where v.RucE=@RucE /*and v.Ejer=@Ejer*/ and (v.Cd_Clt=@Cd_Aux /* or v.Cd_Prv=@Cd_Aux*/) /*v.Cd_Aux=@Cd_Aux*/ and p.Ejer=@Ejer and v.RucE=p.RucE and v.NroCta=p.NroCta and p.IB_CtasXCbr<>0 and isnull(v.IB_Cndo,0)!=1 and isnull(v.IB_Anulado,0)!=1
...
begin
	...
	select * from (
		...
		from Voucher v, PlanCtas p 
		where v.RucE=@RucE/* and v.Ejer=@Ejer*/ and (/*v.Cd_Clt=@Cd_Aux or*/ v.Cd_Prv=@Cd_Aux) /*v.Cd_Aux=@Cd_Aux*/and p.Ejer=@Ejer and  v.RucE=p.RucE and v.NroCta=p.NroCta and p.IB_CtasXPag<>0 and isnull(v.IB_Cndo,0)!=1 and isnull(v.IB_Anulado,0)!=1
...
begin
	...
	select 
		...
		--sum(case(IB_EsProv) when '1' then v.CamMda else 0 end) as TC_Org --> Tipo de Cam. de la Provision
	from Voucher v, PlanCtas p 
	where v.RucE=@RucE/* and v.Ejer=@Ejer*/ and (v.Cd_Clt=@Cd_Aux or v.Cd_Prv=@Cd_Aux) /*v.Cd_Aux=@Cd_Aux*/and p.Ejer=@Ejer  and v.RucE=p.RucE and v.NroCta=p.NroCta and (p.IB_CtasXCbr<>0 or p.IB_CtasXPag<>0) and isnull(v.IB_Cndo,0)!=1 and isnull(v.IB_Anulado,0)!=1
	Group by /*v.Cd_Vou,v.RegCtb,*/v.NroCta,p.NomCta,v.Cd_TD,v.NroSre,v.NroDoc--,v.Cd_MdRg --, v.Cd_Vou --case(IB_EsProv) when '1' then v.Cd_Vou else 0 end--,v.Cd_Vou
	having Sum(v.MtoD-v.MtoH)!=0
