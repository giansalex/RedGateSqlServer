...
SET ANSI_NULLS ON
GO
create proc [dbo].[Com_OrdCompra_Cons_Imprimir]
@RucE nvarchar(11),
@EstaAut bit,
...
/*and Cd_Prod is not null*/ group by Cd_OC)/* and co.Id_EstOC in ('01','02','04','05')*/ and (co.ib_aut = 1 or co.tipAut = 0 or co.tipAut is null)
...
convert(Nvarchar,co.FecE,103) as FecED from OrdCompra co
where co.RucE = @RucE and Cd_OC in(select Cd_OC from OrdCompraDet where RucE = @RucE 
/*and Cd_Prod is not null*/ group by Cd_OC) /*and co.Id_EstOC in ('01','02','04','05')*/
--order by convert(Nvarchar,  co.FecMov,102) desc
order by year(co.FecE) desc, month(co.FecE) desc ,day(co.FecE) desc, SUBSTRING ( CONVERT(char(38),  co.FecE,121), 12,8) desc
END
...
-- exec Com_OrdCompra_Cons_Imprimir '20464722841',false,''

--select * from OrdCompra
--select * from EstadoOC
GO
