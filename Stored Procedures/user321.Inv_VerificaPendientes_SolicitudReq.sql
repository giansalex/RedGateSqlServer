...
SET ANSI_NULLS ON
GO
CREATE procedure [user321].[Inv_VerificaPendientes_SolicitudReq]
@RucE nvarchar(11),
@Cd_SR char(10),
...
select @pend = sum(t.pendiente) from (select Cant-ABS(isnull((select sum(Cant_Ing) from Inventario i where i.RucE = c.RucE and i.Cd_SR = c.Cd_SR and i.Cd_Prod=c.Cd_Prod and i.Item = c.Item),0) ) as pendiente 
...
-- LEYENDA
-- CAM <25/01/2011><Creacion del SP>
-- CAM 04/02/2011 - Cambio de Inv_Inventario_EstadoUpd a Inv_Inventario_EstadoUpd_1 que tiene GR
-- CAM 15/02/2011 - se cambio la invocacion de Inv_Inventario_EstadoUpd_1 a Inv_Inventario_EstadoUpd_2
-- CAM 28/02/2011 - se cambio la invocacion de Inv_Inventario_EstadoUpd_2 a Inv_Inventario_EstadoUpd_3
GO
