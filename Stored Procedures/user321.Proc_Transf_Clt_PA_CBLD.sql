...
SET ANSI_NULLS ON
GO
CREATE Procedure [user321].[Proc_Transf_Clt_PA_CBLD]
@RucE nvarchar(11),
@Cd_Aux char(7),
@Cd_Clt nchar(10) output
AS


--Set @Cd_Clt=dbo.Cod_Prv2(@RucE)
...
--Insert Into Cliente2(RucE,Cd_Clt,Cd_TDI,NDoc,RSocial,ApPat,ApMat,Nom,Cd_Pais,CodPost,Ubigeo,Direc,Telf1,Telf2,
...
Fetch Next From _Cursor Into @Cd_TDI, @NDoc, @RSocial, @ApPat, @ApMat, @Nom, @Cd_Pais,
	...
	While @@Fetch_status = 0 
            Begin
            if exists (select top 1 *from Cliente2 where RucE='''+@RucE+''' and Cd_TDI=@Cd_TDI and NDoc=@NDoc)
            Begin
				update Cliente2 set Cd_Aux='''+@Cd_Aux+''' where Cd_TDI=@Cd_TDI and NDoc=@NDoc
				select @Cd_Clt = Cd_Clt from Cliente2 Where Cd_TDI=@Cd_TDI  and NDoc=@NDoc
            End
            else
            Begin
            ...
            Insert Into Cliente2(RucE,Cd_Clt,Cd_TDI,NDoc,RSocial,ApPat,ApMat,Nom,Cd_Pais,CodPost,Ubigeo,Direc,Telf1,Telf2,
			        Fax,Correo,PWeb,Obs,Estado,CA01,CA02,CA03,CA04,CA05, CA06,CA07,CA08,CA09,CA10)
			Values('''+@RucE+''',@Cd_Clt,@Cd_TDI,@NDoc,@RSocial,@ApPat,@ApMat,@Nom,@Cd_Pais,@CodPost,@Ubigeo,@Direc,@Telf1,@Telf2,
			        @Fax,@Correo,@PWeb,@Obs,@Estado,@CA01,@CA02,@CA03,@CA04,@CA05,@CA06,@CA07,@CA08,@CA09,@CA10)
			Select @Cd_Clt
			End
            Fetch Next From _Cursor Into @Cd_TDI, @NDoc, @RSocial, @ApPat, @ApMat, @Nom, @Cd_Pais,
...
--print @VarAux
exec(@VarAux)
--exec SP_ExecuteSql @VarAux,N'@Cd_Clt nchar(7) output', @Cd_Clt out
GO
