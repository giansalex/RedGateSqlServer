...
CREATE procedure [user321].[DAOT_Exportacion_Sunat]
@RucE nvarchar(11),
@Ejer varchar(4),
...
from (
select t.RucE,Ejer,t.Cd_Prv,Max(t.RegCtb) RegCtb, Max(FecMov) FecMov, p2.NDoc, 
...
from (
      select  t.RucE,t.Ejer,t.RegCtb, FecMov, t.Cd_Prv,t.Cd_TD 
		...
		from(
		   select  v.RucE, v.Ejer, v.RegCtb,Max(v.FecMov) FecMov, Max(v.Cd_Prv) Cd_Prv, Max(Cd_TD) Cd_TD
			...
			from  Voucher v
			 left join PlanCtas p on p.RucE=v.RucE and p.Ejer=v.Ejer and p.NroCta=v.NroCta and Isnull(p.IB_CtasxPag,0)=1 where 
				...
				and Cd_TD not in('00','02') and ISNULL(Cd_Prv,'')<>'' and Cd_Fte='RC')
...
-- Leyenda --
--JJ 24/02/2011 : <Creacion del Procedimiento Almacenado>
GO
