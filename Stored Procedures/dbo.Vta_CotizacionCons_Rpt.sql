...
SET ANSI_NULLS ON
GO
CREATE PROC [dbo].[Vta_CotizacionCons_Rpt]
...
Set @SQL_P2=
	...
	Select 
		...
		isnull(d.CU,0) As CU,
		isnull(d.Costo,0.00) As Costo,
		isnull(d.PU,0.00) As PU,
		isnull(d.Cant,0.00) As Cant,
		isnull(d.Valor,0.00) As Valor,
		isnull(d.DsctoP,0.00) As DsctoP,
		isnull(d.DsctoI,0.00) As DsctoI,
		isnull(d.BIM,0.00) As BIM,
		isnull(d.IGV,0.00) As IGV,
		isnull(d.Total,0.00) As Total,
		isnull(d.MU_Porc,0.00) As MU_Porc,
		isnull(d.MU_Imp,0.00) As MU_Imp,
		isnull(d.Obs,'''') As Obs,
		d.CA01,
		d.CA02,
		d.CA03,
