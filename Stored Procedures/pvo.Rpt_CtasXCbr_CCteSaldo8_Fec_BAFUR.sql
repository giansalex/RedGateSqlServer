...
SET ANSI_NULLS ON
GO
CREATE procedure [pvo].[Rpt_CtasXCbr_CCteSaldo8_Fec_BAFUR]
@RucE nvarchar(11),
@Ejer nvarchar(4),
...
begin
		...
		select 	
			...
			-- DESPUES (MEJORA)
			Max(case(v.IB_EsProv) when '1' then convert(varchar,v.FecMov,103) else case when left(v.NroCta,2)='18' Then convert(varchar,v.FecMov,103) else '' end end) as FecED, -- else '00/00/0000' end) as FecED, 
			...
			sum(case(v.IB_EsProv) when '1' then datediff(day,@FechaFin,IsNull(v.FecCbr,IsNuLL(v.FecED,IsNull(v.FecMov,'Sin Especificacion')))) else 0 end) as Saldo_Dias, --ver 
			v.glosa,--Lo agrege (Javier)
		...
		group by v.regctb,c.NDoc, c.ApPat,c.ApMat,c.Nom,c.RSocial, v.NroCta, /*v.Cd_Aux,*/ v.Cd_TD,v.NroSre,v.NroDoc,v.Cd_Prv,pr.NDoc,pr.ApPat,pr.ApMat,pr.Nom,pr.RSocial,v.Cd_Clt,v.glosa--,v.Cd_MdRg, convert(varchar,v.FecMov,103), /*v.FecVD,*/ datediff(day,v.FecMov,@FechaAl) --,v.Cd_MdRg
		...
		--order by NomAux,FecED --,v.Cd_TD,v.NroSre, v.NroDoc,FecED,a.NDoc,
		order by NomAux,convert(datetime,max(case(v.IB_EsProv) when '1' then convert(varchar,v.FecMov,103) else case when left(v.NroCta,2)='18' Then convert(varchar,v.FecMov,103) else '' end end))
...
--select *from Cliente2
		...
		group by v.regctb,c.NDoc, c.ApPat,c.ApMat,c.Nom,c.RSocial, v.NroCta, v.Cd_TD,v.NroSre,v.NroDoc,v.Cd_Prv,pr.NDoc,pr.ApPat,pr.ApMat,pr.Nom,pr.RSocial,v.Cd_Clt,v.glosa
		having sum(case(@Cd_Mda) when '01' then (v.MtoD) else (v.MtoD_ME) end)-sum(case(@Cd_Mda) when '01' then (v.MtoH) else (v.MtoH_ME) end) + @VarNum <>0
		--order by NomAux ,FecED--,v.Cd_TD,v.NroSre, v.NroDoc,FecED,a.NDoc
		order by NomAux,convert(datetime,max(case(v.IB_EsProv) when '1' then convert(varchar,v.FecMov,103) else case when left(v.NroCta,2)='18' Then convert(varchar,v.FecMov,103) else '' end end))
end
...
--pvo.Rpt_CtasXCbr_CCteSaldo8_Fec_BAFUR '20100977037','2011','','18.9.1.90','18.9.1.90','01/01/2011','31/05/2011','01',0,null

--DEMO
--exec pvo.Rpt_CtasXCbr_CCteSaldo8_Fec_BAFUR '20100977037','2011','CLT0000277','18.9.1.90','18.9.1.90','01/01/2011','30/04/2011','01',1,null
