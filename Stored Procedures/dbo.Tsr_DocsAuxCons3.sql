...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Tsr_DocsAuxCons3]
@RucE nvarchar(11),
@Ejer varchar(4),
@Cd_Clt char(10),
@Cd_Prv char(7),
...
select	Cd_Vou, NroCta, NomCta, Case When Isnull(DR_CdTD,'''')='''' Then TD Else DR_CdTD End As DR_CdTD
		...
		MdReg, TC_Org from (
		select 
			sum(case(IB_EsProv) when ''1'' then v.Cd_Vou else 0 end) as Cd_Vou, Max(v.DR_CdTD) as DR_CdTD, Max(v.DR_NSre) as DR_NSre, Max(v.DR_NDoc) as DR_NDoc
...
select 	Case When Conteo=1 then Convert(bit,1) else Convert(bit,0) End As Jala
		...
		, '''' Sre, '''' NroDoc, SaldoS, SaldoD, MdReg
