...
--exec Rpt_VentaDetallada1 '20522276236','2013','01','','','01/03/2013','31/03/2013','','','','','',''
CREATE PROCEDURE [dbo].[Rpt_VentaDetallada1]
--declare
@RucE nvarchar(11),
@Ejer varchar(4),
@Cd_Mda char(2),
...
--SET @RucE = '20522276236'
--SET @Ejer = '2013'
--Set @Cd_Mda = '01'
--SET @vendedor = ''
--Set @Area = ''
--Set @FecIni =  '01/03/2013'
--SET @FecFin = '31/03/2013'
--SET @msj = ''
--Set @Cd_Clt = 'CLT0000030'
--SET @Cd_CC = ''
--SET @Cd_SC = ''
--SEt @Cd_SS = ''
--Set @Cd_Prod = ''
...
set @Cons1='
	SELECT 
		...
		cast(case when Cd_Mda='''+Convert(varchar,Isnull(@Cd_Mda,''))+''' then  v.Total  else case when isnull('''+Convert(varchar,isnull(@Cd_Mda,''))+''','''')=''01'' then v.Total*v.CamMda else  v.Total/v.CamMda end end as decimal(16,2)) as Total
	...
	Where v.RucE='''+Convert(varchar,@RucE)+''' and v.Eje='''+Convert(Varchar,@Ejer)+'''
		...
		group by v.Prdo, v.nrosre, v.nrodoc, v.fecmov, cl.ndoc, cl.rsocial, cl.appat, cl.apmat, cl.nom, v.BIM_Neto,v.total,v.cammda,v.cd_mda, v.IGV, v.cd_vdr, v2.ndoc, v.cd_vta, v2.rsocial, v2.appat, v2.apmat, v2.nom
	'
...
set @Cons2='
	SELECT 
		...
		vd.Cd_CC, vd.Cd_SC, vd.Cd_SS,  vd.Valor,vd.Cant,vd.IMP,vd.IGV ,vd.total, '''' cd_Vdr, '''' Ndoc_Vdr, v.cd_Vta , '''' as NomVdr
	FROM VENTA V 
		INNER JOIN VENTADET vd on vd.RucE=v.RucE and vd.Cd_Vta=v.Cd_Vta
		left join Vendedor2 as v2 on v.RucE=v2.RucE and v.Cd_Vdr=v2.Cd_Vdr
		inner join Cliente2  as cl on v.RucE=cl.RucE and v.Cd_Clt= cl.Cd_Clt
...
--MODIFICADO POR RAFAEL 01/04/2013 AGREGADO GROUP BY EN LA CABECERA
GO
