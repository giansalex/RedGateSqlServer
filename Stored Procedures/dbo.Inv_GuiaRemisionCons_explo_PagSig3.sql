...
CREATE procedure [dbo].[Inv_GuiaRemisionCons_explo_PagSig3]
@RucE nvarchar(11),
@FecD datetime,
@FecH datetime,
@Colum varchar(100),
...
as
	...
	set @Consulta = '	select top '+Convert(nvarchar,@TamPag)+'
			...
			--						then  isnull((case(isnull(len(c2.RSocial),0)) when 0 then isnull(c2.ApPat,'''')+'' ''+isnull(c2.ApMat,'''')+''- ''+isnull(c2.Nom,'''') else c2.ApPat end),''-'')
			--						else c2.RSocial end 
			--					  )end as Cliente,  
...
--exec Inv_GuiaRemisionCons_explo_PagSig2 '11111111111','01/02/2011','01/06/2011',null,null,100,null,null,null,null,null,S,null
GO
