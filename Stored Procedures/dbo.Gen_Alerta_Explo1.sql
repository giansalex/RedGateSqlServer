...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[Gen_Alerta_Explo1]
@RucE char(11),
@Ejer varchar(4),
@FecD date,
@FecH date,
...
BEGIN
	...
	INSERT INTO @TempDETRACCION (Pendiente,Vencido)  exec Rpt_Detraccion_Alerta1 @RucE,@Ejer,@FecD,@FecH
	UPDATE @TempDETRACCION set RucE=@RucE , Cd_TA='02', DescripTA='Alerta de Detracciones'
								 
	INSERT INTO @Temp(Nro)  exec Rpt_CtasXCbr_Detallada_CtbNroVR1 @RucE,@Ejer,'01',null,null,null,0,@FecD,@FecH,@Dias
	INSERT INTO @Temp1(Nro)  exec Rpt_CtasXCbr_Detallada_CtbNroVN1 @RucE,@Ejer,'01',null,null,null,0,@FecD,@FecH
	INSERT INTO @Temp2(Nro)	 exec Rpt_CtasXPag_Detallada_CtbNroVR1 @RucE,@Ejer,'01',null,null,null,0,@FecD,@FecH,@Dias
	INSERT INTO @Temp3(Nro)	 exec Rpt_CtasXPag_Detallada_CtbNroVN1 @RucE,@Ejer,'01',null,null,null,0,@FecD,@FecH

	SET NOCOUNT ON;

	select au.RucE,au.Cd_TA,au.DescripTA as DescripTA,NomUsu as usuario,tg.Nro as DocProdAfec,au.IB_NoRecordar as NRecordar, au.IB_RecProxIni as RInicio, au.IB_RecCada as RCada,au.IB_RecDentro as RDentro , RecordarCada , RecordarDentro
	...
	Where RucE = @RucE )as au
		inner join (
			...
			where (c.RucE=@RucE and datediff(d,@FecH,@FecD)<convert(int,@Dias) and datediff(d,@FecH,@FecD)>-1) or 
				  (c.RucE=@RucE and datediff(d,@FecH,@FecD)<0 and datediff(d,@FecH,@FecD)>-31)
			...
			--(X)
			select Cd_TA,DescripTA,usuario,(isnull(Pendiente,0)+isnull(Vencido,0)) as Nro
			 from( SELECT td.RucE,td.Cd_TA,DescripTA,au.NomUsu as usuario,Pendiente,Vencido 
			...
			where ct.RucE = @RucE and (IB_Aut is null or IB_Aut = 0) and TipAut !=0 and  YEAR(ct.FecEmi)<=@Ejer and convert(date,ct.FecEmi) between @FecD AND @FecH
			...
			where oc.RucE = @RucE and (IB_Aut is null or IB_Aut = 0) and TipAut !=0 and YEAR(oc.FecE)<=@Ejer and convert(date,oc.FecE) between @FecD AND @FecH
			...
			where ofb.RucE = @RucE and (IB_Aut is null or IB_Aut = 0) and TipAut !=0 and YEAR(ofb.FecE)<=@Ejer and convert(date,ofb.FecE) between @FecD AND @FecH
			...
			where op.RucE = @RucE and (IB_Aut is null or IB_Aut = 0) and TipAut !=0 and YEAR(op.FecE)<=@Ejer and convert(date,op.FecE) between @FecD AND @FecH
			...
			where sc.RucE = @RucE and (IB_Aut is null or IB_Aut = 0) and TipAut !=0 and YEAR(sc.FecEmi)<=@Ejer and convert(date,sc.FecEmi) between @FecD AND @FecH
			...
			case (niv) when 1 then (case (dbo.verificarAutNvl(@RucE, sr.cd_sr, 1, ib_autcomniv, 3)) when 1 then 'NO' else 'SI' end) 
				...
				where sr.RucE = @RucE and (IB_Aut is null or IB_Aut = 0) and TipAut !=0 and YEAR(sr.FecEmi)<=@Ejer   and convert(date,sr.FecEmi) between @FecD AND @FecH
			...
			where i.RucE = @RucE and Convert(date,i.FecMov) between @FecD and @FecH  
			...
			--where tg.Nro>0
			--where tg.Nro>0 and au.IB_RecProxIni=1  or (au.IB_RecProxIni=0 and  au.RecordarCada > 0 or au.IB_RecProxIni=0 and au.RecordarDentro>0)
			where tg.Nro>0 and (au.RecordarCada > 0 or au.RecordarDentro>0)
...
/* 26/01/2017  : Se creo este procedimiento para consultar los registro de tipo de alertas que tiene cada usuario dentro de cada empresa */
/* codigo de Ejecucion SP :  EXEC	Gen_Alerta_Explo1 '20160000001','2016','01/01/2016','31/12/2016',0,null */
