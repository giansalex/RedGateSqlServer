...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Inv_InventarioCons_explo_PagSig]
@RucE nvarchar(11),
@Cd_Prod char(7),
@FecD datetime,
@FecH datetime,
...
as
	...
	set @Cond = 'a.RucE=''' + @RucE + ''' and a.Cd_Prod='''+@Cd_Prod+''' and a.FecMov between '''+ convert(nvarchar,@FecD,103)+''' and '''+Convert(nvarchar,@FecH,103)+' 23:59:29'''
	...
	set @Consulta = '	select top '+ convert(nvarchar,@TamPag) +'
		...
		a.SCant as Suma_Cantidad, a.CProm'+@x+' as CProm, a.SCT'+@x+' as SCT, a.Cd_CC, a.Cd_SC, a.Cd_SS, a.Cd_OF,  a.NroGC,
...
-- Epsilower 13/04/2011 11:01:32:293 : Modificacion Correcccion del los max y min, reorden y en la condicion
...
@MaxFec datetime,
@MinFec datetime 
exec dbo.Inv_InventarioCons_explo_PagSig '11111111111','PD00042','01/01/2012','31/12/2012',null, null, '01', 10, null, null, @NroRegs output, @NroPags output, @Max output, @Min output, @MaxFec output, @MinFec output, null
...
select top 10  *  from Inventario a 
		inner join Almacen e on e.Cd_Alm=a.Cd_Alm and e.RucE = a.RucE
		inner join Producto2 d on d.Cd_Prod=a.Cd_Prod and d.RucE = a.RucE
		left join TipDocES f on f.Cd_TDES=a.Cd_TDES and f.RucE = a.RucE
		left join TIPDOC g on g.Cd_TD=a.Cd_TD 
		...
		left join Area h on h.Cd_Area=a.Cd_Area and h.RucE=a.RucE
		left join MtvoIngSal i on i.Cd_MIS=a.Cd_MIS and a.RucE=i.RucE
where a.RucE='11111111111' and a.Cd_Prod='PD00001' and a.FecMov between '01/01/2010' and '31/12/2011' and a.FecMov >= '03/08/2010 14:19:00:000' and not (a.Cd_Inv <= 'INV000000125' and  a.FecMov = '03/08/2010 14:19:00:000')
order by a.FecMov, a.Cd_Inv

select top 1 Cd_Inv, FecMov from Inventario a 
		inner join Almacen e on e.Cd_Alm=a.Cd_Alm and e.RucE = a.RucE
		inner join Producto2 d on d.Cd_Prod=a.Cd_Prod and d.RucE = a.RucE
		left join TipDocES f on f.Cd_TDES=a.Cd_TDES and f.RucE = a.RucE
		left join TIPDOC g on g.Cd_TD=a.Cd_TD 
...
*/
--select * from Empresa where RSocial like '%MER %'
--  exec Inv_InventarioCons_explo_PagSig '20512141022','PD00434','01/03/2012','31/03/2012','','',500,'','',50,'','','','','',''
