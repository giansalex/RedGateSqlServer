...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [user321].[Cfg_ConsultarDocPendientesXUsuario]
@RucE nvarchar(11),
@usuario varchar(10)
as
...
left join solicitudcom sc on sc.RucE = @RucE and ac.cd_dma = 'SC' and sc.tipaut = ac.tipo and (sc.ib_aut = 0 or sc.ib_aut is null)
left join autsc atsc on atsc.RucE = @RucE and atsc.NomUsu = @usuario and sc.cd_sco = atsc.cd_sco
left join ordpedido op on op.RucE = @RucE and ac.cd_dma = 'OP' and op.tipaut = ac.tipo and (op.ib_aut = 0 or op.ib_aut is null)
