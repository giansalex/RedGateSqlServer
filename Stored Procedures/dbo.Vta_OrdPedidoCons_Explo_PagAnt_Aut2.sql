...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Vta_OrdPedidoCons_Explo_PagAnt_Aut2]
@RucE nvarchar(11),
@FecD datetime,
@FecH datetime,
@Colum varchar(100),
...
as
	...
	join(
		...
		where op.RucE = '''+@RucE+''' and (IB_Aut is null or IB_Aut = 0) and TipAut !=0
...
*/
	...
	set @Consulta=	'select *from(select top ' +Convert(nvarchar,@Tampag)+'	op.Cd_OP,op.NroOP,op.Cd_Vdr, (isnull(ve.ApPat,'''') + '' '' + isnull(ve.ApMat,'''') +case when isnull(ve.Nom,'''')<> '''' then '', '' else '''' end +isnull(ve.Nom,'''')) as Vendedor,
			...
			,isnull(op.tipAut, 0) as TipAut, isnull(cfg.DescripTip, ''**No requiere autorizacion**'') as DescripTip, op.AutorizadoPor, case op.IB_Aut when 1 then 1 else 0 end as IB_Aut,s.Descrip, op.FecVencimiento, op.Cd_SS , ct.NroCot
			from '
			+@Inter+' 
