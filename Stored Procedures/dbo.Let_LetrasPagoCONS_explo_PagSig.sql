...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Let_LetrasPagoCONS_explo_PagSig]
...
set language 'spanish'
	...
	--Interseccion
		set @Inter = 'CanjePago Can inner join Area Ar on Ar.Cd_Area = Can.Cd_Area and Can.RucE = Ar.RucE
	...
	else if(@Colum = 'Proveedor') set @Cond=@Cond +' and case(isnull(len(Can.Cd_Prv),0)) when 0 then '''' else case(isnull(len(Clt.RSocial),0)) when 0 then isnull(Clt.ApPat,'''')+'' ''+isnull(Clt.ApMat,'''')+'', ''+isnull(Clt.Nom,'''') else Clt.RSocial end end like '''+@Dato+''''
