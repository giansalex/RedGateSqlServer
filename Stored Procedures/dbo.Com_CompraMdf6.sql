...
SET ANSI_NULLS ON
GO
CREATE proc [dbo].[Com_CompraMdf6]
@RucE nvarchar(11),
@Cd_Com char(10),
@Ejer nvarchar(4),
@Prdo nvarchar(2),
...
if not exists (select * from Compra where RucE=@RucE and Cd_Com=@Cd_Com)
	Set @msj = 'No existe codigo de Compra' 
--------------
else if(User123.VPrdo(@RucE,@Ejer,SubString(@RegCtb,8,2)) = 1)
	set @msj = 'Compra no se puede modificar, el periodo '+User123.DamePeriodo(SubString(@RegCtb,8,2))+' no se encuentra habilitado'
--------------
...
begin 
	...
	CA23=@CA23, CA24=@CA24, CA25=@CA25, TipNC = @TipNC, ImpDetr=@ImpDetr, FecPagDtr=@FecPagDtr
