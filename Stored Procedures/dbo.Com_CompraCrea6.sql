...
CREATE proc [dbo].[Com_CompraCrea6]
@RucE nvarchar(11),
...
if isnull(@NroDoc,'')=''
begin
 	set @msj = 'Es obligatorio ingresar el numero documento'
	return 
end
...
if(User123.VPrdo(@RucE,@Ejer,SubString(@RegCtb,8,2)) = 1)
	set @msj = 'Compra no puede ser creada, el periodo '+User123.DamePeriodo(SubString(@RegCtb,8,2))+' no se encuentra habilitado'
------------------------
else if exists (select * from Compra where RucE=@RucE and Cd_Com=@Cd_Com)
...
begin 
	insert into Compra(RucE,Cd_Com,Ejer,Prdo,RegCtb,FecMov,Cd_FPC,FecApag,Cd_TD,NroSre,NroDoc,FecED,FecVD,Cd_Prv,
			   ...
			   CA07,CA08,CA09,CA10,CA11,CA12,CA13,CA14,CA15,CA16,CA17,CA18,CA19,CA20,CA21,CA22,CA23,CA24,CA25,TipNC,ImpDetr,FecPagDtr,ImpDetr_ME)
			values(@RucE,@Cd_Com,@Ejer,@Prdo,@RegCtb,@FecMov,@Cd_FPC,@FecApag,@Cd_TD,@NroSre,@NroDoc,@FecED,
			       ...
			       @CA19,@CA20,@CA21,@CA22,@CA23,@CA24,@CA25, @TipNC,@ImpDetr,@FecPagDtr,@ImpDetrME)
	if @@rowcount <= 0
