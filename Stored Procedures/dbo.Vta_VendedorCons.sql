...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Vta_VendedorCons]
@RucE nvarchar(11),
@TipCons int,
...
begin
	if(@TipCons=0)
	    begin
		select a.RucE,a.Cd_Aux as Cd_Vdr,b.NCorto as NCortoTDI,a.NDoc,a.RSocial,a.ApPat,a.ApMat,a.Nom,
		          a.Cd_Pais,a.CodPost,a.Ubigeo,a.Direc,a.Telf1,a.Telf2,a.Fax,a.Correo,a.PWeb,c.Cta,a.Estado
                from Auxiliar a, TipDocIdn b, Vendedor c where a.RucE=@RucE and a.Cd_Aux=c.Cd_Aux and a.RucE=c.RucE and a.Cd_TDI=b.Cd_TDI
	    end
	else if (@TipCons=1) 
	    begin
		select case(isnull(len(a.RSocial),0))
		       ...
		       a.Cd_Aux as Cd_Vdr
		from Auxiliar a, TipDocIdn b, Vendedor c where a.RucE=@RucE and a.Cd_Aux=c.Cd_Aux and a.RucE=c.RucE and a.Cd_TDI=b.Cd_TDI and a.Estado=1 and c.Estado=1
	    end
	else if (@TipCons=2)
	    begin
