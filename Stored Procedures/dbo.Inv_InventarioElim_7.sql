...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Inv_InventarioElim_7]
@RucE nvarchar(11),
@RegCtb nvarchar(15),
...
if(User123.VPrdo(@RucE,@Ejer,SubString(@RegCtb,8,2)) = 1)
	set @msj = 'Inventario no se puede eliminar, el periodo '+User123.DamePeriodo(SubString(@RegCtb,8,2))+' no se encuentra habilitado.'
...
else if(@UsuModf = 'j.garrido' and @RucE = '20512141022')
...
begin
	...
	--------------------------------------------------------------------------
	exec dbo.Inv_Lote_EliminarProductoxLote @RucE,@RegCtb,@Ejer,''
	---------------------------------------------------------------------------
...
-- CAM: 30/11/2012 Mdf: Eliminar lotes al eliminar el movimiento
