...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Inv_InventarioCons_Area_explo_PagAnt]
@RucE nvarchar(11),
@Cd_Prod char(7),
@Cd_Area nvarchar(6),
@FecD datetime,
...
as
	...
	--Condicion
	set @Cond = 'a.RucE=''' + @RucE + ''' and a.Cd_Prod='''+@Cd_Prod+''' and a.Cd_Area like '''+@Cd_Area+''' and a.FecMov between '''+ convert(nvarchar,@FecD,103)+''' and '''+Convert(nvarchar,@FecH,103)+' 23:59:59'' and a.FecMov <= '''+convert(nvarchar, @Ult_Fec, 103)+' '+convert(nvarchar, @Ult_Fec, 114) +''' and not (a.Cd_Inv >= '''+Convert(nvarchar, @Ult_CdInv) +''' and  a.FecMov = '''+convert(nvarchar, @Ult_Fec, 103)+' '+convert(nvarchar, @Ult_Fec, 114) +''')'
	print @Cond
...
-- Epsilower 14/04/2011 11:53:23:297 : Modificacion Correcccion del los max y min, reorden y en la condicion
...
select @NroRegs,@NroPags, @Min, @MinFec, @Max, @MaxFec
exec dbo.Inv_InventarioCons_Area_explo_PagAnt '11111111111','PD00001','0001','01/01/2010','31/12/2011',null, null, 10, @Min, @MinFec, @Max output, @Min output, @MaxFec output, @MinFec output, null
select @NroRegs,@NroPags, @Min, @MinFec, @Max, @MaxFec
exec dbo.Inv_InventarioCons_Area_explo_PagSig '11111111111','PD00001','0001','01/01/2010','31/12/2011',null, null, 10, @Max, @MaxFec, @NroRegs output, @NroPags output, @Max output, @Min output, @MaxFec output, @MinFec output, null
select @NroRegs,@NroPags, @Min, @MinFec, @Max, @MaxFec
exec dbo.Inv_InventarioCons_Area_explo_PagSig '11111111111','PD00001','0001','01/01/2010','31/12/2011',null, null, 10, @Max, @MaxFec, @NroRegs output, @NroPags output, @Max output, @Min output, @MaxFec output, @MinFec output, null
