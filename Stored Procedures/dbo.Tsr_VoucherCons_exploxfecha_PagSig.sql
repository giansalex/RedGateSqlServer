...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Tsr_VoucherCons_exploxfecha_PagSig]
@RucE nvarchar(11),
@Eje nvarchar(4),
@FechaIni datetime,
@FechaFin datetime,
...
as
	...
	set @Inter = 'Voucher vou
		...
		inner join Banco ban on vou.RucE=ban.RucE and vou.NroCta=ban.NroCta'
	--Condicion
	declare @Cond varchar(1000)
	declare @sql nvarchar(1000)

	set @Cond = 'vou.RucE='''+@RucE+''' and vou.Ejer='''+@Eje+''' and vou.FecMov between '''+ Convert(nvarchar,@FechaIni,103)+''' and ''' +Convert(nvarchar,@FechaFin,103) +''' and vou.Cd_Fte=''CB'' and vou.Cd_Vou > '''+ Convert(nvarchar, isnull(@Ult_CdTsr,'')) +''''
	...
	set @Consulta = '	select top '+ convert(nvarchar,@TamPag) +'
			...
			'+@inter+' 
			where 
			'+@Cond+ ' order by vou.Cd_Vou, vou.FecMov' 
	...
	if NOT  exists (select top 1 * from Voucher where RucE = @RucE and Ejer=@Eje)
	  ...
	  begin
		...
		if(@Ult_CdTsr is null) -- si es primera pagina y primera busqueda
		  ...
		  set @sql = 'select @RMax = max(Cd_Vou) from(select top '+ Convert(nvarchar,@TamPag)+' Cd_Vou  from ' + @Inter + ' where ' + @Cond +' order by Cd_Vou, FecMov) as  Tesoreria'
		  exec sp_executesql @sql, N'@RMax int output',@Max output
		  if(@Max is null) set @Max = 0

		  set @sql = 'select top 1 @RMin = Cd_Vou from ' + @Inter + ' where ' +@Cond+' order by Cd_Vou, FecMov'
