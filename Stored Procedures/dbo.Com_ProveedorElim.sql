...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Com_ProveedorElim]
@RucE nvarchar(11),
@Cd_Aux nvarchar(7),
...
if not exists (select * from Auxiliar where RucE=@RucE and Cd_Aux=@Cd_Aux)
	...
	set @msj = 'Proveedor ' + @Cd_Aux + ' no puede ser eliminado por tener data relacionada'
else
begin
begin transaction	
	...
	insert into AuxiliarRM(RucE,NroReg,Cd_Aux,Cd_TDI,NroDoc,Cd_TA,Cd_MR,Usu,FecMov,Cd_Est)
	        values(@RucE,@NroReg,@Cd_Aux,@Cd_TDI,@NDoc,'02','02',@Usu,getdate(),'03')
	-----------------------------------------------------------------------------------
