...
SET ANSI_NULLS ON
GO
CREATE proc [dbo].[Com_OrdCompraCons_explo_PagSig_SoloAutorizadas]
...
as
	...
	begin
		set @Cond = 'oc.RucE= '''+@RucE+''' and oc.Cd_OC >'''+isnull(@Ult_CdOC,'')+''' and ((isnull(oc.TipAut,0) = 0) or (isnull(oc.TipAut,0) != 0 and isnull(IB_Aut,0) = 1))'
	end
	else
	begin
		set @Cond = 'oc.RucE= '''+@RucE+'''
		and oc.FecE between '''+Convert(nvarchar,@FecD,103)+''' and '''+Convert(nvarchar,@FecH,103)+''' and oc.Cd_OC >'''+isnull(@Ult_CdOC,'')+''' and ((isnull(oc.TipAut,0) = 0) or (isnull(oc.TipAut,0) != 0 and isnull(IB_Aut,0) = 1))'
	end
	...
	oc.Cd_OC, oc.NroOC,	oc.Cd_Prv,pr.NDoc as RucProv, 
			...
			oc.AutorizadoPor, oc.UsuCrea,oc.UsuModf,oc.FecReg,oc.FecMdf,fpc.Cd_FPC,fpc.Nombre as DescripFPC,oc.FecAPag,
			s.Id_EstOC ,s.Descrip,
			srv.Descrip as EstadoSrv,
			oc.CA01, oc.CA02, oc.CA03, oc.CA04, oc.CA05, oc.CA06, oc.CA07, oc.CA08, oc.CA09, oc.CA10, oc.CA11, oc.CA12, oc.CA13, oc.CA14, oc.CA15 
			from '		
			+@Inter+' 
	...
	print 'cond: ' + @cond
	-- exec Com_OrdCompraCons_explo_PagSig_SoloAutorizadas '11111111111','01/01/2013','31/12/2013',null,null,50,null,null,null,null,null,null

-------------------------------------------------------------------------------------------------------------------------
-- Leyenda --
-- CAM: 13/02/2012 : <Creacion> 
