...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Vta_VendedorCrea]
@RucE nvarchar(11),
--@Cd_Aux nvarchar(7),
@Cd_TDI nvarchar(2),
@NDoc nvarchar(15),
...
as
if exists (select * from Auxiliar where RucE=@RucE and NDoc=@NDoc and left(Cd_Aux,3)='VND')
	set @msj = 'Ya existe Vendedor con el mismo numero de documento'
else
begin
begin transaction
declare @Cd_Aux varchar(7)
	set @Cd_Aux = (select user123.Cod_Vnd(@RucE))
	...
	insert into Auxiliar(RuCE,Cd_Aux,Cd_TDI,NDoc,RSocial,ApPat,ApMat,Nom,Cd_Pais,CodPost,
		     ...
		     values(@RuCE,@Cd_Aux,@Cd_TDI,@NDoc,@RSocial,@ApPat,@ApMat,@Nom,@Cd_Pais,@CodPost,
			    @Ubigeo,@Direc,@Telf1,@Telf2,@Fax,@Correo,@PWeb,@Obs,'03',1)
	if @@rowcount <= 0
	...
	insert into Vendedor(RucE,Cd_Aux,Cta,Estado)
	...
	insert into AuxiliarRM(RucE,NroReg,Cd_Aux,Cd_TDI,NroDoc,Cd_TA,Cd_MR,Usu,FecMov,Cd_Est)
	        values(@RucE,@NroReg,@Cd_Aux,@Cd_TDI,@NDoc,'03','01',@Usu,getdate(),'01')
	-----------------------------------------------------------------------------------
commit transaction
end
print @msj
