...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Vta_VentaCrea3]
@RucE nvarchar(11),
...
as
set @msj = 'Para eliminar, debe actualizar el sistema'


--(Soluc 1)  -- Desencadena un error que no devolviera los saldos a la hora de registrar el voucher (VER XQ)
...
/* NO SE PUEDE UTILIZAR ESTE METODO XQ NO TODOS LOS NUMERO DE DOCUMENTO SON NUMERICOS
	...
	begin	set @msj = 'Existe un numero de documento parecido en modulo contable, verificar.'
		print @msj
--		return
	end
*/
...
begin
	...
	insert intoVenta(RucE, Cd_Vta, Eje, Prdo, RegCtb, FecMov, Cd_FPC, FecCbr,Cd_TD,NroDoc,NroSre,Cd_Num,FecED,FecVD,Cd_Cte,Cd_Vdr,Cd_Area,Cd_MR,Obs/*,BIM,INF,EXO,IGV,Total*/,Cd_Mda,CamMda,FecReg,FecMdf,UsuCrea,UsuModf,IB_Anulado, DR_FecED, DR_CdTD, DR_NSre, DR_NDoc, Cd_CC, Cd_SC, Cd_SS)
		   values(@RucE,@Cd_Vta,@Eje, @Prdo, @RegCtb,@FecMov, @Cd_FPC, @FecCbr,@Cd_TD, @NroDoc,@Cd_Sr,@Cd_Num,@FecED,@FecVD,@Cd_Cte,@Cd_Vdr,@Cd_Area,@Cd_MR,@Obs/*,@BIM,@INF,@EXO,@IGV,@Total*/,@Cd_Mda,@CamMda,getdate(),getdate(),@UsuCrea,@UsuCrea,0, @DR_FecED, @DR_CdTD, @DR_NSre, @DR_NDoc, @Cd_CC, @Cd_SC, @Cd_SS)
...
--DI: Lun 19/01/09
--DI: Lun 23/01/09
--PV: Jue 13/02/09  --> Validacin RegCtb
