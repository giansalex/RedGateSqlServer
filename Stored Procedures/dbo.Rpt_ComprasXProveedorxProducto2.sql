...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[Rpt_ComprasXProveedorxProducto2] 
...
SELECT  Empresa.Ruc, Empresa.RSocial,  'Desde el ' + convert(varchar,@fecIni,103)+ ' hasta el ' + convert(varchar,@fecFin,103)   as 'Fecha Consulta', 'COMPRAS '+ case @Est_Imp when 0 then 'LOCALES' when 1 then 'POR IMPORTACIÓN' end +' POR PROVEEDOR POR PRODUCTO'  as IC_Imp   FROM Empresa WHERE Empresa.Ruc = @RucEmp
