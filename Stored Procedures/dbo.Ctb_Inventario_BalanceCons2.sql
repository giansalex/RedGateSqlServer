...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Ctb_Inventario_BalanceCons2]
@RucE nvarchar(11),
@Eje nvarchar(4),
@PrdoIni nvarchar(2),
@PrdoFin nvarchar(2),
...
begin
	if(@Nivel1=1)Begin Set @a = @a + 1 Set @tn1=',NULL as Total1' end else Set @tn1=''
	if(@Nivel2=1)Begin Set @a = @a + 1 Set @tn2=',NULL as Total2' end else Set @tn2=''
	if(@Nivel3=1)Begin Set @a = @a + 1 Set @tn3=',NULL as Total3' end else Set @tn3=''
	if(@Nivel4=1)Begin Set @a = @a + 1 Set @tn4=',NULL as Total4' end else Set @tn4=''

	Set @vouT = 'Convert(decimal(13,3),sum(a.MtoD'+@Ex+'-a.MtoH'+@Ex+')) as Saldo'
	Set @Saldo = 'NULL as Saldo'
	Set @Having= ' having sum(a.MtoD'+@Ex+'-a.MtoH'+@Ex+') <> 0'
end	
else	-- SOLES y DOLARES
begin
	if(@Nivel1=1)Begin Set @a = @a + 1 Set @tn1=',NULL as Total1_MN,NULL as Total1_ME' 
			 ...
			 Set @cn1_ME =',Case(('+@Suma_ME+')) when ''0'' then NULL else Convert(decimal(13,3),('+@Suma_ME+')) end As Total1_ME'
		     end else begin Set @tn1='' Set @cn1_ME ='' end
	if(@Nivel2=1)Begin Set @a = @a + 1 Set @tn2=',NULL as Total2_MN,NULL as Total2_ME' 
		     ...
		     Set @cn2_ME =',Case(('+@Suma_ME+')) when ''0'' then NULL else Convert(decimal(13,3),('+@Suma_ME+')) end As Total2_ME'
		     end else begin Set @tn2='' Set @cn2_ME = '' end
	if(@Nivel3=1)Begin Set @a = @a + 1 Set @tn3=',NULL as Total3_MN,NULL as Total3_ME' 
		     ...
		     Set @cn3_ME =',Case(('+@Suma_ME+')) when ''0'' then NULL else Convert(decimal(13,3),('+@Suma_ME+')) end As Total3_ME'
		     end else begin Set @tn3='' Set @cn3_ME ='' end
	if(@Nivel4=1)Begin Set @a = @a + 1 Set @tn4=',NULL as Total4_MN,NULL as Total4_ME' 
		     ...
		     Set @cn4_ME =',Case(('+@Suma_ME+')) when ''0'' then NULL else Convert(decimal(13,3),('+@Suma_ME+')) end As Total4_ME'
		     end else begin Set @tn4='' Set @cn4_ME ='' end

	Set @vouT_SD = '
		       Convert(decimal(13,3),sum(a.MtoD-a.MtoH)) as Saldo_MN,
		       Convert(decimal(13,3),sum(a.MtoD_ME-a.MtoH_ME)) as Saldo_ME
		       '
	Set @Saldo = 'NULL as Saldo_MN,NULL as Saldo_ME'
...
begin
	Set @SQL1 = 
		'
		(Select 1 as Num,a.RucE,a.Ejer,a.NroCtaN1 as NroCta ,b.NomCta,'''' as Cd_Aux,'''' As Cd_TDI,'''' As NomTDI,'''' As NDocAux ,'''' as NomAux,'''' as Cd_TD,'''' as NomTD,'''' as NroSre,'''' as NroDoc,'+@Saldo+'
			'+@tn4+@tn3+@tn2+',
		        ...
		        Case(('+@Suma+')) when ''0'' then NULL else Convert(decimal(13,3),('+@Suma+')) end as Total1'+@Mn+'
			'+@cn1_ME+'
...
begin
	...
	Set @SQL2 = @SQL2 +
		   '
		   (Select 2 as Num,a.RucE,a.Ejer,a.NroCtaN2 as NroCta,b.NomCta,'''' as Cd_Aux,'''' As Cd_TDI,'''' As NomTDI,'''' As NDocAux,'''' as NomAux,'''' as Cd_TD,'''' as NomTD,'''' as NroSre,'''' as NroDoc,'+@Saldo+'
			 '+@tn4+@tn3+',
		  	 ...
		  	 Case(('+@Suma+')) when ''0'' then NULL else Convert(decimal(13,3),('+@Suma+')) end as Total2'+@Mn+'
			 '+@cn2_ME+' 
			 '+@tn1+'
...
begin
	...
	Set @SQL3 = @SQL3 +
		   '
		   (Select 3 as Num,a.RucE,a.Ejer,a.NroCtaN3 as NroCta,b.NomCta,'''' as Cd_Aux,'''' As Cd_TDI,'''' As NomTDI,'''' As NDocAux,'''' as NomAux,'''' as Cd_TD,'''' as NomTD,'''' as NroSre,'''' as NroDoc,'+@Saldo+'
			 '+@tn4+',
			 ...
			 Case(('+@Suma+')) when ''0'' then NULL else Convert(decimal(13,3),('+@Suma+')) end as Total3'+@Mn+'
			 '+@cn3_ME+'
			 '+@tn2+@tn1+'
...
begin
	...
	Set @SQL4 = @SQL4 + 
		    '
		    (select 4 as Num,a.RucE,a.Ejer,a.NroCtaN4 as NroCta,b.NomCta,'''' as Cd_Aux,'''' As Cd_TDI,'''' As NomTDI,'''' As NDocAux,'''' as NomAux,'''' as Cd_TD,'''' as NomTD,'''' as NroSre,'''' as NroDoc,'+@Saldo+'
			 ...
			 ,Case(('+@Suma+')) when ''0'' then NULL else Convert(decimal(13,3),('+@Suma+')) end as Total4'+@Mn+'
			 '+@cn4_ME+'
			 '+@tn3+@tn2+@tn1+'
	...
	Set @SQL5_1 =
		    ...
		    (Select 5 as Num,
			 ...
			 Case When isnull(len(a.Cd_Clt),0)>0 and isnull(len(a.Cd_Prv),0)>0 Then i2.NCorto+''-CLTyPRV''
			 							Else case(isnull(len(a.Cd_Clt),0)) when 0 then i1.NCorto+''-PRV'' else i2.NCorto+''-CLI'' end
			 End As NomTDI,
			 --case(isnull(len(a.Cd_Clt),0)) when 0 then i1.NCorto+''-PRV'' else i2.NCorto+''-CLI'' end As NomTDI,
	...
	Set @SQL5_2 =
			...
			left join TipDoc t On t.Cd_TD=a.Cd_TD
			left join TipDocIdn i1 On i1.Cd_TDI=p.Cd_TDI and i1.Cd_TDI=p.Cd_TDI
			left join TipDocIdn i2 On i2.Cd_TDI=c.Cd_TDI and i2.Cd_TDI=c.Cd_TDI
			...
			isnull(a.Cd_TD,''''),t.NCorto,isnull(a.NroSre,''''),p.NDoc,c.NDoc,p.Cd_Prv,c.Cd_Clt,
			isnull(a.NroDoc,'''') '+@Having+')
		    '
		   -- Select * From TipDocIdn NCorto
/*Set @SQL5_1 = '
		    UNION ALL
		    (Select 5 as Num,
			a.RucE,
...
-- DI: 23/09/2011 <Se agrego TipoAux a la columna NomTDocInd.>
