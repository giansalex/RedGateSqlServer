...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Rpt_CtasXCbr_Detallada_CtbVN1]
@RucE nvarchar(11),
@Ejer varchar(4),
@Cd_Mda char(2),
@NroCta1 nvarchar(10),
...
set @Cons_2='
			Where 
				  ...
				  and Convert(date,v.FecCbr)  between convert(date,'''+Convert(varchar(30),@FecD)+''') and  convert(date,'''+Convert(varchar(30),@FecH)+''')
...
) As Con Where Vencido <> .0 and Hasta60 =.0 and Hasta90 = .0 and AMAS = .0 and Hasta30 = .0 Group By 	CliProv,RucC,RSocial
'
...
group by CliProv,RucC,RSocial,RegCtb
Having SUM(Saldo_Dias) <= 0 and SUM(Saldo)<> .0
Order by RucC Desc
'
print @Cons_1
print @Cons_2
