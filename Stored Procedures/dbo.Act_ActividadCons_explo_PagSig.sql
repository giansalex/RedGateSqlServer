...
CREATE PROCEDURE [dbo].[Act_ActividadCons_explo_PagSig]
--@RucE NVARCHAR(11),
@FecD DATETIME,
@FecH DATETIME,
@Colum VARCHAR(100),
...
PRINT @Consulta
	...
	ELSE
	  BEGIN	  
		...
		IF(@Ult_CdAct IS NULL) -- si es primera pagina y primera busqueda
		  BEGIN		  
		  ...
		  SET @sql = 'select @RMax = max(Cd_Act) from(select top '+ CONVERT(NVARCHAR,@TamPag)+' Cd_Act  from ' + @Inter + ' where ' + @Cond +' order by Cd_Act) as  Actividad'		  		  
		  EXEC sp_executesql @sql, N'@RMax char(10) output',@Max OUTPUT
		  
		  SET @sql = 'select top 1 @RMin = Cd_Act from ' + @Inter + ' where ' +@Cond+' order by Cd_Act'		  
