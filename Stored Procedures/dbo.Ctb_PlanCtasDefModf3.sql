...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Ctb_PlanCtasDefModf3]
@RucE nvarchar(11),
@Ejer varchar(4),
@IGV nvarchar(10),
@ISC nvarchar(10),
...
if not exists (select * from PlanCtasDef where RucE=@RucE and Ejer=@Ejer)
	insert into PlanCtasDef (RucE,Ejer,IGV,ISC,QCtg,RCons,Perc,Det,Ret,LCm,DC_MN,DC_ME,DP_MN,DP_ME,DCPer,DCGan,IN_DigCls,CtaClt,CtaPrv)
	values (@RucE,@Ejer,@IGV,@ISC,@QCtg,@RCons,@Perc,@Det,@Ret,@LCm,@DC_MN,@DC_ME,@DP_MN,@DP_ME,@DCPer,@DCGan,@IN_DigCls,@CtaClt,@CtaPrv/*,@REjer*/)
...
begin
	update PlanCtasDef set IGV=@IGV, ISC=@ISC, QCtg=@QCtg, RCons=@RCons, Perc=@Perc, Det=@Det,
			       ...
			       CtaClt=@CtaClt, CtaPrv=@CtaPrv--, REjer=@REjer
...
--MP: 04/06/2012 : <Creacion del procedimiento almacenado>
GO
