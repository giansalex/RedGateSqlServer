...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Com_SolicitudComCons_explo_PagSig1]
@RucE nvarchar(11),
@FecD datetime,
@FecH datetime,
@Colum varchar(100),
...
--print 'Cadena = '+@Cond
	...
	Set @CONSULTA='	select top '+Convert(nvarchar,@TamPag)+' sc.Cd_SCo,sc.NroSC,
			...
			sc.FecMdf,sc.UsuCrea,sc.UsuMdf,sc.Id_EstSC,sc.Cd_CC,sc.Cd_SC,sc.Cd_SS, isnull(sc.tipAut, 0) as TipAut, cfg.DescripTip, isnull(sc.IB_Aut, Convert(bit,0)) as IB_Aut 		
			from '+@Inter+'
...
-- Leyenda --
-- J : 2010-05-13 13:18:50.823	: <Creacion del procedimiento almacenado>
/*-----------------Ejemplo------------------
Declare @NroRegs int,@NroPags int,@Max char(10),@Min char(10)
exec dbo.Com_SolicitudComCons_explo_PagSig '11111111111','10/01/2010','20/08/2010','Cd_SCo','',20,'',@NroRegs out,@NroPags out,@Max out,@Min out,null
print @NroRegs
print @NroPags
print @Max
print @Min
