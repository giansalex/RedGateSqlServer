...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Vta_ServMdf2]
--//SERVICIO
@RucE nvarchar(11),
@Cd_Srv nvarchar(7),
@Cd_GS nvarchar(6),
...
if not exists (select * from Producto where RucE=@RucE and Cd_Pro=@Cd_Srv) -- and isnull(CodCo,'')=isnull(NULL,''))
	set @msj = 'No existe el Producto'
else
begin
begin transaction
	update Producto set CodCo=@CodCo,Nombre=@Nombre, Descrip=@Descrip,Cta1=@Cta1, Cta2=@Cta2, 
	...
	where RucE=@RucE and Cd_Pro=@Cd_Srv -- and isnull(CodCo,'')=isnull(NULL,'')
...
print @msj
-- Leyenda --
-- PP : 2010-05-13 11:12:16.640	: <Modificacion del procedimiento almacenado linea 34 Y 44  COMENTE -- and isnull(CodCo,'')=isnull(NULL,''))>
