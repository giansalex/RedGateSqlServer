...
SET ANSI_NULLS ON
GO
CREATE proc [user321].[Com_OrdCompra_ConsUnparaInv]
@RucE nvarchar(11),
@Cd_OC char(10),
...
begin
	select	oc.Cd_OC, oc.NroOC,pr.Cd_TDI as Cd_TDI_Prv,pr.NDoc, oc.Cd_Prv,case(isnull(len(pr.RSocial),0)) when 0 then pr.ApPat +' '+ pr.ApMat +','+ pr.Nom 
		...
		oc.CA01,oc.CA02,oc.CA03,oc.CA04,oc.CA05,oc.CA06,oc.CA07,oc.CA08,oc.CA09,oc.CA10,us1.NomComp as NAut1,us2.NomComp as NAut2,us3.NomComp as NAut3, 
		CantDet.Item as CantItem, oc.TipAut, oc.AutdoPorN1 
	from OrdCompra oc
