...
SET ANSI_NULLS ON
GO
CREATE procedure [user321].[Rpt_GuiaRemisionGeneral1]
@RucE nvarchar(11),
@Cd_GR char(10),
@NroSre varchar(5),
@NroGR varchar(15),
...
--print @CdGRAux
		...
		select 	GR.RucE, GR.Cd_GR, GR.FecEmi, GR.FecIniTras, GR.NroSre, GR.NroGR, GR.AutorizadoPor, GR.PtoPartida,GRL.PtoLlegada,
			...
			GR.Cd_TO,tp.Nombre as TipoOperacion,tr.Direc,GR.Cd_CC,GR.Cd_SC,GR.Cd_SS
		from 	(select GR.RucE, GR.Cd_GR, convert(nvarchar,GR.FecEmi,103) as FecEmi , convert(nvarchar,GR.FecIniTras,103) as FecIniTras,
	 		GR.NroSre,GR.NroGR, GR.AutorizadoPor, GR.PtoPartida,GR.Cd_Tra,GR.DescripTra,GR.Cd_TO,GR.Cd_CC,GR.Cd_SC,GR.Cd_SS from GuiaRemision GR  where Cd_GR=@CdGRAux and RucE=@RucE)as GR
			left join (select top 1 RucE,Cd_GR, Obs as Obs, Direc as PtoLlegada from GRPtoLlegada where Cd_GR=@CdGRAux and RucE=@RucE) as GRL
			...
			left join (select tr.RucE, tr.Cd_Tra, tr.NDoc as RucT,tr.LicCond,tr.NroPlaca,tr.McaVeh,
	 		case(isnull(len(tr.RSocial),0)) when 0 then tr.ApPat + ' ' + tr.ApMat + ', ' + tr.Nom else tr.RSocial end as Sres,tr.Direc
...
declare @nfactura varchar(15)
declare @fecfactura varchar(10)
		set @nfactura=(select top 1 v.NroSre +'-' +v.NroDoc as NFactura from guiaxventa a inner join venta v on a.RucE=v.RucE and a.Cd_Vta=v.Cd_Vta where a.ruce=@RucE and a.Cd_GR=@Cd_GR)
		set @fecfactura=(select top 1 Convert(nvarchar,FecMov,103) as FecMov from guiaxventa a inner join venta v on a.RucE=v.RucE and a.Cd_Vta=v.Cd_Vta where a.ruce=@RucE and a.Cd_GR=@Cd_GR)
if(isnull(@nfactura,'')='')
	begin
		set @nfactura=(select top 1 v.NroSre +'-' +v.NroDoc as NFactura from guiaxCompra a inner join Compra v on a.RucE=v.RucE and a.Cd_Com=v.Cd_Com where a.ruce=@RucE and a.Cd_GR=@Cd_GR)
		set @fecfactura=(select top 1 Convert(nvarchar,FecMov,103) as FecMov from guiaxventa a inner join venta v on a.RucE=v.RucE and a.Cd_Vta=v.Cd_Vta where a.ruce=@RucE and a.Cd_GR=@Cd_GR)
	end
select @nfactura as NFactura , @fecfactura as FFactura
--select *from GuiaxCompra where ruce='11111111111'
...
select 	GR.RucE, GR.Cd_GR, GR.FecEmi, GR.FecIniTras, GR.NroSre, GR.NroGR, GR.AutorizadoPor, GR.PtoPartida,GRL.PtoLlegada,
	tr.RucT,tr.LicCond,tr.NroPlaca,tr.McaVeh,case(isnull(len(GR.Cd_Tra),0)) when 0 then GR.DescripTra else tr.Sres end as Sres,
	GR.Cd_TO,tp.Nombre as TipoOperacion,GR.Cd_CC,GR.Cd_SC,GR.Cd_SS
from 	(select GR.RucE, GR.Cd_GR, convert(nvarchar,GR.FecEmi,103) as FecEmi , convert(nvarchar,GR.FecIniTras,103) as FecIniTras,
	 GR.NroSre,GR.NroGR, GR.AutorizadoPor, GR.PtoPartida,GR.Cd_Tra,GR.DescripTra,GR.Cd_TO,GR.Cd_CC,GR.Cd_SC,GR.Cd_SS from GuiaRemision GR  where Cd_GR=@Cd_GR and RucE=@RucE)as GR
	left join (select top 1 RucE,Cd_GR, Obs as Obs, Direc as PtoLlegada from GRPtoLlegada where Cd_GR=@Cd_GR and RucE=@RucE) as GRL
	 on GRL.RucE=GR.RucE and GRL.Cd_GR=GR.Cd_GR
	left join (select tr.RucE, tr.Cd_Tra, tr.NDoc as RucT,tr.LicCond,tr.NroPlaca,tr.McaVeh,
...
--print @Cd_CltAux
select case(isnull(len(cl.RSocial),0)) when 0 then cl.ApPat +' '+ cl.ApMat +','+ cl.Nom else cl.RSocial end as Destinario, cl.RucE RucD, tipd.NCorto, cl.NDoc
from cliente2 cl
...
--exec user321.Rpt_GuiaRemisionGeneral1 '11111111111','GR00000002',null,null,null
