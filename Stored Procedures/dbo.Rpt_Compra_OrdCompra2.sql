...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[Rpt_Compra_OrdCompra2]
@RucE nvarchar(11),
@Cd_OC nvarchar(4000),
...
declare @Cab varchar (8000) set @Cab =''
declare @Det varchar(8000) set @Det = ''
...
select 
	...
	case(IsNull(oc.DsctoFnzI,0)) when 0 then oc.DsctoFnzI else oc.DsctoFnzI end as DsctoFnz,
	...
	prv.CA10 as PrvCA10,
	0.0 as PercepSRV
	,oc.Asunto as AsuntoOC
	,isnull(usu.Correo1,'''') as CorreoUsu
	,prv.CtasCrtes as PrvCtasCrtes
	,oc.AutorizadoPor as NomUsuAut
	,usu2.NomComp as  NomCompAut
	,sco.UsuCrea as UsuCreaSCO
	,usu3.NomComp as NomCompSCO
	,srq.UsuCrea as UsuCreaSRQ
	,usu4.NomComp as NomCompSRQ
	,case when isnull(srq.UsuCrea,'''')<>'''' then srq.UsuCrea else case when isnull(sco.UsuCrea,'''')<>'''' then sco.UsuCrea else oc.UsuCrea end end as UsuCreaReqPor
	,case when isnull(srq.UsuCrea,'''')<>'''' then usu4.NomComp  else case when isnull(sco.UsuCrea,'''')<>'''' then usu3.NomComp else usu.NomComp end end as NomCompReqPor
	,oc.FecE as FecEmision
	,oc.FecEntR as FecEntrega
	,oc.FecAPag as FecAPagar
	'
...
' from OrdCompra oc
	...
	left Join Usuario usu2 on usu2.NomUsu = oc.AutorizadoPor
	left join SolicitudCom sco on sco.RucE = oc.RucE and sco.Cd_Sco = oc.Cd_Sco
	left join Usuario usu3 on usu3.NomUsu = sco.UsuCrea
	Left join SolicitudReq srq on srq.RucE = oc.RucE and srq.Cd_SR = sco.Cd_SR
	left Join Usuario usu4 on usu4.NomUsu = srq.UsuCrea
where oc.RucE='''+@RucE+''' and oc.Cd_Oc in ('+@Cd_OC +')'
set @Det = 
'
select 
	...
	det.CA03 as DCA03,det.CA04 as DCA04,det.CA05 as DCA05,
			...
			case(IsNull(det.Cd_Prod,''0'')) when ''0'' then det.Cd_Srv else det.Cd_Prod end as Cd_Prod,
			case(IsNull(det.Cd_Prod,''0'')) when ''0'' then srv.Nombre else pr.Nombre1 end as NomProd,
			case(IsNull(det.Cd_Prod,''0'')) when ''0'' then srv.Descrip else pr.Descrip end as DescripProd,
			case(IsNull(det.Cd_Prod,''0'')) when ''0'' then isnull(srv.CodCo,'''') else isnull(pr.CodCo1_,'''') end as CodCom,
...
where det.RucE='''+@Ruce+''' and det.Cd_Oc in ('+@Cd_OC+')'
...
where cd.RucE ='''+@RucE+''' and cd.Codigo in ('+@Cd_OC+')'
...
/*Leyenda*/
--JA : <Creado> ---> 05-23-2011
--exec dbo.Rpt_Compra_OrdCompra2 '11111111111','''OC00000431''',null
-- Analy:16/07/2012 <se agrego la columna prv.CtasCrtes>
--JA: Modifique, Agrege contactos
