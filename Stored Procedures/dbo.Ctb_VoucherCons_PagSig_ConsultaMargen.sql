...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Ctb_VoucherCons_PagSig_ConsultaMargen]
@RucE nvarchar(11),
@Eje nvarchar(4),
@PrdoIni nvarchar(2),
@PrdoFin nvarchar(2),
...
SET CONCAT_NULL_YIELDS_NULL off --Para que se pueda concatenar NULLs --PV 
	...
	set @Cond = 'vou.RucE='''+@RucE+''' and vou.Ejer='''+@Eje+''' and vou.Prdo between '''+@PrdoIni+''' and '''+@PrdoFin+''' and Cd_Vou > '+Convert(nvarchar, isnull(@Ult_CdVou,'')) +' and vou.RegCtb in (select RegCtb from Venta v
		left join VentaDet vd on v.RucE = vd.RucE and v.Cd_Vta = vd.Cd_Vta
		where v.RucE='''+@RucE+'''  and v.FecMov between '''+Convert(nvarchar,@FecIni,103)+''' and '''+Convert(nvarchar,@FecFin,103)+''' and v.Cd_Vta > '''' and Cd_Cte_NO is null and vd.Cd_Prod = '''+@Cd_Prod+''' and v.IB_Anulado = 0)'
	...
	set @Consulta ='		select top '+Convert(nvarchar,@TamPag)+'
		...
		case(isnull(len(vou.Cd_Clt),0)) when 0 then tdip.NCorto else tdic.NCorto end as NCorto, --NCorto
...
set @NroRegs = 99999
exec dbo.Ctb_VoucherCons_PagSig_ConsultaMargen '11111111111','2012','01','01','','PD00001',@NroRegs,@min, @NroRegs out,@NroPags out,@max, @min,'01/01/2012','31/01/2012',null
