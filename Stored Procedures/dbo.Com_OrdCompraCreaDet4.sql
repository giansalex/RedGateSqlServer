...
SET ANSI_NULLS ON
GO
CREATE proc [dbo].[Com_OrdCompraCreaDet4]
@RucE nvarchar(11),
@Cd_OC char(10),
@Item int,
@Cd_Prod char(7),
...
begin 
	...
	insert into OrdCompraDet(RucE,Cd_OC,Item,Cd_Prod,Cd_Srv,Descrip,ID_UMP,PU,Cant,Valor,DsctoP,DsctoI,
				BIM,IGV,Total,PendRcb,Cd_Alm,Obs,FecMdf,UsuMdf,CA01,CA02,CA03,CA04,CA05,IB_AtSrv, Cd_SCo)
			values(@RucE,@Cd_OC,@Item,@Cd_Prod,@Cd_Srv,@Descrip,@ID_UMP,@PU,@Cant,@Valor,@DsctoP,@DsctoI,
				...
				@CA04,@CA05,0, @Cd_SCo)--case(isnull(@Cd_Prod,0)) when 0 then 0 else null end)
	if @@rowcount <= 0
