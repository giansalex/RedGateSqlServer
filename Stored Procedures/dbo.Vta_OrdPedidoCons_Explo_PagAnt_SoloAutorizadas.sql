...
CREATE procedure [dbo].[Vta_OrdPedidoCons_Explo_PagAnt_SoloAutorizadas]
@RucE nvarchar(11),
@FecD datetime,
@FecH datetime,
@Colum varchar(100),
...
--	set @Cond = 'op.RucE= '''+@RucE+''' and op.FecE between '''+Convert(nvarchar,@FecD,103)+''' and '''+Convert(nvarchar,@FecH,103)+''' and op.Cd_OP <'''+isnull(@Ult_CdOP,'')+''''
	...
	begin
		set @Cond = 'op.RucE= '''+@RucE+''' and op.Cd_OP <'''+isnull(@Ult_CdOP,'')+'''and ((isnull(op.TipAut,0) = 0) or (isnull(op.TipAut,0) != 0 and isnull(op.IB_Aut,0) = 1))'
	end
	else
	begin
		set @Cond = 'op.RucE= '''+@RucE+''' and op.FecE between '''+Convert(nvarchar,@FecD,103)+''' and '''+
		Convert(nvarchar,@FecH,103)+''' and op.Cd_OP <'''+isnull(@Ult_CdOP,'')+'''and ((isnull(op.TipAut,0) = 0) or (isnull(op.TipAut,0) != 0 and isnull(op.IB_Aut,0) = 1))'
	end
...
/****** Object:  StoredProcedure [dbo].[Vta_OrdPedidoCons_Explo_PagAnt_Aut2]    Script Date: 02/07/2013 17:06:54 ******/
SET ANSI_NULLS ON
