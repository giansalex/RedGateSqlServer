...
SET ANSI_NULLS ON
GO
CREATE PROC [dbo].[Ltr_Letra_PagoCrea]
...
DECLARE @Cd_Ltr int
Set @Cd_Ltr = ( Select isnull(max(Cd_Ltr),0)+1 From Letra_Pago Where RucE=@RucE)

If exists (Select * From Letra_Pago Where RucE=@RucE and Cd_Cnj=@Cd_Cnj and Cd_Ltr=@Cd_Ltr)
...
Begin
	...
	Insert into Letra_Pago(RucE,Cd_Cnj,Cd_Ltr,Cd_TD,NroRenv,NroLtr,RefGdor,LugGdor,FecGiro,FecVenc,Plazo,Imp,Dsct,Total,FecReg,
	...
	Begin
		...
		insert into Letra_PagoRM( RucE,Cd_Cnj,Cd_Ltr,Cd_TD,NroRenv,NroLtr,RefGdor,LugGdor,
