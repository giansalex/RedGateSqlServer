...
SET ANSI_NULLS ON
GO
CREATE procedure [pvo].[Ctb_VoucherCrea6]
@RucE		nvarchar(11),
--@Cd_Vou	int,
@Ejer		nvarchar(4),
@Prdo		nvarchar(2),
...
*/
	...
	if @TipMov = 'M'
	begin
		exec pvo.Ctb_VoucherInsert3 @RucE, @Ejer, @Prdo, @RegCtb, @Cd_Fte, @FecMov, @FecCbr, @NroCta, @Cd_Aux, @Cd_TD, @NroSre,
...
--		select * from Tasas
		...
		if(@Cd_Fte='RC')
			if @IC_TipAfec != 'F' and @IC_TipAfec != 'N'
		...
		begin
			...
			exec pvo.Ctb_VoucherInsert3 @RucE, @Ejer, @Prdo, @RegCtb, @Cd_Fte, @FecMov, @FecCbr, @CtaAsoc, @Cd_Aux, @Cd_TD, @NroSre,
			...
			begin
				set @MtoIGV = @MtoH - @Mto
				exec pvo.Ctb_VoucherInsert3 @RucE, @Ejer, @Prdo, @RegCtb, @Cd_Fte, @FecMov, @FecCbr, @CtaIGV, @Cd_Aux, @Cd_TD, @NroSre,
							   @NroDoc, @FecED, @FecVD, @Glosa, @MtoOr, @MtoIGV, 0.00, @Cd_MdOr, @Cd_MdRg, @CamMda, @Cd_CC,
							   @Cd_SC, @Cd_SS,  @Cd_Area, @Cd_MR, @NroChke, @Cd_TG, @IC_CtrMd, @UsuCrea, 
							   /*@IC_TipAfec*/null, @TipOper, @Grdo, @msj output
			end
...
--				else set @Mto=@MtoH
				...
				begin
					print 'Insertamos amarre ' + @CtaD
					exec pvo.Ctb_VoucherInsert3 @RucE, @Ejer, @Prdo, @RegCtb, @Cd_Fte, @FecMov, @FecCbr, @CtaD, @Cd_Aux, @Cd_TD, @NroSre, @NroDoc, @FecED, @FecVD, @Glosa, @MtoOr, 
				...
				begin 	print 'Modificamos amarre ' + @CtaD
				...
				begin
					print 'Insertamos amarre ' + @CtaH
					exec pvo.Ctb_VoucherInsert3 @RucE, @Ejer, @Prdo, @RegCtb, @Cd_Fte, @FecMov, @FecCbr, @CtaH, @Cd_Aux, @Cd_TD, @NroSre, @NroDoc, @FecED, @FecVD, @Glosa, @MtoOr, 
				...
				begin 	print 'Modificamos amarre ' + @CtaH
...
*/
	...
	if(select IB_DifC from PlanCtas where RucE=@RucE and NroCta=@NroCta) = 1 --> Solo se aplica Dif de Camb a las ctas marcadas
	    if (select count(RegCtb) from Voucher where RucE=@RucE and RegCtb=@RegCtb)>1 and @IB_PgTot=1 --> Y si es un pago Total
		...
		begin 
			...
			set @IC_CtrMd = '$'
			set @Cd_MdRg = '02'
			exec pvo.Ctb_VoucherInsert3 @RucE, @Ejer, @Prdo, @RegCtb, @Cd_Fte, @FecMov, @FecCbr, @Cta_DCPer, null/*@Cd_Aux*/, null/*@Cd_TD*/, null/*@NroSre*/,
			...
			set @NroCta = @Cta_DCPer
		     ...
		     begin
			exec pvo.Ctb_VoucherInsert3 @RucE, @Ejer, @Prdo, @RegCtb, @Cd_Fte, @FecMov, @FecCbr, @Cta_DCGan, null/*@Cd_Aux*/, null/*@Cd_TD*/, null/*@NroSre*/,
...
/*	
	...
	--				else set @Mto=@MtoH
					...
					--AGREGAMOS CUENTA DESTINO DEBE
					if not exists (select NroCta from Voucher where RucE=@RucE and Prdo=@Prdo and RegCtb=@RegCtb and NroCta=@CtaD )
					begin
						exec pvo.Ctb_VoucherInsert3 @RucE, @Ejer, @Prdo, @RegCtb, @Cd_Fte, @FecMov, @FecCbr, @CtaD, @Cd_Aux, @Cd_TD, @NroSre, @NroDoc, @FecED, @FecVD, @Glosa, @MtoOr, 
									   @MtoDest, 0.00, @Cd_MdOr, @Cd_MdRg, @CamMda, @Cd_CC, @Cd_SC, @Cd_SS,  @Cd_Area, @Cd_MR, @NroChke, @Cd_TG, @IC_CtrMd, @UsuCrea, 
									   @IC_TipAfec, @TipOper, @Grdo, @msj output
					...
					begin exec pvo.Ctb_VoucherMdf_Mto @RucE,@Ejer,@Prdo,@RegCtb,@MtoDest,0.00,@CtaD,@Cd_MdRg,@CamMda,@IC_CtrMd,@Msj output end

	
	
	
					--AGREGAMOS CUENTA DESTINO HABER
					if not exists (select NroCta from Voucher where RucE=@RucE and Prdo=@Prdo and RegCtb=@RegCtb and NroCta=@CtaH )
					begin
						exec pvo.Ctb_VoucherInsert3 @RucE, @Ejer, @Prdo, @RegCtb, @Cd_Fte, @FecMov, @FecCbr, @CtaH, @Cd_Aux, @Cd_TD, @NroSre, @NroDoc, @FecED, @FecVD, @Glosa, @MtoOr, 
									   0.00, @MtoDest, @Cd_MdOr, @Cd_MdRg, @CamMda, @Cd_CC, @Cd_SC, @Cd_SS,  @Cd_Area, @Cd_MR, @NroChke, @Cd_TG, @IC_CtrMd, @UsuCrea, 
									   @IC_TipAfec, @TipOper, @Grdo, @msj output
					...
					begin exec pvo.Ctb_VoucherMdf_Mto @RucE,@Ejer,@Prdo,@RegCtb,0.00,@MtoDest,@CtaH,@Cd_MdRg,@CamMda,@IC_CtrMd,@Msj output end
