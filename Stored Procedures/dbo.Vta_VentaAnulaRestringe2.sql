...
SET ANSI_NULLS ON
GO
create procedure [dbo].[Vta_VentaAnulaRestringe2] 
@RucE nvarchar(11),
@Eje nvarchar(4), -- DATO AGREGADO
@Cd_Vta nvarchar(10),
@UsuModf nvarchar(10),
@MtvoBaja varchar(200),
...
as
	...
	if exists (SELECT * FROM Venta WHERE DR_CdVta = @Cd_Vta AND IB_Anulado = 0 AND Cd_TD = '07')
	BEGIN
		SET @msj = 'No se puede anular una venta referenciada a una Nota de Crédito'
		RETURN
	END

	if exists (SELECT * FROM Venta WHERE DR_CdVta = @Cd_Vta AND IB_Anulado = 0 AND Cd_TD = '08')
	BEGIN
		SET @msj = 'No se puede anular una venta referenciada a una Nota de Débito'
		RETURN
	END

	if exists(select * from Venta ven left join
				CanjeDet det on (ven.RucE=det.RucE and ven.Cd_Vta=det.Cd_Vta) 
				where ven.RucE=@RucE and ven.Eje=@Eje and det.Cd_Vta=@Cd_Vta)
	begin 	
		set @msj = 'Usuario no puede anular ventas del tipo Canje de Letras'
		return
	end
	...
	begin 	
		set @msj = 'Usuario no puede anular movimientos registrados por otro usuario'
		return
	end
	...
	update Venta set FecMdf=getdate(), UsuModf=@UsuModf, IB_Anulado=1, MtvoBaja = @MtvoBaja
...
--GS : Lun 18/04/2017  -> Se agrego validacion NCR,NDB y motivo de baja
GO
