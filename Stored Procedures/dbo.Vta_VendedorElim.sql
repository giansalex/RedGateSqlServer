...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Vta_VendedorElim]
@RucE nvarchar(11),
@Cd_Aux nvarchar(7),
...
if not exists (select * from Auxiliar where RucE=@RucE and Cd_Aux=@Cd_Aux)
	...
	set @msj = 'Vendedor ' + @Cd_Aux + ' no puede ser eliminado por tener data relacionada'
else
begin	
begin transaction
	...
	delete from Vendedor where RucE=@RucE and Cd_Aux=@Cd_Aux
	...
	insert into AuxiliarRM(RucE,NroReg,Cd_Aux,Cd_TDI,NroDoc,Cd_TA,Cd_MR,Usu,FecMov,Cd_Est)
	        values(@RucE,@NroReg,@Cd_Aux,@Cd_TDI,@NDoc,'03','01',@Usu,getdate(),'03')
	-----------------------------------------------------------------------------------
