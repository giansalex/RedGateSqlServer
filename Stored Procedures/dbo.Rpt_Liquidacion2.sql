...
CREATE Procedure [dbo].[Rpt_Liquidacion2]
@RucE nvarchar(11),
@Ejer nvarchar(4),
@FecIni nvarchar(12),
@FecFin nvarchar(12),
...
select NomComp, '' As IF01,'' As IF02,'' As IF03,'' As IF04,'' As IF05, (select Descrip from CCSub where RucE = @RucE and Cd_SC = @cd_SC) as IF06, '' as IF07 from Usuario where NomUsu = @usu
...
From 
	...
	(	Select v.RucE,
		...
		Where  v.RucE=@RucE and v.Ejer=@Ejer and isnull(v.IB_Anulado,0)<>1 
			and isnull(v.IB_Anulado,0)=0
		Group by v.RucE, v.Ejer,	v.NroCta, isnull(v.Cd_Clt,isnull(v.Cd_Prv,'')), isnull(v.Cd_TD,''), isnull(v.NroSre,''), isnull(v.NroDoc,'')
		Having Sum(v.MtoD)-Sum(v.MtoH)=0 and Sum(v.MtoD)+Sum(v.MtoH)<>0 and Max(Case When isnull(v.IB_EsProv,0)<>1 Then Month(v.FecMov) Else 0 End) < Month(@FecIni)--between Month(@FecIni) and Month(@FecFin)
	) As g On g.RucE=v.RucE and g.Ejer=v.Ejer and (g.NroCta=v.NroCta and isnull(g.CodAux,'')=isnull(v.Cd_Clt,isnull(v.Cd_Prv,'')) and g.Cd_TD=v.Cd_TD and g.NroSre=v.NroSre and g.NroDoc=v.NroDoc)
	Left Join
	(	
		Select v.* From Voucher v Where v.RucE=@RucE and v.Ejer=@Ejer and isnull(v.IC_TipAfec,'')<>''
...
--03/07/2011
--Prueba
--Exec Rpt_Liquidacion2 '20111412341','2011','01/06/2011','30/06/2011','A4','jacho'
GO
