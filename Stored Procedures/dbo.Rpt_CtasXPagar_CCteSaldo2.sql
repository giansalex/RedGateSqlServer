...
declare @FechaAl datetime
set @FechaAl = '30/06/2009'
exec dbo.Rpt_CtasXPagar_CCteSaldo2 '11111111111','2009','', @FechaAl,'01',null
*/
CREATE procedure [dbo].[Rpt_CtasXPagar_CCteSaldo2]
@RucE nvarchar(11),
@Ejer nvarchar(4),
@Cd_Aux nvarchar(7),
@NroCta1 nvarchar(10),
...
select 	v.NroCta, a.NDoc as Cd_Aux, v.Cd_TD, v.NroSre, v.NroDoc, convert(varchar,v.FecMov,103) as FecMov, v.FecVD, datediff(day,v.FecMov,@FechaAl) as Saldo_Dias, sum(v.MtoD) Debe, sum(v.MtoH) Haber,
	...
	where v.RucE=@RucE and v.Ejer=@Ejer and (v.NroCta between @NroCta1 and @NroCta2) and (left(v.NroCta,2)=42 or left(v.NroCta,2)=46) and v.Cd_Aux= @Cd_Aux
...
--	e.Rsocial
	...
	left join Auxiliar as a on a.RucE=v.RucE and a.Cd_Aux=v.Cd_Aux
	--left join Empresa as e on e.Ruc=v.RucE
	where v.RucE=@RucE and v.Ejer=@Ejer and (v.NroCta between @NroCta1 and @NroCta2)/*and a.cd_aux=@Cd_Aux*/ and (left(v.NroCta,2)=42 or left(v.NroCta,2)=46)
