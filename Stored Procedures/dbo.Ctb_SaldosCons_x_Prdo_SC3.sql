...
SET ANSI_NULLS ON
GO
CREATE proc [dbo].[Ctb_SaldosCons_x_Prdo_SC3]
@RucE nvarchar(11),
@Ejer  nvarchar(4),
@Prdo1  nvarchar(2),
@Prdo2  nvarchar(2),
...
Set @SQL1 = 
	'
	Declare @CtaHml int
	Set @CtaHml = '+Convert(varchar,@CtaHml)+'
	Declare @CtaHomo nvarchar(50),@NomHomo nvarchar(50)
	Declare @CtaHomoG nvarchar(50),@NomHomoG nvarchar(50)
	Declare @CtaHomoT nvarchar(50),@NomHomoT nvarchar(50)
	...
	If(@CtaHml = 1) 
	Begin 
		Set @CtaHomo = '',isnull(p.NroCtaH1,'''' '''') As NroCtaH''
		Set @NomHomo = '',isnull(p.NomCtaH1,'''' '''') As NomCtaH''
	...
	Else If(@CtaHml = 2) 
	Begin 
		Set @CtaHomo = '',isnull(p.NroCtaH2,'''' '''') As NroCtaH''
		Set @NomHomo = '',isnull(p.NomCtaH2,'''' '''') As NomCtaH''
	...
	Declare _cursor Cursor 
		For Select Cd_SC,NCorto From CCSub Where RucE=@RucE and Cd_CC=@Cd_CC and Cd_SC in ('+@Datos+')
...
Set @SQL2 =
	'
	Open _cursor
		Fetch Next From _cursor Into @Cd_SC,@NCorto
		While @@Fetch_Status = 0
			Begin
				Set @Cadena = @Cadena+''	
			,Sum(Case(v.Cd_SC)when ''''''+@Cd_SC+'''''' then v.MtoD''+@TMda+''-v.MtoH''+@TMda+'' else 0.00 end) as ''''''+@NCorto+''''''''
				Print @Cadena
				Fetch Next From _cursor Into @Cd_SC,@NCorto
			End
	Close _cursor
...
Set @SQL3 =
	...
	if(@N1 = ''1'')
	begin
		Set @Part1_N1 = '' Select 1 as ind, left(v.NroCta,2) as NroCta,p.NomCta''+@CtaHomo+@NomHomo+''''
		Set @Part2_N1 = '' ,''''0'''' as Total
				     From Voucher v 
				left join PlanCtas p On p.RucE=v.RucE and p.NroCta=left(v.NroCta,2) and v.Ejer=p.Ejer
				where v.RucE=''''''+@RucE+'''''' and v.Ejer=''''''+@Ejer+'''''' and v.IB_Anulado=0 and v.Prdo between ''''''+@Prdo1+'''''' and ''''''+@Prdo2+'''''' and v.Cd_CC=''''''+@Cd_CC+'''''''+@RangoN1+'
			      	Group by left(v.NroCta,2),p.NomCta''+@CtaHomoG+@NomHomoG+''
			      	''
	end
	'
Set @SQL4 =
	...
	begin
		...
		Set @Part1_N2 = @Part1_N2+'' Select 2 as ind,left(v.NroCta,4) as NroCTa,p.NomCta''+@CtaHomo+@NomHomo+''''
		Set @Part2_N2 = '' ,''''0'''' as Total
				     From Voucher v 
			left join PlanCtas p On p.RucE=v.RucE and p.NroCta=left(v.NroCta,4) and v.Ejer=p.Ejer
			where v.RucE=''''''+@RucE+'''''' and v.Ejer=''''''+@Ejer+'''''' and v.IB_Anulado=0 and v.Prdo between ''''''+@Prdo1+'''''' and ''''''+@Prdo2+'''''' and v.Cd_CC=''''''+@Cd_CC+'''''''+@RangoN2+'
			Group by left(v.NroCta,4),p.NomCta''+@CtaHomoG+@NomHomoG+''
			''
	end
	...
	begin
		...
		Set @Part1_N3 = @Part1_N3+'' Select 3 as ind,left(v.NroCta,6) as NroCTa,p.NomCta''+@CtaHomo+@NomHomo+''''
		Set @Part2_N3 = '' ,''''0'''' as Total
			                   From Voucher v 
			left join PlanCtas p On p.RucE=v.RucE and p.NroCta=left(v.NroCta,6) and v.Ejer=p.Ejer
			where v.RucE=''''''+@RucE+'''''' and v.Ejer=''''''+@Ejer+'''''' and v.IB_Anulado=0 and v.Prdo between ''''''+@Prdo1+'''''' and ''''''+@Prdo2+'''''' and v.Cd_CC=''''''+@Cd_CC+'''''''+@RangoN3+'
			Group by left(v.NroCta,6),p.NomCta''+@CtaHomoG+@NomHomoG+''
			''
	end
	'
Set @SQL5 = 
	...
	begin
		...
		Set @Part1_N4 = @Part1_N4 +'' Select 4 as ind,v.NroCta,p.NomCta''+@CtaHomo+@NomHomo+''''
		Set @Part2_N4 = '' ,''''0'''' as Total
				     From Voucher v 
			left join PlanCtas p On p.RucE=v.RucE and p.NroCta=v.NroCta and v.Ejer=p.Ejer
			where v.RucE=''''''+@RucE+'''''' and v.Ejer=''''''+@Ejer+'''''' and v.IB_Anulado=0 and v.Prdo between ''''''+@Prdo1+'''''' and ''''''+@Prdo2+'''''' and v.Cd_CC=''''''+@Cd_CC+'''''''+@RangoN4+'
			Group by v.NroCta,p.NomCta''+@CtaHomoG+@NomHomoG+''
			''
	end
...
Set @SQL6 = 
	'
	Set @Tot_P1 = '' Select *--,'''' '''' as NumCtaH,'''' '''' as NomCtaH--Ind,NroCta,NomCta''+@CtaHomoT+@NomHomoT+'',Total 
					 from (Select 0 as ind,''''TOTAL'''' as NroCta,''''********************'''' as NomCta''
	Set @Tot_P2 = '' ,''''0'''' as Total
			   From Voucher v 
			where v.RucE=''''''+@RucE+'''''' and v.Ejer=''''''+@Ejer+'''''' and v.IB_Anulado=0 and v.Prdo between ''''''+@Prdo1+'''''' and ''''''+@Prdo2+'''''' and v.Cd_CC=''''''+@Cd_CC+'''''''+@RangoN4+'
			) As Tab
	--Group by Ind,NroCta,NomCta,Total			
			''
...
----------------------PRUEBA------------------------
--exec [Ctb_SaldosCons_x_Prdo_SC3] '11111111111','2012','00','03','','''''',1,0,0,1,'01','', '','1',null

------CODIGO DE MODIFICACION--------
--CM=RE01

----------------------LEYENDA----------------------
-- DI : 11/11/2009 Creacion del procedimiento almacenado
