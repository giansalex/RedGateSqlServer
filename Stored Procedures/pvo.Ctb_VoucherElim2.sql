...
SET ANSI_NULLS ON
GO
CREATE procedure [pvo].[Ctb_VoucherElim2]
@RucE nvarchar(11),
@Ejer nvarchar(4),
@RegCtb nvarchar(15),
@UsuMdf nvarchar(10),
...
--Select * From Empresa Where RSocial like '%PURE%'
...
begin
	...
	if(@RucE in ('20527146130','20527146136')) --> PARA PURE PERU
	begin
		if not exists (Select * From Voucher Where RucE=@RucE and Ejer=@Ejer and RegCtb=@RegCtb and UsuCrea=@UsuMdf)
		begin
			set @msj = 'Usted no tiene autorizacion para eliminar el voucher creado por otro usuario'
			return
		end
	end
...
--DI: MIE 16/01/2013 Mdf : Se Agrego condicion para que un usuario solo pueda eliminar los registros que solo ah realizado 
