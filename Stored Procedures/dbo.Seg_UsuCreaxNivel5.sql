...
SET ANSI_NULLS ON
GO
create procedure [dbo].[Seg_UsuCreaxNivel5]
@NomUsu	nvarchar(10),
...
begin
    insert into Usuario(NomUsu,Pass,NomComp,Cd_Trab,Cd_Prf,Nivel,Estado,IB_TipCamCrear,IB_TipCamMdf,IB_TipCamElim,Correo1,UsuCrea,FecReg,Cargo, Numero)
	        values (@NomUsu,@Pass,@NomComp,@Cd_Trab,@Cd_Prf,
			dbo.Asigna_NivelxUsuario(@NomUsuCrea),1,@IB_TipCamCrear,@IB_TipCamMdf,@IB_TipCamElim,@correo1,@UsuCrea, @FecReg,@Cargo,@Numero)
    if @@rowcount<=0
