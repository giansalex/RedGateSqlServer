...
SET ANSI_NULLS ON
GO
 CREATE Procedure [dbo].[Rpt_ListaDeOC_USD_SOL3]
...
SELECT  Empresa.Ruc, Empresa.RSocial,   'Desde el ' + convert(varchar,@fecIni,103)+ ' hasta el ' + convert(varchar,@fecFin,103)   as 'Fecha Consulta' ,   case @Impt when '0' then 'RELACIÓN DE ORDENES DE COMPRA LOCALES' when '1' then 'RELACIÓN DE ORDENES DE IMPORTACIÓN' end   as IC_Imp  FROM Empresa WHERE Empresa.Ruc = @RucEmp
...
-- Exec  Rpt_ListaDeOC_USD_SOL2  '20102028687', '1/02/2000' , '28/02/2013',     ''        ,'DESC'               ,''               ,''               ,''			,1        ,'', 'A'
