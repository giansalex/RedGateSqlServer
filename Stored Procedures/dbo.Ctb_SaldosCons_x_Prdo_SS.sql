...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Ctb_SaldosCons_x_Prdo_SS]
@RucE nvarchar(11),
@Eje nvarchar(4),
@PrdoIni nvarchar(2),
@PrdoFin nvarchar(2),
...
Set @RangoC = ''
if(isnull(len(@Cd_CCD),0) = 0 and isnull(len(@Cd_SCD),0) = 0 and isnull(len(@Cd_SSD),0) = 0 and isnull(len(@Cd_CCH),0) = 0 and isnull(len(@Cd_SCH),0) = 0 and isnull(len(@Cd_SSH),0) = 0)
Begin
	set @RangoC = ''
End
else	Set @RangoC = ' and a.Cd_CC between '''+@Cd_CCD+''' and '''+@Cd_CCH+''''+
		      ' and a.Cd_SC between '''+@Cd_SCD+''' and '''+@Cd_SCH+''''+
                      ' and a.Cd_SS between '''+@Cd_SSD+''' and '''+@Cd_SSH+''''
