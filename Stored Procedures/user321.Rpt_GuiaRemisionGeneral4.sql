...
SET ANSI_NULLS ON
GO
CREATE procedure [user321].[Rpt_GuiaRemisionGeneral4]
@RucE nvarchar(11),
@Cd_GR varchar(4000),
@NroSre varchar(5),
@NroGR varchar(15),
...
from      VGuiaRemision as gr Left join VGuiaRemisionLLegada as GRL on GRL.RucE=GR.RucE and GRL.Cd_GR=GR.Cd_GR
          ...
          left join Venta on Venta.RucE = GuiaXVenta.RucE and Venta.Cd_Vta = GuiaXVenta.Cd_Vta
Where GR.Cd_GR in ('+@Cd_GR+') and GR.RucE = '''+@RucE+''' and GR.Cd_Td = '''+@Cd_TD+''''
...
select    GRD.Item, GRD.Cd_Prod as Codigo,GRD.PesoCantKg as Kilos, GRD.Cant as Cantidad, GRD.ID_UMP as UMedida
		...
		,GRD.CA01 as CA01Det,GRD.CA02 as CA02Det,GRD.CA03 as CA03Det,GRD.CA04 as CA04Det,GRD.CA05 as CA05Det
...
set @Cd_CltAux= (select Cd_clt from guiaremision where Ruce='''+@RucE+''' and Cd_GR in ('+@Cd_GR+'))
...
exec Rpt_GuiaRemisionGeneral4 '20536756541','''GR00000002''','001','0000557','09',null
