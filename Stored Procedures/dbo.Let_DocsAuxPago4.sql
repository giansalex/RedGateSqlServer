...
SET ANSI_NULLS ON
GO
CREATE PROC [dbo].[Let_DocsAuxPago4]
...
Begin
	...
	FROM
		(	
			Select * From Doc_Compra Where RucE=@RucE and Cd_Prv=@Cd_Prv
			UNION ALL
			Select * From Doc_CompraCbr Where RucE=@RucE and Cd_Prv=@Cd_Prv
			UNION ALL
			Select * From Doc_CompraLtr Where RucE=@RucE and Cd_Prv=@Cd_Prv
			UNION ALL
			Select * From Doc_VouCom Where RucE=@RucE and Cd_Prv=@Cd_Prv
		) TB
	...
	GROUP BY 
		TB.DR_CdTD,TB.DR_NSre,TB.DR_NDoc,TB.Cd_Com,TB.Cd_Vou,TB.Cd_Ltr,TB.Cd_TD,TB.NomTD,TB.NroSre,TB.NroDoc,TB.Cd_Mda,TB.NomMda,TB.CamMda
	HAVING 
		Sum(TB.SaldoS)+Sum(TB.SaldoD)<>0
	ORDER BY 
