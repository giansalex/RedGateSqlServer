...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[Rpt_ComprasAProveedor2] 
--Declare
								@RucEmp nvarchar(11)
								,@fecIni datetime
								,@fecFin datetime
...
SELECT  Empresa.Ruc, Empresa.RSocial,  'Desde el ' + convert(varchar,@fecIni,103)+ ' hasta el ' + convert(varchar,@fecFin,103)   as 'Fecha Consulta', 'REPORTE DE COMPRAS A PROVEEDORES '+ case @Impt when '0' then 'LOCALES' when '1' then  'DE IMPORTACIÓN' end  +' EN DOLARES Y SOLES'   as IC_Imp  FROM Empresa WHERE Empresa.Ruc = @RucEmp
...
select
	...
	,isnull(compra.CA01,'''') as  CONDICION_PAGO
	
	--, FormaPC.Nombre as CONDICION_PAGO
	,  sum(case compra.Prdo when ''01'' then case when isnull(compra.IB_anulado,0) = 1 then 0.0 else isnull(cd.Total,0) end  else 0 end) as ENERO
	, sum(case compra.Prdo when ''02'' then case when isnull(compra.IB_anulado,0) = 1 then 0.0 else isnull(cd.Total,0) end   else 0 end) as FEBRERO
	, sum(case compra.Prdo when ''03'' then case when isnull(compra.IB_anulado,0) = 1 then 0.0 else isnull(cd.Total,0) end   else 0 end) as MARZO
	, sum(case compra.Prdo when ''04'' then case when isnull(compra.IB_anulado,0) = 1 then 0.0 else isnull(cd.Total,0) end   else 0 end) as ABRIL
	, sum(case compra.Prdo when ''05'' then case when isnull(compra.IB_anulado,0) = 1 then 0.0 else isnull(cd.Total,0) end   else 0 end) as MAYO
	, sum(case compra.Prdo when ''06'' then case when isnull(compra.IB_anulado,0) = 1 then 0.0 else isnull(cd.Total,0) end   else 0 end) as JUNIO
	, sum(case compra.Prdo when ''07'' then case when isnull(compra.IB_anulado,0) = 1 then 0.0 else isnull(cd.Total,0) end   else 0 end) as JULIO
	, sum(case compra.Prdo when ''08'' then case when isnull(compra.IB_anulado,0) = 1 then 0.0 else isnull(cd.Total,0) end   else 0 end) as AGOSTO
	, sum(case compra.Prdo when ''09'' then case when isnull(compra.IB_anulado,0) = 1 then 0.0 else isnull(cd.Total,0) end   else 0 end) as SEPTIEMBRE
	, sum(case compra.Prdo when ''10'' then case when isnull(compra.IB_anulado,0) = 1 then 0.0 else isnull(cd.Total,0) end   else 0 end) as OCTUBRE
	, sum(case compra.Prdo when ''11'' then case when isnull(compra.IB_anulado,0) = 1 then 0.0 else isnull(cd.Total,0) end   else 0 end) as NOVIEMBRE
	, sum(case compra.Prdo when ''12'' then case when isnull(compra.IB_anulado,0) = 1 then 0.0 else isnull(cd.Total,0) end   else 0 end) as DICIEMBRE		
