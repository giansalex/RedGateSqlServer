...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Tsr_VoucherCons_exploxfecha_PagAnt]
@RucE nvarchar(11),
@Ejer nvarchar(4),
@FechaIni datetime,
@FechaFin datetime,
...
as
	...
	--Condicion	
	set @Cond = 'vou.RucE='''+@RucE+''' and vou.Ejer='''+@Ejer+''' and  vou.FecMov between '''+ Convert(nvarchar,@FechaIni,103)+''' and ''' +Convert(nvarchar,@FechaFin,103) +''' and vou.Cd_Fte=''CB'' and vou.Cd_Vou <'''+ Convert(nvarchar, isnull(@Ult_CdTsr,'')) +''''
	...
	set @Consulta = '	select *from(select top '+ convert(nvarchar,@TamPag) +'
			...
			vou.Cd_MdRg,morg.Simbolo as SimMdRg, vou.CamMda, 
			vou.IC_TipAfec,case vou.IB_EsProv when 1 then 1 else 0 end as IB_EsProv, vou.Ejer,vou.Prdo,convert(varchar(10),vou.FecMov,103) as FecMov, 
			vou.Glosa,convert(varchar(10),vou.FecCbr,103) as FecCbr,c2.Cd_TDI,tdic.NCorto,c2.NDoc,vou.Cd_Clt as Cd_Aux, 
			...
			case vou.IB_Conc when 1 then 1 else 0 end as IB_Conc, convert(varchar(10),vou.FecED,103) as FecED,convert(varchar(10),vou.FecVD,103) as FecVD, 
			...
			vou.UsuCrea, vou.UsuModf, convert(varchar,vou.FecReg,108) as HoraReg, case vou.IB_Anulado when 1 then 1 else 0 end as IB_Anulado
			from '+@inter+' 
			where '+@Cond+ ' order by vou.Cd_Vou desc, vou.FecMov desc)  as Voucher order by Cd_Vou' 
	...
	if NOT  exists (select top 1 * from Voucher where RucE = @RucE and Ejer=@Ejer)
	  ...
	  begin
		Exec (@Consulta)
		  ...
		  set @sql = 'select top 1 @RMax = Cd_Vou from ' + @Inter + ' where ' +@Cond+' order by Cd_Vou desc, FecMov desc'
		  exec sp_executesql @sql, N'@RMax int output', @Max output
		  if(@Max is null) set @Max = 0

		  set @sql = 'select @RMin = min(Cd_Vou) from(select top '+ Convert(nvarchar,@TamPag)+' Cd_Vou  from ' + @Inter + ' where ' + @Cond +' order by Cd_Vou desc, FecMov desc) as  Voucher'
		  exec sp_executesql @sql, N'@RMin int output',@Min output
		  if(@Min is null) set @Min = 0

	  end
-- Leyenda --
-- MP : 04/07/2011 : <Creacion del procedimiento almacenado>
