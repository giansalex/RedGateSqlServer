...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Rpt_RegistroVentas_X_Sr]
@RucE nvarchar(11),
@Eje nvarchar(4),
@Rprdo1 nvarchar(2),
@Rprdo2 nvarchar(2),
...
select
	...
	/*v.BIM, v.EXO, v.INF, v.IGV, v.Total,s.NroSerie*/v.NroSre as Grupo,@Rprdo1+' - '+@Rprdo2+' del '+@Eje as Prdo, --v.CamMda
  ...
  --   Case(IB_Anulado)  when 0 then ( case(v.Cd_Mda) when '02' then v.Total*v.CamMda else v.Total end) else '0.00' end as Total, 
	...
	-- Cliente c, Auxiliar ca --<<-- Modificado linea 56
	Cliente2 c2 --<<-- Nueva
	...
	v.RucE=c2.RucE and --<<-- Nueva
      	-- v.Cd_Cte=c.Cd_Aux and c.RucE=ca.RucE and c.Cd_Aux=ca.Cd_Aux --and v.IB_Anulado<>1 --<<-- Modificado linea61
...
--sp_help Rpt_RegistroVentas_X_Sr
--exec Rpt_RegistroVentas_X_Sr '11111111111','2010','01','09','01',''
