...
SET ANSI_NULLS ON
GO
CREATE procedure [pvo].[Ctb_VoucherInsert4Imdo3] --(Se usa solo para importar voucher)
@RucE		nvarchar(11),
--@Cd_Vou	int,
@Ejer		nvarchar(4),
@Prdo		nvarchar(2),
...
@NroCta	nvarchar(15), -- anteriormente estaba con longitud de 10 y lo modifique ha 15  GG     
--@Cd_Aux	nvarchar(7),
----------------------
@Cd_TD	nvarchar(2),
@NroSre	nvarchar(4),
...
if not exists ( select NroCta from PlanCtas where RucE=@RucE and Ejer=@Ejer and NroCta=@NroCta ) and (ISNULL(@NroCta,'')<>'')
begin
set @msj = 'Numero de Cuenta Contable no existe: '+@NroCta 
return	
end

if not exists ( select * from Cliente2 where RucE=@RucE and  Cd_TDI=@Cd_TDI  and NDoc=@RucC) and (ISNULL(@RucC,'')<>'') and  (ISNULL(@Cd_TDI,'')<>'') 
begin
set @msj = 'No existe cliente con numero N° Doc. : '+@RucC 
return	
end

if not exists ( select * from Proveedor2 where RucE=@RucE and Cd_TDI=@Cd_TDI and NDoc=@RucP) and (ISNULL(@RucP,'')<>'') and  (ISNULL(@Cd_TDI,'')<>'') 
begin
set @msj = 'No existe Proveedor con numero N° Doc. : '+@RucP 
return	
end
...
--GG: Mar 29/11/2016 - se creo este nuesvo SP Ctb_VoucherInsert4Imdo3 para usarlo solo en importacion de voucher y no alterar el anterior Ctb_VoucherInsert4Imdo2 que puede esta siendo utilizado en otras funcionalidades
