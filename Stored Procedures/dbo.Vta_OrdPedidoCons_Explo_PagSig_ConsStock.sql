...
SET ANSI_NULLS ON
GO
CREATE proc [dbo].[Vta_OrdPedidoCons_Explo_PagSig_ConsStock]
...
as
	...
	if(@TipStock = 0)--Stock Pedido
		set @estadoOC = ' and oc.Id_EstOP != ''06'''
	else if(@TipStock = 1)--Stock Recibido
		set @estadoOC = ' and (oc.Id_EstOP = ''02'' or oc.Id_EstOP = ''03'')'
	else if(@TipStock = 2)--Stock Pendiente
		set @estadoOC = ' and (oc.Id_EstOP = ''01'' or oc.Id_EstOP = ''02'')'
	------------------------------------------------------------------------------
	declare @Inter varchar(4000)
	set @Inter = 'OrdPedido op inner join Vendedor2 ve on ve.Cd_Vdr=op.Cd_Vdr and ve.RucE=op.RucE
		join Cliente2 cl on cl.Cd_Clt=op.Cd_Clt and cl.RucE=op.RucE
		join Moneda mo on mo.Cd_Mda=op.Cd_Mda
		...
		join (
			select Cd_OP as ''CD_OPD'' from ordpedidodet
			where RucE = '''+@RucE+''' and Cd_Prod = '''+@Cd_Prod+''' and Cd_Alm = '''+@Cd_Alm+'''
			group by Cd_OP
		)as opd on op.Cd_OP = opd.Cd_OPD and op.RucE = '''+@RucE+''''+@estadoOC
