...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Prd_FormulaCons_explo_PagAnt2]
@RucE nvarchar(11),
@TamPag int,
@Ult_NroFor char(10),
@Max char(10) output,
...
as
	...
	set @Consulta = 
	'select *from (select top '+Convert(nvarchar,@TamPag)+ ' p.Cd_Prod, p.CodCo1_, p.Nombre1, p.Descrip, u.ID_UMP, u.DescripAlt, count(*) as CantForm from Formula as f 
	...
	where '+@Cond+'
	group by p.Cd_Prod, p.Nombre1, p.Descrip, u.ID_UMP, u.DescripAlt, p.CodCo1_
