...
SET ANSI_NULLS ON
GO
CREATE PROC [dbo].[Pre_CCPresupuestadosCrea_Import]
...
AS
	...
	if not exists(select * from PlanCtas where RucE=@RucE and Ejer=@Ejer and NroCta=@NroCta) and (ISNULL(@Ejer,'')<>'') and (ISNULL(@NroCta,'')<>'')
	begin
		set @msj='No existe cuenta contable '+@NroCta
		return
	end
