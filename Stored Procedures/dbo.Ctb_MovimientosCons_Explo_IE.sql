...
CREATE procedure [dbo].[Ctb_MovimientosCons_Explo_IE]
...
Declare @Tipo char(1) Set @Tipo=''
if(@Most = 0) --<-- Es Funcion
	Set @Tipo = 'F'
Else
	Set @Tipo = 'N'
...
Set @SQL1 = 
	...
	from Voucher v
	Inner join PlanCtas p On p.RucE=v.RucE and p.Ejer=v.Ejer and p.NroCta=v.NroCta and isnull(p.IC_IE'+@Tipo+','''')='''+@TipoRpt+'''
...
-- DI->(30/11/2011) : Creacion del procedimiento almacenado
