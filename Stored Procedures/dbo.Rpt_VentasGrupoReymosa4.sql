...
CREATE procedure [dbo].[Rpt_VentasGrupoReymosa4]
@RucE nvarchar(11),
@Ejer nvarchar(4),
@Cd_Vdr nvarchar(10),
@Cd_Clt nvarchar(10),
...
--Rpt_VentasGrupoReymosa4 '20102028687','2012'   ,''    ,''  ,'01/12/2012','31/12/2012'     ,0       ,'02'        ,'P'        ,'5'       ,'desc'       ,''       , '' 
--Rpt_VentasGrupoReymosa4 RUC__EMPRESA, _EJER, CD_VDR, CD_CLT,   FEC_INI,    FEC_FIN,   IB_INCLUIR,  CD_MDA,   IC_InPrdSrv,  ColOrder,   TipOrder,   Cd_Clase, Cd_Prd
...
select e.*,'DEL '+Convert(nvarchar,@FecIni,103)+ ' AL '+Convert(nvarchar,@FecFin,103) as FecCons,@IC_InPrdSrv as IncPrdSrv from empresa e where e.Ruc=@RucE
...
,case when isnull(v.IB_Anulado,0)=1 then 0.0 else  case when isnull(vd.IGV,0) = 0 then case when '''+@Cd_Mda+''' = v.Cd_Mda then isnull(vd.IMP,0.00) else case when '''+@Cd_Mda+''' = ''01'' then isnull(v.CamMda,0.00)*isnull(vd.IMP,0.00) else isnull(vd.IMP,0.00)/isnull(v.CamMda,0.00)end end else 0.00 end end  as INF_Det --inafecto que no tiene igv, precio sin igv, MONTO SIN IGV
, case when isnull(v.IB_Anulado,0)=1 then 0.0 else case when '''+@Cd_Mda+''' = v.Cd_Mda then isnull(vd.Valor,0.00) else case when '''+@Cd_Mda+''' = ''01'' then isnull(v.CamMda,0.00)*isnull(vd.Valor,0.00) else isnull(vd.Valor,0.00)/isnull(v.CamMda,0.00)end end end  as Valor_Det  --valor por unidad sin igv
...
,  case when isnull(v.IB_Anulado,0)=1 then 0.0 else  case when isnull(vd.IGV,0) <> 0 then case when '''+@Cd_Mda+''' = v.Cd_Mda then isnull(vd.IMP,0.00) else  case when '''+@Cd_Mda+''' = ''01'' then isnull(v.CamMda,0.00)*isnull(vd.IMP,0.00) else isnull(vd.IMP,0.00)/isnull(v.CamMda,0.00)end end else 0.00 end end   as BIM_Det --total sin igv
,(case when isnull(v.IB_Anulado,0)=1 then 0.0 else case when isnull(vd.IGV,0) <> 0 then case when '''+@Cd_Mda+''' = v.Cd_Mda then isnull(vd.IMP,0.00) else case when '''+@Cd_Mda+''' = ''01'' then isnull(v.CamMda,0.00)*isnull(vd.IMP,0.00) else isnull(vd.IMP,0.00)/isnull(v.CamMda,0.00)end end else 0.00 end end)/vd.Cant  as PrecioU -- precio unitario
...
,case when isnull(v.IB_Anulado,0) = 1 then 0.0 else vd.cant end as Cant
,v.Cd_Mda,v.CamMda
