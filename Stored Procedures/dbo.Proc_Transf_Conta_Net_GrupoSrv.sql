...
GO
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
CREATE procedure [dbo].[Proc_Transf_Conta_Net_GrupoSrv]
@RucE nvarchar(11),
...
set @consulta = 'insert into GrupoSrv 
	select * from OPENROWSET(''SQLOLEDB'','''+@IpServer+''';'''+@UsuConeccion+''';'''+@PasswordConeccion+''',
		 ''select '''''+@RucE+''''',Cd_GS,Descrip,NCorto,Estado from GrupoSrv where RucE='''''+@RucEBase+''''''')'
exec (@consulta)
