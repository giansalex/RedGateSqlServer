...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[Rpt_ComprasDeProdXMes2] 
...
SELECT  Empresa.Ruc, Empresa.RSocial,   'Desde el ' + convert(varchar,@fecIni,103)+ ' hasta el ' + convert(varchar,@fecFin,103)   as 'Fecha Consulta' , 'COMPRAS '+ case @Impt when '0' then 'LOCALES' when '1' then  'DE IMPORTACIÓN' end  + ' DE PRODUCTOS POR MES'  as IC_Imp FROM Empresa WHERE Empresa.Ruc = @RucEmp


Set @Sql = 

'
select      case '''+@IC_PrdSrv+''' when ''S'' then Compradet.cd_srv when ''P'' then Compradet.Cd_Prod else isnull(Compradet.cd_prod, compradet.cd_srv) end as CodPrdSrv,
			...
			, isnull(UnidadMedida.NCorto,'''') as UNID
			 ...
			 ,SUM(case when isnull(compra.IB_Anulado,0) = 1 then 0.0 else CompraDet.cant end) as TOTAL		 
							 ...
							 where   
								...
								and compra.cd_td <> ''07''								
							 group by Compradet.Cd_Prod, compradet.cd_srv, compra.RucE, UnidadMedida.NCorto, producto2.nombre1, servicio2.nombre
