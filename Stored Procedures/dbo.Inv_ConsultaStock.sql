...
CREATE procedure [dbo].[Inv_ConsultaStock]
@RucE nvarchar(11),
...
if(@RucE = '20546110720')-- NORSUR
	exec ('select P.Cd_Prod as Codigo, T3.ID_UMP,  P.CodCo1_ as ''Codigo Comer.'',P.Nombre1 + '' '' + U.DescripAlt as Producto,Cant as Cantidad'+@col2+' from ( 
		select Cd_Prod, ID_UMP, Sum(Cant) as Cant '+@Col1+' from ( 
			select *'+@col+' from (
				select Cd_Alm, Cd_Prod, ID_UMP, sum(Cant_Ing) as Cant from Inventario where RucE = '''+@RucE+''' group by Cd_Prod, Cd_Alm, ID_UMP)
			as T1 
	...
	EXEC ('select P.Cd_Prod as Codigo, P.CodCo1_ as ''Codigo Comer.'',P.Nombre1 as Producto,Cant as Cantidad'+@col2+' from ( 
		select Cd_Prod, Sum(Cant) as Cant '+@Col1+' from ( 
			select *'+@col+' from (
				select Cd_Alm, Cd_Prod, sum(Cant) as Cant from Inventario where RucE = '''+@RucE+''' group by Cd_Prod, Cd_Alm)
			as T1 
...
--select *, case Cd_Alm when 'A01' then Cant else 0 end  as xD from (select *  from Inventario where RucE = '11111111111'@RucE group by Cd_Prod, Cd_Alm)  as XD order by Cd_Prod, Cd_Alm
-- exec Inv_ConsultaStock '11111111111',''
-- select Cd_Alm, Cd_Prod, sum(Cant) as Cant from Inventario where RucE = '11111111111' group by Cd_Prod, Cd_Alm
GO
