...
SET ANSI_NULLS ON
GO
 CREATE PROCEDURE [dbo].[Rpt_CompraFacturaGuiaProducto3] 
...
declare @Sql nvarchar(4000), @Impt varchar(3), @IC_Mda nvarchar(2)
...
If @Cd_mda is null begin Set @Cd_mda = '' Set @IC_Mda = '' end --CODIGO DE MONEDA
    ...
    If @Cd_Mda = '03' begin set @IC_Mda = '' set @Cd_Mda = '' end
...
SELECT  Empresa.Ruc, Empresa.RSocial, @cd_Mda AS Moneda,   'Desde el ' + convert(varchar,@fecIni,103)+ ' hasta el ' + convert(varchar,@fecFin,103)   as 'Fecha Consulta',  case @Impt when '0' then 'REPORTE DE COMPRAS DE FACTURAS/GUIAS/PRODUCTOS LOCALES' when '1' then  'REPORTE DE IMPORTACIÓN DE FACTURAS/GUIAS/PRODUCTOS' end    as IC_Imp  FROM Empresa WHERE Empresa.Ruc = @RucEmp 
...
select	
		...
		,isnull(case '''+@Impt+''' when ''0'' then OrdCompra.CA01 when ''1'' then OrdCompra.CA03 end,'''') as FORM_PAGO 
		...
		,(case '''+@IC_Mda+''' when ''04'' then (case Compra.cd_mda when ''02'' then ''*S/.''  else moneda.Simbolo end) when ''05'' then (case compra.cd_mda when ''01'' then ''*US$'' else moneda.Simbolo end) else moneda.Simbolo end) as Moneda
			 ...
			 from Compra 
			 left join OrdCompra on ordcompra.cd_oc = compra.cd_oc and ordcompra.ruce = compra.ruce
