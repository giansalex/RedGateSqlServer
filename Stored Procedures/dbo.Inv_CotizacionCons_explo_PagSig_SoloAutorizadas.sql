...
CREATE PROCEDURE [dbo].[Inv_CotizacionCons_explo_PagSig_SoloAutorizadas]
...
begin 
  set @Cond= '(c.RucE='''+@RucE+''') and c.Cd_Cot > ''' + Convert(nvarchar,isnull(@Ult_CdCot,'')) +'''and ((isnull(c.TipAut,0) = 0) or (isnull(c.TipAut,0) != 0 and isnull(c.IB_Aut,0) = 1))'  		
end 
else 
begin 
set @Cond = '(c.RucE='''+@RucE+''')  and (c.FecEmi between '''+Convert(nvarchar,@FecD,103)+''' and '''+
		case when @Dato IS null then Convert(nvarchar,@FecH,103)+''') and c.Cd_Cot > ''' + Convert(nvarchar,isnull(@Ult_CdCot,'')) +'''and ((isnull(c.TipAut,0) = 0) or (isnull(c.TipAut,0) != 0 and isnull(c.IB_Aut,0) = 1))'
		else
		Convert(nvarchar,@FecH,103)+''') and c.Cd_Cot < ''' + Convert(nvarchar,isnull(@Ult_CdCot,'')) +'''and ((isnull(c.TipAut,0) = 0) or (isnull(c.TipAut,0) != 0 and isnull(c.IB_Aut,0) = 1))'
		end
	end		
