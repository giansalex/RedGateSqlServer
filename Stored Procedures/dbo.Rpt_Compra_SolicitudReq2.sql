...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[Rpt_Compra_SolicitudReq2]
---------------------------------------------------------------------------------
...
sr.Obs,us.NomComp as ElaboradoPor,sr.AutorizadoPor,
...
sr.CA01,sr.CA02,sr.CA03,sr.CA04,sr.CA05, us.NomComp as NomUsu
,us2.NomComp as NomCompAut
...
left join Usuario us on us.NomUsu = sr.UsuCrea 
left join Usuario us2 on us2.NomUsu = sr.AutorizadoPor
Where sr.RucE='''+@RucE+''' and sr.Cd_SR in ('+@Cd_SR+')'
...
set @SQL2 = 
'
select d.RucE,d.Cd_SR,d.Item,
			case(IsNull(d.Cd_Prod,''0'')) when ''0'' then d.Cd_Srv else d.Cd_Prod end as Cd_Prod,
			case(IsNull(d.Cd_Prod,''0'')) when ''0'' then srv.Nombre else pr.Nombre1 end as Nombre1,
			case(IsNull(d.Cd_Prod,''0'')) when ''0'' then srv.Descrip else pr.Descrip end as DescripProd,
			case(IsNull(d.Cd_Prod,''0'')) when ''0'' then srv.CodCo else pr.CodCo1_ end as CodCom,
...
left join Servicio2 srv on srv.RucE = d.RucE and srv.Cd_Srv = d.Cd_SRV 
...
Where d.RucE='''+@RucE+''' and d.Cd_SR in ('+@Cd_SR+')'
...
/*Leyenda*/
--J : <Creado> ---> 06-12-2010
--exec dbo.Rpt_Compra_SolicitudReq2 '20528206931','''SR00000015''',null 
