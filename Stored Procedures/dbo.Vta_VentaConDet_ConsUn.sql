...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Vta_VentaConDet_ConsUn]
@RucE nvarchar(11),
--@Ejer nvarchar(4),
@Cd_Vta nvarchar(10),
...
begin
		...
		select 
			...
			vta.UsuModf,vta.IB_Anulado, vta.MtvoBaja,vta.IB_Cbdo,vta.DR_CdVta,
	...
	------------------------------------------------
		...
		select 
			...
			/*********************************************************************************************/
			ump.Cd_UM,um.Nombre as NombreUM,um.NCorto as NCortoUM,um.CodSNT_,
			/*************DETALLE********************************************************************************/
...
--Leyenda
--GGONZ -> 16/03/2017 consulta para factura electronica.
--select * from Venta where RucE = '11111111111'
--exec Vta_VentaConsUnX '11111111111','VT00000120',''
-- GS -> 19/04/2017 -> Add MtvBaja field to consult.
GO
