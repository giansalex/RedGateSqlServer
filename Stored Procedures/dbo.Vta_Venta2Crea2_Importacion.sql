...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Vta_Venta2Crea2_Importacion]
@RucE nvarchar(11),
...
/**************************** Strat Validaciones ************************************/
	...
	if not exists(select * from TipDocIdn where Cd_TDI=@Cd_TDICLI ) and (isnull(@Cd_TDICLI, '') <> '') 
	begin
		Set @msj = 'No existe Tipo Documento de Identificacion : '+@Cd_TDICLI 
		return 
	end


	if not exists(select * from Cliente2 where Cd_TDI=@Cd_TDICLI  and NDoc=@NDocCLI) and (isnull(@Cd_TDICLI, '') <> '') and (isnull(@NDocCLI, '') <> '') 
	begin
		Set @msj = 'No existe Cliente con de Identificacion : '+@NDocCLI 
		return 
	end
	...
	if not exists(select * from TipDocIdn where Cd_TDI=@Cd_TDIVDR ) and (isnull(@Cd_TDIVDR, '') <> '') 
	begin
		Set @msj = 'No existe Tipo Documento de Identificacion : '+@Cd_TDIVDR
		return 
	end
