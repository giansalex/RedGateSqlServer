...
SET ANSI_NULLS ON
GO
--exec Rpt_Detraccion_Explo '11111111111','2012','01','02',null,null,null,null,0,0,0

CREATE procedure [dbo].[Rpt_Detraccion_Explo]
@RucE nvarchar(11),
@Ejer varchar(4),
@PrdoIni varchar(2),
@PrdoFin varchar(2),
...
--exec Rpt_Detraccion_Explo '11111111111','2012','01','03',null,null,null,null,0,0,0,null
set @Cons_Niv1_1='
declare @Today datetime
set @Today=getdate()

select 
	niv,Canc,RucE,Ejer,Cd_Prv,NDoc,Proveedor, FecMov, FecED, '''' FecVD
	, case when Canc=1 then ''Pagado'' else case when Datediff(Day,@Today,FecVD) <0 then ''Vencido'' when Datediff(Day,@Today,FecVD) >=0 then ''Pendiente'' End End as EstDtr
	, RegCtb, Glosa, Cd_TD, NroSre, NroDoc, MtoD,MtoH,SaldoMN, MtoD_ME
	, MtoH_ME, SaldoME, Cd_Area, Cd_CC, Cd_SC,Cd_SS IB_EsProv, IB_Anulado
	, UsuCrea , FecReg
	from (
select 
	1 as niv,
	...
	,'''' FecMov, '''' FecED, Max(v.FecVD) FecVD,'''' as RegCtb, '''' Glosa, v.Cd_TD, Isnull(v.NroSre,'''') NroSre ,Isnull(v.NroDoc,'''') NroDoc
...
select 
	...
	, case when ISNULL(p2.RSocial,'''')='''' then ISNULL(p2.ApPat,'''')+'' ''+ISNULL(p2.ApMat,'''')+'' ''+ISNULL(p2.Nom,'''') else ISNULL(p2.RSocial,'''') end as Proveedor
	,'''' FecMov, '''' FecED, Convert(varchar,Max(v.FecVD),103) FecVD,'''' as RegCtb, '''' Glosa, v.Cd_TD, Isnull(v.NroSre,'''') NroSre ,Isnull(v.NroDoc,'''') NroDoc
	,Sum(v.MtoD) MtoD, Sum(v.MtoH) MtoH,Sum(v.MtoD - v.MtoH) SaldoMN
	,Sum(v.MtoD_ME) MtoD_ME, Sum(v.MtoH_ME) MtoH_ME, Sum(v.MtoD_ME - v.MtoH_ME) SaldoME
	,'''' Cd_Area,'''' Cd_CC,'''' Cd_SC,'''' Cd_SS,Convert(bit,0) IB_EsProv, Convert(bit,0) IB_Anulado
	,'''' UsuCrea,'''' FecReg
...
'
--, case when Datediff(Day,@Today,FecVD) <0 then ''Vencido'' when Datediff(Day,@Today,FecVD) >=0 then ''Pendiente'' End as EstDtr
--exec Rpt_Detraccion_Explo '11111111111','2012','01','03',null,null,null,null,0,0,0,null
set @Cons_Niv2_1='
select niv, Canc, RucE, Ejer, Cd_Prv,NDoc,Proveedor,Convert(varchar,FecMov,103) FecMov, FecED, Convert(varchar,isnull(FecVD,FecMov),103) FecVD
, case when Canc=1 then ''Pagado'' else case when Datediff(Day,@Today,Isnull(FecVD,FecMov)) <0 then ''Vencido'' when Datediff(Day,@Today,Isnull(FecVD,FecMov)) >=0 then ''Pendiente'' End End as EstDtr
, RegCtb, Glosa, Cd_TD, NroSre, NroDoc, MtoD, MtoH, SaldoMN, MtoD_ME, MtoH_ME, SaldoME, Cd_Area, Cd_CC
, Cd_SC, Cd_SS, IB_esProv, IB_Anulado, UsuCrea, FecReg 
 from (
select distinct
2 as niv, CONVERT(bit,1) as Canc,v.RucE,v.Ejer,v.Cd_Prv,p2.NDoc
, case when ISNULL(p2.RSocial,'''')='''' then ISNULL(p2.ApPat,'''')+'' ''+ISNULL(p2.ApMat,'''')+'' ''+ISNULL(p2.Nom,'''') else ISNULL(p2.RSocial,'''') end as Proveedor
, v.FecMov, CONVERT(Varchar,v.FecED,103) FecED, v.FecVD
