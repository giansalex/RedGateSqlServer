...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Vta_VentaCons_explo_PagSig_ConsultaMargen]
@RucE nvarchar(11),
@FecD datetime,
@FecH datetime,
@Colum varchar(100),
...
set @FecH = @FecH  + '23:59:29'

	--Interseccion
		set @Inter = 'venta v inner join Moneda mo on mo.Cd_Mda=v.Cd_Mda
				left join VentaDet vd on v.RucE = vd.RucE and v.Cd_Vta = vd.Cd_Vta
	...
	begin
		set @Cond = '(v.RucE='''+@RucE+''')  and v.Cd_Vta > ''' + Convert(nvarchar,isnull(@Ult_CdVta,'')) +''' and Cd_Cte_NO is null and vd.Cd_Prod = '''+ @Cd_Prod +''' and v.IB_Anulado = 0'
	end
	else
	begin
		set @Cond = '(v.RucE='''+@RucE+''')  and (v.FecMov between '''+Convert(nvarchar,@FecD,103)+''' and '''+
		Convert(nvarchar,@FecH,103)+''') and v.Cd_Vta > ''' + Convert(nvarchar,isnull(@Ult_CdVta,'')) +''' and Cd_Cte_NO is null and vd.Cd_Prod = '''+ @Cd_Prod +''' and v.IB_Anulado = 0'
	end
...
set @Consulta='		select top '+convert(nvarchar,@TamPag)+'
				...
				case (IB_Anulado) when 1 then ''0.00'' else isnull(v.ValorNeto,''0.00'') end as ValorNeto,
				...
				case (IB_Anulado) when 1 then ''0.00'' else isnull(v.BaseSinDsctoF,''0.00'') end as BaseSinDsctoF,
				case (IB_Anulado) when 1 then ''0.00'' else isnull(v.DsctoFnz_P,''0.00'') end as DsctoFnz_P,
				case (IB_Anulado) when 1 then ''0.00'' else isnull(v.DsctoFnz_I,''0.00'') end as DsctoFnz_I,
				case (IB_Anulado) when 1 then ''0.00'' else isnull(v.INF_Neto,''0.00'') end as INF_Neto,
				case (IB_Anulado) when 1 then ''0.00'' else isnull(v.EXO_Neto,''0.00'') end as EXO_Neto,
				case (IB_Anulado) when 1 then ''0.00'' else isnull(v.EXPO_Neto,''0.00'') end as EXPO_Neto,				
				case (IB_Anulado) when 1 then ''0.00'' else isnull(v.BIM_Neto,''0.00'') end as BIM_Neto,				
				case(v.IGV) when 0 then 0 else 1 end as IncIGV,
				case (IB_Anulado) when 1 then ''0.00'' else isnull(v.IGV,''0.00'') end as IGV,
				case (IB_Anulado) when 1 then ''0.00'' else isnull(v.Total,''0.00'') end as Total,
				case (IB_Anulado) when 1 then ''0.00'' else isnull(v.Percep,''0.00'') end as Percep,
				...
				case (IB_Anulado) when 1 then ''0.00'' else isnull(v.Valor,''0.00'') end as Valor,
				case (IB_Anulado) when 1 then ''0.00'' else isnull(v.TotDsctoP,''0.00'') end as TotDsctoP,
				case (IB_Anulado) when 1 then ''0.00'' else isnull(v.TotDsctoI,''0.00'') end as TotDsctoI,
	...
	if not exists (select top 1 * from Venta where RucE=@RucE) --and Cd_Clt = @Cd_Clt)
	  ...
	  begin
		...
		if(@Ult_CdVta is null) -- si es primera pagina y primera busqueda
		  ...
		  set @sql = 'select @RMax = max(Cd_Vta) from(select top '+ Convert(nvarchar,@TamPag)+' v.Cd_Vta  from ' + @Inter + ' where ' + @Cond +' order by v.Cd_Vta) as  Venta'
		  exec sp_executesql @sql, N'@RMax nvarchar(10) output',@Max output

		  set @sql = 'select top 1 @RMin = v.Cd_Vta from ' + @Inter + ' where ' +@Cond+' order by v.Cd_Vta'
		  exec sp_executesql @sql, N'@RMin nvarchar(10) output', @Min output
	end
-- Leyenda --
-- CAM : 2012-01-28 	: <Creacion del procedimiento almacenado>
/*
set language spanish
exec Vta_VentaCons_explo_PagSig_ConsultaMargen '11111111111','01/03/2012','31/03/2012',null,null,500,null,null,null,null,null,'PD00042',null

exec Vta_VentaCons_explo_PagSig_ConsultaMargen '11111111111','01/03/2012','31/03/2012',null,null,500,null,null,null,null,null,'',null

--select *from venta where ruce='11111111111' and eje='2012'
*/
GO
