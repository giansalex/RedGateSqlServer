...
SET ANSI_NULLS ON
GO
CREATE procedure [pvo].[Rpt_CtasXCbr_EstCta4]
@RucE nvarchar(11),
@Ejer nvarchar(4),
@Cd_Aux nvarchar(7),
@NroCta1 nvarchar(10),
...
begin
		select 	a.NDoc as NDocAux, case(isnull(len(a.RSocial),0)) when 0 then a.ApPat+' '+a.ApMat+' '+a.Nom else a.RSocial end as NomAux, max(RegCtb) as RegCtb, v.NroCta,
		...
		group by a.NDoc, a.ApPat,a.ApMat,a.Nom,a.RSocial, /*RegCtb,*/ v.NroCta, /*v.Cd_Aux,*/ v.Cd_TD, v.NroSre, v.NroDoc, convert(varchar,v.FecMov,103), /*v.FecVD,*/ datediff(day,v.FecMov,@FechaAl),v.IB_Anulado --,v.Cd_MdRg
		having sum(v.MtoD)-sum(v.MtoH)<>0 and v.IB_Anulado<>'1'
		order by a.NDoc,NomAux,v.Cd_TD, v.NroSre, v.NroDoc,FecED

end
else -- No tiene aux
begin
		select 	a.NDoc as NDocAux, case(isnull(len(a.RSocial),0)) when 0 then a.ApPat+' '+a.ApMat+' '+a.Nom else a.RSocial end as NomAux, max(RegCtb) as RegCtb, /*RegCtb,*/ v.NroCta,
		v.Cd_TD, v.NroSre,v.NroDoc, convert(varchar,v.FecMov,103) as FecED, '' as FecVD, /*v.FecVD,*/ datediff(day,v.FecMov,@FechaAl) as Saldo_Dias,  
		case(@Cd_Mda) when '01' then sum(v.MtoD) else sum(v.MtoD_ME) end as Debe, case(@Cd_Mda) when '01' then sum(v.MtoH) else sum(v.MtoH_ME) end as Haber,
		@Cd_Mda as Cd_MdRg --> DEBERIA JALAR LA MONEDA DEL VOUCHER
		--e.Rsocial
