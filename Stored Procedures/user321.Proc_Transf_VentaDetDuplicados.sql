...
SET ANSI_NULLS ON
GO
CREATE procedure [user321].[Proc_Transf_VentaDetDuplicados]
@RucE nvarchar(11),
@Cd_VtaAnt nvarchar(10),
@Cd_VtaAct nvarchar(10),
@Ejer varchar(4)
as
...
SELECT 
	RucE, '''+@Cd_VtaAct+''', Nro_RegVdt, Cant, Valor, DsctoP, DsctoI, 
...
from 
	OPENROWSET(''SQLOLEDB'', ''netserver'';''Usu123_1'';''user123'',
		...
		where 
			vd.RucE='''''+@RucE+''''' and ve.Eje='''''+@Ejer+''''' and vd.Cd_Vta='''''+@Cd_VtaAnt+'''''
		order by 
			vd.Cd_Vta'') '
Print @Consulta
Exec(@Consulta)
