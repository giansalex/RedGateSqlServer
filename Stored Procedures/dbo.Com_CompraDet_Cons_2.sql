...
SET ANSI_NULLS ON
GO
CREATE proc [dbo].[Com_CompraDet_Cons_2]
@RucE nvarchar(11),
@Cd_Com char(10),
@msj varchar(100) output
---with encryption
as
select  cd.RucE,cd.Item, cd.Cd_Com,
	...
	case when isnull(pr.CodCo1_,'')='' then '-' else pr.CodCo1_ end as CodCom,
	...
	cd.Total,cd.Cd_Alm,cd.Cd_IA,cd.Cd_CC,cd.Cd_SC,cd.Cd_SS,pu.DescripAlt as UM,
...
left join Producto2 pr on cd.RucE=pr.RucE and cd.Cd_Prod=pr.Cd_Prod where  cd.RucE =@RucE  and cd.Cd_Com=@Cd_Com
