...
SET ANSI_NULLS ON
GO
CREATE proc [dbo].[Ctb_SaldosCons_x_Prdo_SS3]
@RucE nvarchar(11),
@Ejer  nvarchar(4),
@Prdo1  nvarchar(2),
@Prdo2  nvarchar(2),
...
Set @SQL1 = 
	...
	Set @Mda='''+@MdaReg+''' Set @Desde='''+@Prdo1+''' Set @Hasta='''+@Prdo2+'''
...
Set @SQL2 =
	...
	Open _cursor
		...
		While @@Fetch_Status = 0
			Begin
				Set @Cadena = @Cadena+''	
			,Sum(Case(v.Cd_SS)when ''''''+@Cd_SS+'''''' then v.MtoD''+@TMda+''-v.MtoH''+@TMda+'' else 0.00 end) as ''''''+@NCorto+''''''''
				Print @Cadena
				Fetch Next From _cursor Into @Cd_SS,@NCorto
			End
	Close _cursor
...
Set @SQL3 =
	...
	begin
		...
		Set @Part2_N1 = '' ,'''''''' as Total
				     From Voucher v 
				left join PlanCtas p On p.RucE=v.RucE and p.NroCta=left(v.NroCta,2) and p.Ejer=v.Ejer
				where v.RucE=''''''+@RucE+'''''' and v.Ejer=''''''+@Ejer+'''''' and v.IB_Anulado=0 and v.Prdo between ''''''+@Prdo1+'''''' and ''''''+@Prdo2+'''''' and v.Cd_CC=''''''+@Cd_CC+'''''' and v.Cd_SC=''''''+@Cd_SC+'''''''+@RangoN1+'
...
Set @SQL4 =
	...
	begin
		...
		Set @Part2_N2 = '' ,'''''''' as Total
				    From Voucher v 
			left join PlanCtas p On p.RucE=v.RucE and p.NroCta=left(v.NroCta,4) and p.Ejer=v.Ejer
			where v.RucE=''''''+@RucE+'''''' and v.Ejer=''''''+@Ejer+'''''' and v.IB_Anulado=0 and v.Prdo between ''''''+@Prdo1+'''''' and ''''''+@Prdo2+'''''' and v.Cd_CC=''''''+@Cd_CC+'''''' and v.Cd_SC=''''''+@Cd_SC+'''''''+@RangoN2+'
	...
	begin
		...
		Set @Part2_N3 = '' ,'''''''' as Total
				     From Voucher v 
			left join PlanCtas p On p.RucE=v.RucE and p.NroCta=left(v.NroCta,6) and p.Ejer=v.Ejer
			where v.RucE=''''''+@RucE+'''''' and v.Ejer=''''''+@Ejer+'''''' and v.IB_Anulado=0 and v.Prdo between ''''''+@Prdo1+'''''' and ''''''+@Prdo2+'''''' and v.Cd_CC=''''''+@Cd_CC+'''''' and v.Cd_SC=''''''+@Cd_SC+'''''''+@RangoN3+'
...
Set @SQL5 = 
	...
	begin
		...
		Set @Part2_N4 = '' ,'''''''' as Total
			...
			where v.RucE=''''''+@RucE+'''''' and v.Ejer=''''''+@Ejer+'''''' and v.IB_Anulado=0 and v.Prdo between ''''''+@Prdo1+'''''' and ''''''+@Prdo2+'''''' and v.Cd_CC=''''''+@Cd_CC+'''''' and v.Cd_SC=''''''+@Cd_SC+'''''''+@RangoN4+'
...
Set @SQL6 = 
	'
	Set @Tot_P1 = '' Select Ind,NroCta,NomCta''+@CtaHomoT+@NomHomoT+'',Total from (Select 0 as ind,''''TOTAL'''' as NroCta,''''********************'''' as NomCta''
	Set @Tot_P2 = '' ,'''''''' as Total
			  From Voucher v 
			where v.RucE=''''''+@RucE+'''''' and v.Ejer=''''''+@Ejer+'''''' and v.IB_Anulado=0 and v.Prdo between ''''''+@Prdo1+'''''' and ''''''+@Prdo2+'''''' and v.Cd_CC=''''''+@Cd_CC+'''''' and v.Cd_SC=''''''+@Cd_SC+'''''''+@RangoN4+'
			) As Tab
