...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Vta_VentaCons_explo_PagAnt_ConsultaMargen]
@RucE nvarchar(11),
@FecD datetime,
@FecH datetime,
@Colum varchar(100),
	...
	if not exists (select top 1 * from Venta where RucE=@RucE)
	  ...
	  begin
		Exec (@Consulta)
		  ...
		  set @sql = 'select top 1 @RMax = v.Cd_Vta from ' + @Inter + ' where ' +@Cond+' order by v.Cd_Vta desc'
		  exec sp_executesql @sql, N'@RMax char(12) output', @Max output

		  set @sql = 'select @RMin = min(Cd_Vta) from(select top '+ Convert(nvarchar,@TamPag)+' v.Cd_Vta  from ' + @Inter + ' where ' + @Cond +' order by v.Cd_Vta desc) as  Venta'
...
-- exec Vta_VentaCons_explo_PagAnt_ConsultaMargen '11111111111','01/01/2012', '31/01/2012', '','', 4, 'VT00000825', null, null, PD00001,''
GO
