...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Rpt_Venta_ALZATEC]
@RucE nvarchar(11),
@Eje nvarchar(4),
@Cd_Vta nvarchar(10),
...
set @CampoInsp = (select Valor from CampoV where RucE=@RucE and Cd_Vta=@Cd_Vta and Cd_Cp='01')
	...
	/*================================================================================================*/
	 ...
	 ------------------------------------------------------------------------------------------------------	
	      --Datos empresa--
	       ...
	       --Datos venta--
	       ve.Cd_Vta,convert(char,ve.FecMov,103) as FecMov,convert(char,ve.FecCbr,103) as FecCbr,' ' as Cd_Sr,ve.NroSre as NroSerie,' ' as NroAutSunat,/*ve.Cd_Sr,se.NroSerie,nu.NroAutSunat,*/convert(char,ve.FecED,103) as FecED,convert(char,ve.FecVD,103) as FecVD,
		/*ve.Cd_MR,ve.Obs,ve.INF,ve.EXO,ve.BIM,ve.BIM + ve.INF+ ve.EXO as BIM_Vta,*/
		ve.Cd_MR,ve.Obs,ve.INF_Neto as INF,ve.EXO_Neto as EXO,ve.BIM_NETO as BIM,ve.BIM_Neto + ve.INF_Neto + ve.EXO_Neto as BIM_Vta,ve.IGV as IGV_Vta,ve.Total as Total_Vta,ve.CamMda,ve.FecMov as FecMov,ve.UsuCrea,
	       --Datos Vendedor--
	...
	left join Cliente2 ca on ve.RucE=ca.RucE and ca.Cd_Clt=ve.Cd_Clt
	...
	um.RucE=de.RucE and de.ID_UMP=um.ID_UMP and um.Cd_Prod=de.Cd_Prod and
	de.RucE=se.RucE and de.Cd_Srv=se.Cd_Srv and
	se.RucE=gr.RucE and se.Cd_GS=gr.Cd_GS
