...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Vta_Venta2Crea4]
@RucE nvarchar(11),
...
@DR_NDoc  nvarchar(15),
@IB_TieneRet bit,
@MontoRet decimal(20,7),
@MovVtaCtb bit = null output,
------------------------------
@msj varchar(100) output,
@direccionFacturacion varchar(400) = ''
as
...
if @MovVtaCtb is null 
	set @MovVtaCtb =  0
---------------------------------
if(User123.VPrdo(@RucE,@Eje,SubString(@RegCtb,8,2)) = 1)
	set @msj = 'Venta no puede ser creada, el periodo '+User123.DamePeriodo(SubString(@RegCtb,8,2))+' no se encuentra habilitado'
--------- Verif 1: ----------
else if exists (select * from Venta where RucE=@RucE and Cd_TD=@Cd_TD and NroDoc=@NroDoc and NroSre=@NroSre)
	set @msj = 'Ya existe una venta con la misma serie y numero de documento: ' + @NroSre +' - '+ @NroDoc
...
begin
	...
	insert into Venta(RucE,Cd_Vta,Eje,Prdo,RegCtb,FecMov,Cd_FPC,FecCbr,Cd_TD,NroDoc,
			  ...
			  DR_CdVta,DR_FecED,DR_CdTD,DR_NSre,DR_NDoc,IB_TieneRetencion,Monto_Retencion,DireccionFacturacion)
		   values(@RucE,@Cd_Vta,@Eje,@Prdo,@RegCtb,@FecMov,@Cd_FPC,@FecCbr,@Cd_TD,@NroDoc,
			  ...
			  @Cd_OP,@Cd_CC,@Cd_SC,@Cd_SS,@NroSre,@Cd_MIS,@NroOP,
			  --Documento Referencia
			  @DR_CdVta,@DR_FecED,@DR_CdTD,@DR_NSre,@DR_NDoc,@IB_TieneRet,@MontoRet,@direccionFacturacion)
	if @@rowcount <= 0
	begin
...
--JV: 2013-04-03: Se agrega dirección de facturacion

print('Se crea sp para creación de venta con dirección de facturación')
GO
