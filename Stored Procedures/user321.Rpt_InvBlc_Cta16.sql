...
SET ANSI_NULLS ON
GO
CREATE Procedure [user321].[Rpt_InvBlc_Cta16]
@RucE nvarchar(11),
@Ejer varchar(4),
@Cd_Mda char(2),
@FecIni datetime,
...
as
	if not exists ( select top 1 *from voucher where RucE=@RucE and Ejer=@Ejer and NroCta like '16%')
	...
	--Cuerpo Reporte
		...
		where 	vou.RucE=@RucE and vou.Ejer=@Ejer and Vou.NroCta like '16%' and vou.FecMov between Convert(nvarchar,@FecIni,103) and Convert(nvarchar,@FecFin,103)
