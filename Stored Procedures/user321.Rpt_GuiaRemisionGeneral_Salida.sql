...
SET ANSI_NULLS ON
GO
CREATE procedure [user321].[Rpt_GuiaRemisionGeneral_Salida]
@RucE nvarchar(11),
@Cd_GR char(10),
@NroSre varchar(5),
@NroGR varchar(15),
...
begin
/******************GUIA REMISION DATOS*************************************/
Select top 1 gr.Cd_CC, gr.Cd_SC, gr.Cd_SS,gr.RucE, gr.Cd_GR, gr.FecEmi, gr.FecIniTras, gr.NroSre as NoSerie, gr.NroGR as NumeroGuia,
...
from      VGuiaRemision as gr Left join VGuiaRemisionLLegada as GRL on GRL.RucE=GR.RucE and GRL.Cd_GR=GR.Cd_GR
          ...
          left join Venta on Venta.RucE = GuiaXVenta.RucE and Venta.Cd_Vta = GuiaXVenta.Cd_Vta
Where GR.Cd_GR =@Cd_GR and GR.RucE = @RucE AND GR.IC_ES='S'
...
from     GuiaRemision GR inner join  GuiaRemisionDet GRD on GRD.RucE=GR.RucE and GRD.Cd_GR=GR.Cd_GR inner join Producto2 P2 on P2.RucE=GRD.RucE and P2.Cd_Prod=GRD.Cd_Prod
                inner join Prod_UM PM on PM.Cd_Prod=GRD.Cd_Prod and PM.RucE=GRD.RucE and PM.ID_UMP=GRD.ID_UMP
                inner join UnidadMedida UM on UM.Cd_UM=PM.Cd_UM
where GRD.Cd_GR=@Cd_GR and GRD.RucE=@RucE and GR.IC_ES='S'
...
/*
ejemplo
exec Rpt_GuiaRemisionGeneral_Salida '11111111111','GR00000083',null,null,null,null
*/
/********Creado por Javier A.*********************************/
/************************************************************/
