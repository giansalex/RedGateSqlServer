...
SET ANSI_NULLS ON
GO
CREATE procedure [pvo].[Rpt_CtasXCbr_EstCta8]
@RucE nvarchar(11),
@Ejer nvarchar(4),
@Cd_Clt char(10),--Modificado, antes era @Cd_Aux nvarchar(7)
@NroCta1 nvarchar(10),
--@Prdo nvarchar(2),
@NroCta2 nvarchar(10),
...
--PRINT datediff(second,v.FecMov,@FechaAl)
		...
		select 	
			...
			sum(case(v.IB_EsProv) when '1' then datediff(day,@FechaAl,IsNull(v.FecCbr,IsNuLL(v.FecED,IsNull(v.FecMov,'Sin EspecificaciÃ³n')))) else 0 end) as Saldo_Dias,
...
--(Otra Forma)	where v.RucE=@RucE and v.Ejer=@Ejer and (v.NroCta between @NroCta1 and @NroCta2) and v.Cd_Aux= @Cd_Aux and /*(v.FecMov <= @FechaAl)*/ datediff(day,FecMov,@FechaAl) >=0 and p.IB_CtasXCbr=1 and v.IB_Anulado<>'1'
		...
		group by a.NDoc, a.ApPat,a.ApMat,a.Nom,a.RSocial,
			 v.RegCtb,v.NroCta,v.Cd_TD, v.NroSre, v.NroDoc, v.FecMov,datediff(day,v.FecMov,@FechaAl),v.Cd_MdRg
...
--MP: VIE 17-09-2010 --> Se quito las referencias a la tabla Auxiliar y se enzalo con Cliente2
