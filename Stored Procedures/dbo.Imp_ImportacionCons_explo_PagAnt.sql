...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Imp_ImportacionCons_explo_PagAnt]
@RucE nvarchar(11),
@FecD datetime,
@FecH datetime,
@Colum varchar(100),
...
as
	...
	set @Consulta = '	select *from(select top '+ convert(nvarchar,@TamPag) +'
		...
		from '+@inter+' 
		where '+@Cond+ ' order by a.Cd_IP desc)  as Impor order by Cd_IP' 
	Exec (@Consulta)
	
	set @sql = 'select top 1 @RMin = Cd_IP from(select top '+ convert(nvarchar,@TamPag) +' a.Cd_IP from ' + @Inter + ' where ' +@Cond+' order by a.Cd_IP desc)  as Impor order by Cd_IP'
	exec sp_executesql @sql, N'@RMin char(7) output', @Min output	
	set @sql = 'select top 1 @RMax = a.Cd_IP  from ' + @Inter + ' where ' + @Cond +' order by a.Cd_IP desc'
