...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Rpt_VoucherCons_F] --procedimiento final de la consulta de vouchers para imprimir en reporte
@RucE nvarchar(11),
@Ejer nvarchar(4),
@RegCtb1 nvarchar(15),
@RegCtb2 nvarchar(15),
...
Set @RucE = '20512635025' Set @Ejer = '2009' Set @RegCtb1 = 'TSGN_CB10-00066' Set @RegCtb2 = 'TSGN_CB10-00066'
*/
...
select v.RegCtb,Max(convert(varchar,v.FecED,103)) as FecDoc,Max(convert(varchar,v.FecMov,103)) as FecReg 
       ,Max(Case(v.Cd_Fte) when 'CB' then isnull(a.RSocial,a.ApPAt+' '+a.ApMat+' '+a.Nom) else '' end) as NomAux
from Voucher v 
left join Auxiliar a On a.RucE=v.RucE and a.Cd_Aux=v.Cd_Aux
where v.RucE=@RucE and v.Ejer=@Ejer
      and v.RegCtb between @RegCtb1 and @RegCtb2
