...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Rpt_VoucherCons_LM3]
...
SET @SQL3 = '
	  ...
	  from voucher vou
	  left join PlanCtas p on p.RucE = vou.RucE and p.Ejer = vou.Ejer and p.NroCta = vou.NroCta
	  where vou.RucE='''+@RucE+''' and vou.Ejer='''+@Ejer+''' and vou.Prdo < '''+@PrdoIni+''' and vou.Cd_Fte in ('+@Tipos+')'+' and vou.NroCta >= '''+@Rango1+''' and vou.NroCta <= '''+@Rango2+''' /*and vou.IB_Anulado<>1*/  Group by vou.RucE,vou.NroCta,p.NomCta
	  UNION ALL
	  Select 
	  ...
	  where v.RucE='''+@RucE+''' and v.Ejer='''+@Ejer+''' and v.Prdo >= '''+@PrdoIni+''' and v.Cd_Fte in ('+@Tipos+')'+' and v.NroCta >= '''+@Rango1+''' and v.NroCta <= '''+@Rango2+''' and 
		...
		/*and v.IB_Anulado<>1*/ 
	  Group by v.RucE,v.NroCta,p.NomCta
	  Order by NroCta'

PRINT @SQL3
EXEC sp_executesql @SQL3
end
--<Creado: Javier 27/08/2012>
--Se agrego el medio de pago, asi como el nombre de cuenta en lña tercera consulta

--sp_help Rpt_VoucherCons_LM3
--exec Rpt_VoucherCons_LM3 '11111111111','2012','08','08','01','''LD'',''CB'',''RV'',''RC''','10.0.0.01','9999999999',''
GO
