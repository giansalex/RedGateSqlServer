...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Rpt_Venta_CampoAdd3] 
...
set @CampoInsp = (select Valor from CampoV where RucE=@RucE and Cd_Vta=@Cd_Vta and Cd_Cp='01')
	...
	select 
	       ...
	       /*case(isnull(len(de.CA01),0)) when 0 then '--1--' else CA01 end as CAMPINF01,
	       case(isnull(len(de.CA02),0)) when 0 then '--2--' else CA02 end as CAMPINF02,
	       case(isnull(len(de.CA03),0)) when 0 then '--3--' else CA03 end as CAMPINF03,
               case(isnull(len(de.CA04),0)) when 0 then '--4--' else CA04 end as CAMPINF04,
               case(isnull(len(de.CA05),0)) when 0 then '--5--' else CA05 end as CAMPINF05,
	       case(isnull(len(de.CA06),0)) when 0 then '--6--' else CA06 end as CAMPINF06,
               case(isnull(len(de.CA07),0)) when 0 then '--7--' else CA07 end as CAMPINF07,
	       case(isnull(len(de.CA08),0)) when 0 then '--8--' else CA08 end as CAMPINF08,
	       case(isnull(len(de.CA09),0)) when 0 then '--9--' else CA09 end as CAMPINF09,
	       case(isnull(len(de.CA10),0)) when 0 then '--0--' else CA10 end as CAMPINF10*/
               -------------------------------------------------------------------------------------------------------	       
