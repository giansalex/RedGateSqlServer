...
SET ANSI_NULLS ON
GO
CREATE proc [dbo].[Prd_OrdFabricacionCons_explo_PagSig1]
...
set @Inter = 'OrdFabricacion ofb
		left join Producto2 prod on ofb.Cd_Prod = prod.Cd_Prod and prod.RucE = ofb.RucE
		left join EstadoOF eof on ofb.Id_EstOF = eof.Id_EstOF
		left join Prod_UM um on um.RucE =  ofb.RucE and um.ID_UMP = ofb.ID_UMP and um.Cd_Prod = ofb.Cd_Prod
		left join Almacen alm on alm.RucE =  ofb.RucE and alm.Cd_Alm = ofb.Cd_Alm
		left join Area area on area.RucE =  ofb.RucE and area.Cd_Area = ofb.Cd_Area
		left join Moneda mon on mon.Cd_Mda = ofb.Cd_Mda 
		left join CfgAutorizacion ca on ca.Tipo = ofb.TipAut and Cd_DMA = ''OF'' and ca.RucE =  ofb.RucE'
...
begin
	set @Cond = ' Cd_OF not in (select CdOF_BASE from ordfabricacion where RucE = '''+@RucE+''' and CdOF_BASE is not null) and ofb.RucE= '''+@RucE+''' and ofb.Cd_OF >'''+isnull(@Ult_CdOF,'')+''''
end
else
begin
	set @Cond = ' Cd_OF not in (select CdOF_BASE from ordfabricacion where RucE = '''+@RucE+''' and CdOF_BASE is not null) and ofb.RucE= '''+@RucE+''' and ofb.FecE between '''+
	Convert(nvarchar,@FecD,103)+''' and '''+	Convert(nvarchar,@FecH,103)+''' and ofb.Cd_OF >'''+isnull(@Ult_CdOF,'')+''''
end
...
-- exec Prd_OrdFabricacionCons_explo_PagSig1  '20546110720', '01/02/2010', '01/04/2013', null, null, 100, '', null, null, null, null, null
