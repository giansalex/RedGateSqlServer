...
CREATE proc [dbo].[Fab_FabricacionCons_explo_PagAnt]
...
set @sql = 'select top 1 @RMax = Cd_Fab from '+@Inter+' where  '+@Cond+' order by Cd_Fab desc'
exec sp_executesql @sql, N'@RMax char(10) output', @Max output

set @sql = 'select @RMin = min(Cd_Fab) from(select top '+Convert(nvarchar,@TamPag)+' Cd_Fab from '+@Inter+' where  '+@Cond+' order by Cd_Fab desc) as FabFabricacion'
