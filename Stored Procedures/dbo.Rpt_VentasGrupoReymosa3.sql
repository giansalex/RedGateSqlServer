...
CREATE procedure [dbo].[Rpt_VentasGrupoReymosa3]
@RucE nvarchar(11),
@Ejer nvarchar(4),
@Cd_Vdr nvarchar(10),
@Cd_Clt nvarchar(10),
...
as
--Rpt_VentasGrupoReymosa2 '20102028687','2012','','','01/12/2012','31/12/2012',0,'02','P','4','asc'
--Rpt_VentasGrupoReymosa3 '20102028687','2012'   ,''    ,''  ,'05/12/2012','05/12/2012'     ,0       ,'02'        ,'P'        ,'4'       ,'asc'       ,''       ,'PD00099'
--Rpt_VentasGrupoReymosa3 RUC__EMPRESA, _EJER, CD_VDR, CD_CLT,   FEC_INI,    FEC_FIN,   IB_INCLUIR,  CD_MDA,   IC_InPrdSrv,  ColOrder,   TipOrder,   Cd_Clase, Cd_Prod
...
,(case when isnull(v.IB_Anulado,0)=1 then 0.0 else case when isnull(vd.IGV,0) <> 0 then case when '''+@Cd_Mda+''' = v.Cd_Mda then isnull(vd.IMP,0.00) else case when '''+@Cd_Mda+''' = ''01'' then isnull(v.CamMda,0.00)*isnull(vd.IMP,0.00) else isnull(vd.IMP,0.00)/isnull(v.CamMda,0.00)end end else 0.00 end end)/vd.Cant as PrecioU
