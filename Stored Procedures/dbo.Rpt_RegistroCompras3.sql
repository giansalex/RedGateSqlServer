...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Rpt_RegistroCompras3]
@RucE nvarchar(11),
@Ejer nvarchar(4),
@Rprdo1 nvarchar(2),
@Rprdo2 nvarchar(2),
...
Begin
	...
	begin
		...
		select  top 1 
			...
			@RPrdo2 as Prdo2,
			'--' as Cd_Fte,
			Convert(nvarchar,getdate(),103) as FecMov,--convert(varchar,v.FecMov,103) FecMov,
			'--' as Cd_TD,
			'-----' as NroSre,
			'-----' as NroDoc,
			'--' as Cd_TDI,
	...
	*/
	...
	--Order by 2
		...
		select 
			...
			--Case(isnull(len(Max(/*v.Cd_Aux*/v.Cd_Prv)),'0')) when '0' then '*** No Registrado ***' else isnull(Max(a.RSocial),(isnull(Max(isnull(a.ApPat,''))+' '+Max(isnull(a.ApMat,''))+' '+Max(isnull(a.Nom,'')),'*** No Existe en Data ***'))) end NomAux,
			...
			Max(v.DR_NCND) as DR_NCND,Max(v.DR_NroDet) as DR_NroDet,Max(v.DR_FecDet) as DR_FecDet-- esta con max aunke creo k son ! PP
		from Voucher v
		left join Empresa e on v.RucE=e.Ruc
		--left join Auxiliar a on v.RucE=a.RucE and v.Cd_Aux=a.Cd_Aux
		...
		where v.RucE=@RucE and v.Ejer=@Ejer and v.Prdo between @RPrdo1 and @RPrdo2 and v.Cd_Fte='RC' --and (left(v.NroCta,2) in ('42','46'))-- and isnull(len(rtrim(v.IC_TipAfec)),0)=0 --VALIDAR  DE ACUERDO A LAS CUENTAS ASIGNADAS A CUENTAS POR PAGAR Y AUXILIAR
		Group by v.RucE,e.RSocial,v.Ejer,v.RegCtb,/*v.DR_NCND,v.DR_NroDet,v.DR_FecDet,*/v.Cd_Fte,v.IB_Anulado--,v.RegCtb,v.DR_FecED,v.DR_CdTD,v.DR_NSre,v.DR_NDoc/*,v.FecED,v.Cd_TD,v.NroSre,v.NroDoc,a.Cd_TDI,a.NDoc*//*,a.RSocial,a.ApPat,a.ApMat,a.Nom*/--,v.Cd_Aux
		Order by 3,8,9,10
	end
	-------------------------------------------------------------------------------------------------
...
-------------------------
	...
	Sum(
		Case(left(v.NroCta,6)) 
			when '40.1.0' then v.MtoD-v.MtoH  
			else 0 end + 
		Case(v.NroCta)         
			when '40.1.1.01'  then v.MtoD-v.MtoH 
			when '40.1.1.10' then v.MtoD-v.MtoH 
			when '40.1.1.50' then v.MtoD-v.MtoH -- agregado para la Empresa GMC
			when '40.1.7.20' then v.MtoD-v.MtoH -- agregado para la Empresa GMV
			else 0 end + 
		Case(left(v.NroCta,10))  
			when '40.1.1.11' then v.MtoD-v.MtoH -- agregado para la Empresa de Miguel Tubillas
			when '2002.01.01' then v.MtoD-v.MtoH
			when '2002.01.00' Then v.MtoD-v.MtoH  
			else 0 end +
		Case When v.NroCta in ('92.4.1.10','94.4.1.10') and @RucE='20528206931' /*PRECCO*/
			Then v.MtoD-v.MtoH
			else 0 end
	) IGV,
	Sum(Case(v.NroCta) when p.QCtg then v.MtoD-v.MtoH 
	when '40.1.2.01' then v.MtoD-v.MtoH 
	when '40.1.7.02' then v.MtoD-v.MtoH 
	
	when '40.1.7.20'  then v.MtoD-v.MtoH -- Empresa HARVEST ROCK (solicitado por Bryan)

	when '40.1.1.30' then v.MtoD-v.MtoH else 0 end) as Otros,
	sum(Case(v.NroCta) 
	when '40.1.0.02' then v.MtoD-v.MtoH else 0 end) Total
	from Voucher v
...
--		Order by 3		
	...
	*/
	...
	v.RucE,
	v.RegCtb,
	Sum(Case(left(v.NroCta,6)) 
	--when '40.1.0' then v.MtoD-v.MtoH  else 0 end + Case(v.NroCta) when '40.1.1.01'  then v.MtoD-v.MtoH 
	when '40.1.0' then v.MtoD_ME-v.MtoH_ME  else 0 end + Case(v.NroCta) when '40.1.1.01'  then v.MtoD_ME-v.MtoH_ME
	--when '40.1.1.10' then v.MtoD-v.MtoH else 0 end + Case(left(v.NroCta,10)) 
	when '40.1.1.50' then v.MtoD_ME-v.MtoH_ME -- agregado para la Empresa GMC
	when '40.1.7.20' then v.MtoD_ME-v.MtoH_ME -- agregado para la Empresa GMC

	when '40.1.1.10' then v.MtoD_ME-v.MtoH_ME else 0 end + Case(left(v.NroCta,10)) 
	when '40.1.1.11' then v.MtoD_ME-v.MtoH_ME -- agregado para la Empresa de Miguel Tubillas
	--when '2002.01.01' then v.MtoD-v.MtoH  else 0 end) IGV,
	when '2002.01.01' then v.MtoD_ME-v.MtoH_ME
	when '2002.01.00' Then v.MtoD_ME-v.MtoH_ME   else 0 end +
	Case When v.NroCta in ('92.4.1.10','94.4.1.10') and @RucE='20528206931' /*PRECCO*/
		Then v.MtoD-v.MtoH
		else 0 end
	) IGV,
	Sum(Case(v.NroCta) 
	--when p.QCtg then v.MtoD-v.MtoH 
	when p.QCtg then v.MtoD_ME-v.MtoH_ME
	--when '40.1.2.01' then v.MtoD-v.MtoH
	when '40.1.2.01' then v.MtoD_ME-v.MtoH_ME  
	--when '40.1.7.02' then v.MtoD-v.MtoH 

	--when '40.1.7.20'  then v.MtoD-v.MtoH 
	when '40.1.7.20' then v.MtoD_ME-v.MtoH_ME  -- Empresa HARVEST ROCK (solicitado por Bryan)

	when '40.1.7.02' then v.MtoD_ME-v.MtoH_ME  
	--when '40.1.1.30' then v.MtoD-v.MtoH 
	when '40.1.1.30' then v.MtoD_ME-v.MtoH_ME   
	else 0 end) as Otros,
	--sum(Case(v.NroCta) when '40.1.0.02' then v.MtoD-v.MtoH else 0 end) Total
	sum(Case(v.NroCta) when '40.1.0.02' then v.MtoD_ME-v.MtoH_ME else 0 end) Total
	from Voucher v
	left join PlanCtasDef p on v.RucE=p.RucE and p.Ejer=v.Ejer
	where v.RucE=@RucE and v.Ejer=@Ejer and v.Prdo between @RPrdo1 and @RPrdo2 and v.Cd_Fte='RC'---and (left(v.NroCta,4)='40.1' or v.NroCta in ('40.1.0.01','40.1.7.20') or len(v.IC_TipAfec)>0) --and v.RucE=p.RucE
	Group by v.RucE,v.RegCtb
...
-- PP : 2010-09-03 14:01:50.877	 : <Creacion del procedimiento almacenado>
-- J : 03/09/2010 <ModificaciÃ³n : 
--	El registro de compras mostraba el asiento duplicado debido a que uno de los registros tenia el Nro de detracciÃ³n 
--	nula se agregÃ³ la sentencia Max (--Max(v.DR_NCND) as DR_NCND,Max(v.DR_NroDet) as DR_NroDet,Max(v.DR_FecDet) as DR_FecDet) 
--	para que solo tomarÃ¡ el asiento que solo tenia registrado el nÃºmero mÃ¡s no el nulo  >
-- J : 08/09/2010 <ModificaciÃ³n : 
--	El registro de compras, cuando daban la opcion en dolares el monto del IGV salia el mismo que el de soles era porque tomaba el debe y haber en moneda nacional
--	mas no en moneda extranjera, se cambio eso. 
-- J : 08/09/2010 <ModificaciÃ³n : 
--	Se agrego la cuenta 40.1.1.11 para que mostrara el IGV en la Empresa de Ruc 20511812632

-- PV: Lun 27/09/2010 Mdf: Se agrego cuenta 40.1.7.20 de IGV para que salga en el Reg de Comp. -- Empresa HARVEST ROCK (solicitado por Bryan)  <<-- mdf fue replicada desde V1



-- Ejemplo
-- exec Rpt_RegistroCompras3 '11111111111','2010','01','05','01',null

--MP: DOM 19-09-2010 Modf: Se quito las referencias a la tabla Auxiliar y se enlazo con Proveedor2
--CM: PR03
--CM: RA01
--DI: 17/02/2011 <Se agrego los IGV que usa GMC>

--DI: 12/04/2011 <Se agrego la relacion de Cliente 2 ya que algunos datos se encuentran en cliente y no en proveedor> 
