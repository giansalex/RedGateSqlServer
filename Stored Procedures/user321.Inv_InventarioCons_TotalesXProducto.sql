...
SET ANSI_NULLS OFF
GO
CREATE procedure [user321].[Inv_InventarioCons_TotalesXProducto]
@RucE nvarchar(11),
@Cd_Prod char(7),
@FecD datetime,
@FecH datetime,
...
@Dato varchar(100),
--------------------------------------
--@TamPag int, --Tamano de la Pagina
@Ult_CdInv char(12),
...
--	set @Cond = 'a.RucE=''' + @RucE + ''' and d.Cd_Prod='''+@Cd_Prod+''' and a.FecMov between '''+ convert(nvarchar,@FecD,103)+''' and '''+Convert(nvarchar,@FecH,103)+''' and a.Cd_Inv > '''+Convert(nvarchar, isnull(@Ult_CdInv,'')) +''''
	...
	set @Consulta = 'select  ''Total>>>>>>>'' as t1,''>>>>'' as t2, ''>>>>'' as t3 ,''>>>>'' as t4,*, ECant-SCant as Suma_Cantidad, ETotal-STotal as SldoCosto, 
			 ...
			 from(
				...
				'+@inter+' 
				where 
				'+ @Cond+ ' group by a.Cd_Prod, d.Nombre1, d.Descrip 
			) as inv 
...
/*		if(@Ult_CdInv is null) -- si es primera pagina y primera busqueda
		  begin
		    set @sql = 'select @Regs = count(*) from '+@inter+' where '+@Cond --( '+@Consulta +' ) as invT
