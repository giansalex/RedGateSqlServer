...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Rpt_Ingreso_Egreso_SC]
...
Set @SQL3_2 =
	...
	begin
		...
		Set @Part21_N1 = ''	,'''''''' as Total
				...
				and v.Prdo between ''''''+@Prdo1+'''''' and ''''''+@Prdo2+'''''' and v.Cd_CC=''''''+@Cd_CC+''''''
...
Set @SQL4_2 =
	...
	begin
		...
		Set @Part2_N3 = '' ,'''''''' as Total
				     From Voucher v 
			left join PlanCtas p On p.RucE=v.RucE p.Ejer=v.Ejer and p.NroCta=left(v.NroCta,6)
			where v.RucE=''''''+@RucE+'''''' and v.Ejer=''''''+@Ejer+'''''' and v.IB_Anulado=0 and left(v.nroCta,''+@N_Min+'') in (select p.NroCta from PlanCtas p where p.RucE=''''''+@RucE+'''''' and p.Ejer=''''''+@Ejer+'''''' and p.IC_IE'+@opc+'=''''E'''' Group by p.NroCta)
			and v.Prdo between ''''''+@Prdo1+'''''' and ''''''+@Prdo2+'''''' and v.Cd_CC=''''''+@Cd_CC+''''''
			Group by left(v.NroCta,6),p.NomCta
			''
...
Set @SQL6 = 
	...
	Set @Tot1_P2 = '' ,'''''''' as Total
			   From Voucher v 
			where v.RucE=''''''+@RucE+'''''' and v.Ejer=''''''+@Ejer+'''''' and v.IB_Anulado=0 and left(v.nroCta,''+@N_Min+'') in (select p.NroCta from PlanCtas p where p.RucE=''''''+@RucE+'''''' and p.IC_IE'+@opc+'=''''I'''' Group by p.NroCta)
			and v.Prdo between ''''''+@Prdo1+'''''' and ''''''+@Prdo2+'''''' and v.Cd_CC=''''''+@Cd_CC+''''''
			''
	...
	Set @Tot_P2 = '' ,'''''''' as Total
			   From Voucher v 
			where v.RucE=''''''+@RucE+'''''' and v.Ejer=''''''+@Ejer+'''''' and v.IB_Anulado=0 and left(v.nroCta,''+@N_Min+'') in (select p.NroCta from PlanCtas p where p.RucE=''''''+@RucE+'''''' and p.IC_IE'+@opc+'=''''E'''' Group by p.NroCta)
			and v.Prdo between ''''''+@Prdo1+'''''' and ''''''+@Prdo2+'''''' and v.Cd_CC=''''''+@Cd_CC+''''''
			''
