...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Rpt_RegistroVentas_X_Num]
@RucE nvarchar(11),
@Eje nvarchar(4),
@Rprdo1 nvarchar(2),
@Rprdo2 nvarchar(2),
...
select
	...
	/*case(isnull(len(ca.RSocial),0))
	     when 0 then ca.ApPat+' '+ca.ApMat+' '+ca.Nom else ca.RSocial end as NomCli, */ --<<-- Modificado en linea 32,33
	case(isnull(len(c2.RSocial),0))--<<-- Nueva
	     when 0 then c2.ApPat+' '+c2.ApMat+' '+c2.Nom else c2.RSocial end as NomCli, --<<-- Nueva
	
	/*v.BIM, v.EXO, v.INF, v.IGV, v.Total,*/v.NroDoc as Grupo,@Rprdo1+' - '+@Rprdo2+' del '+@Eje as Prdo, --v.CamMda
  ...
  --   Case(IB_Anulado)  when 0 then ( case(v.Cd_Mda) when '02' then v.Total*v.CamMda else v.Total end) else '0.00' end as Total, 
	...
	Cliente2 c2
	where v.RucE=@RucE and v.Eje=@Eje and  v.Prdo >= @Rprdo1 and v.Prdo <= @Rprdo2 and v.RucE=e.Ruc and /*v.RucE=s.RucE and v.Cd_Sr_NO=s.Cd_Sr and*/ 
	...
	v.Cd_Clt=c2.Cd_Clt --<<-- Nueva, no son necesarios los demas
order by 8 asc
end
print @msj
...
--CAM 20/09/2010 Modificado RA01: Quite las tablas Cliente y Auxiliar. Agregue CLiente2
--JA /13/04/2012 modificado Los montos lo converti a decimal con dos decimales
--sp_help Rpt_RegistroVentas_X_Num
--exec Rpt_RegistroVentas_X_Num '11111111111','2010','01','09','01',''
