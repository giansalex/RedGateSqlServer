...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Rpt_Ingreso_Egreso_SC2]
...
--PV: OJO con @SQL2 y @Cadena (deberia hacer que aguante mas de 4000 caracteres)
		...
		While @@Fetch_Status = 0
			Begin
				...
				Begin	Set @Cadena_P1 = @Cadena_P1+isnull(''
					,Sum(Case(v.Cd_SC)when ''''''+@Cd_SC+'''''' then (v.MtoD''+@TMda+''-v.MtoH''+@TMda+'')*-1 else 0.00 end) as ''''''+@NCorto+'''''''','''')
				End
	'
Set @SQL2_2 =
	'			If(@i> 25 and @i<= 50)
...
Set @SQL3_2 =
	...
	begin	Set @Part11_N1 = '' Select 1 as ind,Tem.NroCta,Tem.NomCta''
		Set @Part21_N1 = ''	,'''''''' as Total
				...
				Inner Join Voucher v On v.RucE=''''''+@RucE+'''''' and v.Ejer=''''''+@Ejer+'''''' and v.Prdo between ''''''+@Prdo1+'''''' and ''''''+@Prdo2+'''''' and left(v.NroCta,2)=Tem.NroCta and v.IB_Anulado=0  and v.Cd_CC=''''''+@Cd_CC+''''''
...
Set @SQL4_1 =
	...
	begin
		...
		Set @Part21_N2 = ''	,'''''''' as Total
				...
				Inner Join Voucher v On v.RucE=''''''+@RucE+'''''' and v.Ejer=''''''+@Ejer+'''''' and v.Prdo between ''''''+@Prdo1+'''''' and ''''''+@Prdo2+'''''' and left(v.NroCta,4)=Tem.NroCta and v.IB_Anulado=0 and v.Cd_CC=''''''+@Cd_CC+''''''
				Group by Tem.NroCta,Tem.NomCta
			      	''
...
Set @SQL4_2 =
	...
	begin
		...
		Set @Part21_N3 = ''	,'''''''' as Total
				...
				Inner Join Voucher v On v.RucE=''''''+@RucE+'''''' and v.Ejer=''''''+@Ejer+'''''' and v.Prdo between ''''''+@Prdo1+'''''' and ''''''+@Prdo2+'''''' and left(v.NroCta,6)=Tem.NroCta and v.IB_Anulado=0 and v.Cd_CC=''''''+@Cd_CC+''''''
				Group by Tem.NroCta,Tem.NomCta
			      	''	
...
Set @SQL5_1 = 
	...
	begin
		...
		Set @Part21_N4 = ''	,'''''''' as Total
				From (select p.NroCta,p.NomCta from PlanCtas p where p.RucE=''''''+@RucE+'''''' and p.Ejer=''''''+@Ejer+'''''' and len(p.NroCta)>6 and p.IC_IE''+@Opc+''=''''I'''' Group by p.NroCta,p.NomCta) As Tem
				Inner Join Voucher v On v.RucE=''''''+@RucE+'''''' and v.Ejer=''''''+@Ejer+'''''' and v.Prdo between ''''''+@Prdo1+'''''' and ''''''+@Prdo2+'''''' and v.NroCta=Tem.NroCta and v.IB_Anulado=0 and v.Cd_CC=''''''+@Cd_CC+''''''
				Group by Tem.NroCta,Tem.NomCta
			      	''
	'
...
--exec('Select Cd_CC,NCorto From CCostos Where RucE='''+@RucE+''' and Cd_CC in ('+@Datos+')')
--Salidad emergancia -- PP 12.28 :P
exec('Select Cd_SC,NCorto,Descrip From CCSub Where RucE='''+@RucE+''' and Cd_CC = '''+@Cd_CC+''' and Cd_SC in ('+@Datos+')')
...
exec Rpt_Ingreso_Egreso_SC '20000000001','2010','01','06',1,0,0,1,'01','N','1000',
'''01010101'',
''109101'',
''109101'',
''109102'',
...
-- PV 25/06/10 : Mdf: No jalaba sub centros de costo
...
-- PP 21/08/2010 12:28:45.986	: Mdf: se agrego exec('Select Cd_SC,NCorto From CCSub Where RucE='''+@RucE+''' and Cd_CC = '''+@Cd_CC+''' and Cd_CS in ('+@Datos+')') (para obtener el orden de los SC) -- Emergencia
-- DI 24/01/2011 : Mdf: se agrego año a los plan de cuentas
-- DI 11/03/2011 : Mdf; Se agrego la opcion de Funcion y Naturaleza

--PV: Prueba para Diego:
/*
