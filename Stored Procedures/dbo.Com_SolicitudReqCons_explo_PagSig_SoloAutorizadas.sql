...
CREATE procedure [dbo].[Com_SolicitudReqCons_explo_PagSig_SoloAutorizadas]
@RucE nvarchar(11),
@FecD datetime,
@FecH datetime,
@Colum varchar(100),
...
as
	...
	set @Inter = 'SolicitudReq sc
		...
		left join EstadoSR_Srv ss on ss.Id_EstSRS = sc.Id_EstSRS
	...
	begin
		set @Cond = 'sc.RucE= '''+@RucE+ ''' and sc.Cd_SR>'''+isnull(@Ult_CdSR,'')+''' and ((isnull(sc.TipAut,0) = 0) or (isnull(sc.TipAut,0) != 0 and isnull(IB_Aut,0) = 1))'
	end
	else
	begin
		set @Cond = 'sc.RucE= '''+@RucE+''' and sc.FecEmi between '''+
		Convert(nvarchar(10),@FecD,103)+''' and '''+Convert(nvarchar(10),@FecH,103)+
		''' and sc.Cd_SR>'''+isnull(@Ult_CdSR,'')+''' and ((isnull(sc.TipAut,0) = 0) or (isnull(sc.TipAut,0) != 0 and isnull(IB_Aut,0) = 1))'
	end
...
--print 'Cadena = '+@Cond
	...
	Set @CONSULTA='	select top '+Convert(nvarchar,@TamPag)+' sc.Cd_SR,sc.NroSR,
			...
			sc.Asunto,sc.Cd_Area,a.Descrip as NombreArea, sc.Obs,sc.ElaboradoPor,sc.AutorizadoPor,sc.FecReg,
			sc.FecMdf,sc.UsuCrea,sc.UsuMdf,sc.Id_EstSR,sc.Cd_CC,sc.Cd_SC,sc.Cd_SS, isnull(sc.tipAut, 0) as TipAut, isnull(cfg.DescripTip, ''**No requiere autorizacion**'') as DescripTip, case sc.IB_Aut when 1 then convert(bit,1) else convert(bit,0) end as IB_Aut,
			s.Descrip, ss.Descrip as DescripS, sc.CA01, sc.CA02,sc.CA03,sc.CA04,sc.CA05
			from '+@Inter+'
...
-- CAM: 17/05/2011 <Actualizacion: Version 4. Agregue los campos adicionales>
/*-----------------Ejemplo------------------
Declare @NroRegs int,@NroPags int,@Max char(10),@Min char(10)
exec dbo.Com_SolicitudReqCons_explo_PagSig2 '11111111111','10/01/2010','20/08/2010','Cd_SR','',20,'',@NroRegs out,@NroPags out,@Max out,@Min out,null
print @NroRegs
print @NroPags
print @Max
print @Min
