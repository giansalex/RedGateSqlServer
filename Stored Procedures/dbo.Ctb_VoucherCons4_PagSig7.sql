...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Ctb_VoucherCons4_PagSig7]
@RucE nvarchar(11),
@Eje nvarchar(4),
@PrdoIni nvarchar(2),
@PrdoFin nvarchar(2),
...
SET CONCAT_NULL_YIELDS_NULL off --Para que se pueda concatenar NULLs --PV 
	...
	set @Consulta =' select top '+Convert(nvarchar,@TamPag)+'
		...
		vou.RucE,vou.Cd_Vou, vou.RegCtb, pcta.IB_Prod,pcta.IB_Imp, vou.Cd_Fte, vou.NroCta, pcta.NomCta, 
		...
		vou.Ejer, vou.Prdo, convert(varchar(10),vou.FecMov,103) as FecMov, vou.Glosa, convert(varchar(10),vou.FecCbr,103) as FecCbr, 
		...
		as NomComCte, vou.Cd_TD, td.Descrip as DescripTD, td.NCorto as NCortoTD, 
		vou.NroSre, vou.NroDoc, vou.NroChke, vou.Grdo, 
		case vou.IB_Conc when 1 then convert(bit,1) else convert(bit,0) end as IB_Conc, 
		convert(varchar(10),vou.FecED,103) as FecED, convert(varchar(10),vou.FecVD,103) as FecVD, 	
		cc.Cd_CC, cc.Descrip as nomCC,
		s.Cd_SC, s.Descrip as nomSC,
		ss.Cd_SS, ss.Descrip as nomSS,
		vou.Cd_Area,ar.NCorto as NCortoArea,vou.Cd_MR, md.Nombre as NomMR, 	
	...
	begin
		...
		begin
			set @prueba =@Consulta + 
									' from '+@Inter+' where '+@Cond+' and vou.RegCtb in (
										select v.RegCtb
										from Voucher v
										inner join PlanCtas a on v.RucE=a.RucE and v.NroCta=a.NroCta	
										where
			...
			print @prueba				 
				...
				set @sql = 'select @RMax = max(Cd_Vou) from(select top '+Convert(nvarchar,@TamPag)+' Cd_Vou from '+@Inter+' where '+@Cond+' and vou.RegCtb in (
										select v.RegCtb 
										from Voucher v 
										inner join PlanCtas a on v.RucE=a.RucE and v.NroCta=a.NroCta	
										where 
										v.RucE='''+@RucE+''' and v.Ejer='''+@Eje+''' and v.Prdo between '''+@PrdoIni+'''  and '''+@PrdoFin+'''  
										and a.IB_Imp =1 
									   ) and vou.Cd_Fte in (''LD'',''RC'') '+' order by Cd_vou) as Voucher'
				exec sp_executesql @sql, N'@RMax int output', @Max output
				
				set @sql = 'select top 1 @RMin =Cd_Vou from '+@Inter+' where '+@Cond+' and vou.RegCtb in (
										select v.RegCtb
										from Voucher v
										inner join PlanCtas a on v.RucE=a.RucE and v.NroCta=a.NroCta	
										where
										v.RucE='''+@RucE+''' and v.Ejer='''+@Eje+''' and v.Prdo between '''+@PrdoIni+'''  and '''+@PrdoFin+'''  
										and a.IB_Imp =1 
									   ) and vou.Cd_Fte in (''LD'',''RC'') '+'order by vou.RucE, Cd_Vou'
				exec sp_executesql @sql, N'@RMin int output', @Min output
		end
		else if(@check=1)
		begin
			Exec (@Consulta + ' from '+@Inter+' where '+@Cond+' and pcta.IB_Imp =1 and vou.Cd_Fte in (''LD'',''RC'')order by Cd_Vou')
			print @consulta
				  if (isnull(@Ult_CdVou,'0')='0') -- si es primera pagina y primera busqueda
				begin
					set @sql= 'select @Regs = count(*) from '+@Inter+ '
					...
					set @sql = 'select @RMax = max(Cd_Vou) from(select top '+Convert(nvarchar,@TamPag)+' Cd_Vou from '+@Inter+' where '+@Cond+
						' and pcta.IB_Imp =1 and vou.Cd_Fte in (''LD'',''RC'') '
						+ ' order by Cd_vou) as Voucher'
					exec sp_executesql @sql, N'@RMax int output', @Max output
					
					set @sql = 'select top 1 @RMin =Cd_Vou from '+@Inter+' where '+@Cond+' and pcta.IB_Imp =1 and vou.Cd_Fte in (''LD'',''RC'') '
						+' order by vou.RucE, Cd_Vou'
					exec sp_executesql @sql, N'@RMin int output', @Min output
		end
	end
