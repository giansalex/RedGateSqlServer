...
CREATE PROCEDURE [dbo].[Cfg_CfgVenta_Crea2]
@RucE char(11)
      ,@IB_ValCltSNT bit
      ,@IB_NoPermRegCNV  bit
      ,@DE_IB_GenDE  bit
	  ...
	  ,@DE_RutaCert varchar(256)
	  ,@DE_ClaveCert varchar(100)
	  ,@DE_WS int
...
if not exists (select * from CfgVenta where RucE = @RucE )
	insert into CfgVenta(RucE ,IB_ValCltSNT, IB_NoPermRegCNV , DE_IB_GenDE , DE_IB_EnvSinc  , DE_IB_GuardarArchs , DE_RutaArchs , DE_NroResol_SNT,DE_CdWS, DE_RutaCert, DE_ClaveCert)
	values (@RucE
      ,@IB_ValCltSNT
      ,@IB_NoPermRegCNV
      ,@DE_IB_GenDE
    ...
    , DE_CdWS = @DE_WS
	,DE_RutaCert=@DE_RutaCert
	,DE_ClaveCert=@DE_ClaveCert
   where RucE = @RucE
   	end
