...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Rpt_RegistroComprasCtb_X_Usu]
@RucE nvarchar(11),
@Ejer nvarchar(4),
@Rprdo1 nvarchar(2),
@Rprdo2 nvarchar(2),
...
--		Order by 3		
	...
	*/
	...
	begin
		...
		where v.RucE=@RucE and v.Ejer=@Ejer and v.Prdo between @RPrdo1 and @RPrdo2 and v.Cd_Fte='RC' --and (left(v.NroCta,2) in ('42','46')) and isnull(len(rtrim(v.IC_TipAfec)),0)=0 --VALIDAR  DE ACUERDO A LAS CUENTAS ASIGNADAS A CUENTAS POR PAGAR Y AUXILIAR
		Group by v.RucE,v.Ejer,v.UsuCrea,v.Cd_TD,v.NroDoc,v.RegCtb,v.FecED,e.RSocial,v.Cd_Fte,v.IB_Anulado--,v.RegCtb,v.DR_FecED,v.DR_CdTD,v.DR_NSre,v.DR_NDoc/*,v.FecED,v.Cd_TD,v.NroSre,v.NroDoc,a.Cd_TDI,a.NDoc*//*,a.RSocial,a.ApPat,a.ApMat,a.Nom*/--,v.Cd_Aux
		Order by v.UsuCrea
	end
	-------------------------------------------------------------------------------------------------
	...
	*/
	where v.RucE=@RucE and v.Ejer=@Ejer and v.Prdo between @RPrdo1 and @RPrdo2 and v.Cd_Fte='RC' and (len(v.IC_TipAfec) > 0)-- or left(v.NroCta,6)='40.1.1' or v.NroCta in ('40.1.0.01'))--or v.NroCta in ('40.1.4.01'))
	Group by v.RucE,v.RegCtb,v.UsuCrea--,v.CamMda--,v.Cd_MdRg
	Order by v.UsuCrea
	-------------------------------------------------------------------------------------------------
	...
	Group by v.RucE,v.UsuCrea,v.RegCtb
	Order by v.UsuCrea
...
--		Order by 3		
	...
	*/
	...
	left join proveedor2 p on p.RucE=v.RucE and p.Cd_Prv=v.Cd_Prv
	where v.RucE=@RucE and v.Ejer=@Ejer and v.Prdo between @RPrdo1 and @RPrdo2 and v.Cd_Fte='RC' --and (left(v.NroCta,2) in ('42','46')) and isnull(len(rtrim(v.IC_TipAfec)),0)=0 --VALIDAR  DE ACUERDO A LAS CUENTAS ASIGNADAS A CUENTAS POR PAGAR Y AUXILIAR
	Group by v.RucE,v.Ejer,v.UsuCrea,v.NroSre,v.RegCtb,e.RSocial,v.Cd_Fte,v.IB_Anulado--,v.RegCtb,v.DR_FecED,v.DR_CdTD,v.DR_NSre,v.DR_NDoc/*,v.FecED,v.Cd_TD,v.NroSre,v.NroDoc,a.Cd_TDI,a.NDoc*//*,a.RSocial,a.ApPat,a.ApMat,a.Nom*/--,v.Cd_Aux
	Order by v.UsuCrea
	end
	-------------------------------------------------------------------------------------------------
	--CONSULTANDO LAS BASES IMPONIBLES
	...
	*/
	...
	left join proveedor2 pr on pr.RucE=v.RucE and pr.Cd_Prv=v.Cd_Prv
	where v.RucE=@RucE and v.Ejer=@Ejer and v.Prdo between @RPrdo1 and @RPrdo2 and v.Cd_Fte='RC'---and (left(v.NroCta,4)='40.1' or v.NroCta in ('40.1.0.01','40.1.7.20') or len(v.IC_TipAfec)>0) --and v.RucE=p.RucE
	Group by v.RucE,v.RegCtb,v.UsuCrea
	Order by v.UsuCrea
...
-- Ejemplo
-- exec Rpt_RegistroComprasCtb_X_Usu '11111111111','2010','01','05','01',null
-- JJ: 2010-09-20: Modificacion: se agrego lineas correspondientes a Proveedor2 RA01 MG02
GO
