...
SET ANSI_NULLS ON
GO
--exec Rpt_VoucherCons_LD4  '20160000001','2016','10','10','01',1,1,1,1,0,0,0,0,' ',' ',null
--exec Rpt_VoucherCons_LD4_LE  '20160000001','2016','10','10','01',1,1,1,1,0,0,0,0,' ',' ',null



CREATE Procedure [dbo].[Rpt_VoucherCons_LD4_LE]
...
BEGIN
	...
	set @Registros = ' and left(vou.NroCta,'+Convert(varchar,@var1)+') >= '''+@Rango1+''' and left(vou.NroCta,'+Convert(varchar,@var2)+') <= '''+@Rango2+''''
END
...
IF(@InfRC = 1) /**5*/
begin
	IF(@InfCB = 1 or @InfLD = 1) Set @RC_Cab = ' Union All '  /**5*/
	IF(@DetRC = 1)
	begin
		Set @RC_Cab = +@RC_Cab+'
				select
...
IF(@InfRV = 1) /**7**/
begin
	IF(@InfCB = 1 or @InfLD = 1 or @InfRC = 1) Set @RV_Cab = ' Union All ' /**8**/
	IF(@DetRV = 1)
	begin
		Set @RV_Cab = +@RV_Cab+'
				select
	...
	begin
		Set @CB_Det = '
				select
					...
					Convert(varchar,vou.FecMov,103) as FecMov, 
					Convert(varchar,vou.FecCbr,103) as FecCbr,
					Convert(varchar,vou.FecED,103) as FecED,
					Convert(varchar,vou.FecVD,103) as FecVD,

				    replace(convert(varchar,vou.NroCta,102),''.'','''') as NroCta,	
					cta.NomCta,
...
set @ConsIni='select RucE,RSocial, Cd_Fte,RegCtb,Prdo,Cd_TDIE,NDocE,FecReg,FecMov,FecCbr,FecED,FecVD,NroCta,NomCta,Cd_Aux,NomAux,TD,NroSre,NroDoc,Glosa,Sum(Debe) Debe, Sum(Haber) Haber 
			from ('
set @ConsFin=') as Cons 
			Group By RucE,RSocial, Cd_Fte,RegCtb,Prdo,Cd_TDIE,NDocE,FecReg,FecMov,FecCbr,FecED,FecVD,NroCta,NomCta,Cd_Aux,NomAux,TD,NroSre,NroDoc,Glosa
			'
...
Exec (@ConsIni+@CB_Det+@LD_Det+@RC_Det+@RV_Det+@ConsFin+' Order by 3,4,7')
end
--exec Rpt_VoucherCons_LD4 '11111111111','2012','01','12','01',1,1,1,1,1,1,1,1,'','',null
...
--Gerardo -> 23/11/2016 Se creo este procedimiento para Libro electornica de libro Diario , ya que no mostrar serie ni numero documento 
--Gerardo -> 24/11/2016 se agrego nuevos campos FecMov,FecCbr,FecED y FecVD.
GO
