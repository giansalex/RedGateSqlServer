...
CREATE procedure [dbo].[Inv_ConsultaStock_4]
@RucE nvarchar(11),
	...
	if(@Cod_Alm = '' or @Cod_Alm is null)
		begin
			exec ('select P.Cd_Prod as Codigo, T3.ID_UMP,  P.CodCo1_ as ''Codigo Comer.'',P.Nombre1 + '' '' + U.DescripAlt as Producto,Cant as Cantidad'+@col2+' from ( 
				select Cd_Prod, ID_UMP, Sum(Cant) as Cant '+@Col1+' from ( 
					select *'+@col+' from (
						select Cd_Alm, Cd_Prod, ID_UMP, sum(Cant_Ing) as Cant from Inventario where RucE = '''+@RucE+''' and FecMov <= '''+@FecH+''' group by Cd_Prod, Cd_Alm, ID_UMP)
					as T1 
		...
		begin
			exec ('select P.Cd_Prod as Codigo, T3.ID_UMP,  P.CodCo1_ as ''Codigo Comer.'',P.Nombre1 + '' '' + U.DescripAlt as Producto,Cant as Cantidad'+@col2+' from ( 
				select Cd_Prod, ID_UMP, Sum(Cant) as Cant '+@Col1+' from ( 
					select *'+@col+' from (
						select Cd_Alm, Cd_Prod, ID_UMP, sum(Cant_Ing) as Cant from Inventario where RucE = '''+@RucE+''' and Cd_Alm='''+@Cod_Alm+''' and FecMov <= '''+@FecH+''' group by Cd_Prod, Cd_Alm, ID_UMP)
					as T1 
				) as T2 group by Cd_Prod, ID_UMP
			) as T3
			inner join Producto2 as P on P.RucE = '''+@RucE+''' and P.Cd_Prod = T3.Cd_Prod
	...
	if(@Cod_Alm = '' or @Cod_Alm is null)
		begin
			EXEC ('select P.Cd_Prod as Codigo, P.CodCo1_ as ''Codigo Comer.'',P.Nombre1 as Producto,Cant as Cantidad'+@col2+' from ( 
				select Cd_Prod, Sum(Cant) as Cant '+@Col1+' from ( 
					select *'+@col+' from (
						select Cd_Alm, Cd_Prod, sum(Cant) as Cant from Inventario where RucE = '''+@RucE+''' and FecMov <= '''+@FecH+''' group by Cd_Prod, Cd_Alm)
					as T1
		...
		begin
			EXEC ('select P.Cd_Prod as Codigo, P.CodCo1_ as ''Codigo Comer.'',P.Nombre1 as Producto,Cant as Cantidad'+@col2+' from ( 
				select Cd_Prod, Sum(Cant) as Cant '+@Col1+' from ( 
					select *'+@col+' from (
						select Cd_Alm, Cd_Prod, sum(Cant) as Cant from Inventario where RucE = '''+@RucE+''' and Cd_Alm='''+@Cod_Alm+''' and FecMov <= '''+@FecH+''' group by Cd_Prod, Cd_Alm)
					as T1 
				) as T2 group by Cd_Prod
			) as T3
			inner join Producto2 as P on P.RucE = '''+@RucE+''' and P.Cd_Prod = T3.Cd_Prod
...
-- LEYENDA
-- CAM 10/11/2012 Le quite la fecha desde
-- exec Inv_ConsultaStock_4 '11111111111',null,'10/11/2012',null
GO
