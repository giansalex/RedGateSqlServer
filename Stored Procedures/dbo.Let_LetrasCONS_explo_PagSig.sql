...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Let_LetrasCONS_explo_PagSig]
...
set language 'spanish'
	...
	--Interseccion
		set @Inter = 'Canje Can inner join Area Ar on Ar.Cd_Area = Can.Cd_Area and Can.RucE = Ar.RucE
				inner join Cliente2 Clt on Clt.Cd_Clt=Can.Cd_Clt and Can.RucE = Clt.RucE
				...
				Left Join(	Select d.RucE,c.Cd_Cnj As Padre,Max(d.Cd_Cnj) As Hijo,convert(bit,isnull(c2.IB_Anulado,0)) As HijoAnulado
							From CanjeDet d Left Join Letra_Cobro l On l.RucE=d.RucE and l.Cd_Ltr=d.Cd_Ltr Left Join Canje c On c.RucE=l.RucE and c.Cd_Cnj=l.Cd_Cnj Left Join Canje c2 On c2.RucE=d.RucE and c2.Cd_Cnj=d.Cd_Cnj
	...
	declare @Consulta2 varchar(8000)
		set @Consulta='		select top '+convert(nvarchar,@TamPag)+'
				Can.Cd_Cnj, Can.RegCtb, Can.Prdo, Can.FecMov,Can.Cd_Clt,
				case when Clt.RSocial is not null then Clt.RSocial else (isnull(Clt.ApPat,'''') +'' ''+ isnull(Clt.ApMat,'''') + '' ''+ isnull(Clt.Nom,'''')) end as Cliente,			 
				...
				--print @Consulta
	if not exists (select top 1 * from Canje where RucE=@RucE)
	...
	--select top 1 * from Producto2
	  begin
		...
		if(@Ult_CdCnj is null) -- si es primera pagina y primera busqueda
		  ...
		  set @sql = 'select @RMax = max(Cd_Cnj) from(select top '+ Convert(nvarchar,@TamPag)+' Cd_Cnj  from ' + @Inter + ' where ' + @Cond +' order by Cd_Cnj) as  Canje'
...
-- Leyenda --
-- Di : 08/03/2012 <Se agrego las columnas de pElim,pAnul>
	
	------
--exec Let_LetrasCONS_explo_PagSig '11111111111','01/01/2012','28/12/2012','','',10,null,null,null,null,null,null
