...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Vta_CobroCons_explo_PagAnt]
@RucE nvarchar(11),
@Ejer nvarchar(4),
@PrdoIni datetime,
@PrdoFin datetime,
...
declare @Inter varchar(1000)
	...
	set @Consulta = '	select *from(select top '+ convert(nvarchar,@TamPag) +'
			...
			from  '+@inter+' 
			where '+@Cond+ ' order by c.ItmCo desc)  as Inventario order by ItmCo' 
	if not exists (select top 1 * from Cobro where RucE=@RucE)
	  ...
	  begin
		Exec (@Consulta)
		  ...
		  set @sql = 'select top 1 @RMax = ItmCo from ' + @Inter + ' where ' +@Cond+' order by ItmCo desc'
		  exec sp_executesql @sql, N'@RMax int output', @Max output

		  set @sql = 'select @RMin = min(ItmCo) from(select top '+ Convert(nvarchar,@TamPag)+' ItmCo  from ' + @Inter + ' where ' + @Cond +' order by ItmCo desc) as  Cobro'
