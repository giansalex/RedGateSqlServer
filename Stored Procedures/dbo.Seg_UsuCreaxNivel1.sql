...
SET ANSI_NULLS ON
GO
create procedure [dbo].[Seg_UsuCreaxNivel1]
@NomUsu	nvarchar(10),
...
begin
    insert into Usuario(NomUsu,Pass,NomComp,Cd_Trab,Cd_Prf,Nivel,Estado,IB_TipCamCrear,IB_TipCamMdf,IB_TipCamElim)
	        values (@NomUsu,@Pass,@NomComp,@Cd_Trab,@Cd_Prf,
			dbo.Asigna_NivelxUsuario(@NomUsuCrea),1,@IB_TipCamCrear,@IB_TipCamMdf,@IB_TipCamElim)
    if @@rowcount<=0
