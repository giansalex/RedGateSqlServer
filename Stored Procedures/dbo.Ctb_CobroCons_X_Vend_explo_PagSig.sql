...
SET ANSI_NULLS ON
GO
CREATE proc [dbo].[Ctb_CobroCons_X_Vend_explo_PagSig]
...
as
	...
	begin
		set @sql= 'declare @RucEE nvarchar(11)
			   ...
			   set @FecDD = '''+ Convert(nvarchar,@FecD,103) +'''
			   set @FecHH = '''+ Convert(nvarchar,@FecH,103) +'''
			   select @Regs = count(*) from '+@Inter+ '  where ' + @Cond
	...
	set @sql = 'declare @RucEE nvarchar(11)
		    ...
		    set @FecDD = '''+ Convert(nvarchar,@FecD,103) +'''
		    set @FecHH = '''+ Convert(nvarchar,@FecH,103) +'''
		    select @RMax = max(Cd_Vta) from(select top '+Convert(nvarchar,@TamPag)+' ve.Cd_Vta from '+@Inter+' where '+@Cond+' order by ve.Cd_Vta) as Venta'
	...
	set @sql = 'declare @RucEE nvarchar(11)
		    ...
		    set @FecDD = '''+ Convert(nvarchar,@FecD,103) +'''
		    set @FecHH = '''+ Convert(nvarchar,@FecH,103) +'''
		    select top 1 @RMin =Cd_Vta from '+@Inter+' where '+@Cond+' order by Cd_Vta'
	exec sp_executesql @sql, N'@RMin char(10) output', @Min output

----------------------PRUEBA------------------------
--exec Ctb_CobroCons_X_Vend_explo_PagSig '11111111111','2009','01/01/2008','30/07/2009',null,null,200,null,null,null,null,null,null

------CODIGO DE MODIFICACION--------
--CM=RE01

----------------------LEYENDA----------------------
--FL: 17/09/2010 <se agrego ejercicio en: @Inter, se declaro@EEjer y se seteo con @Ejer>
GO
